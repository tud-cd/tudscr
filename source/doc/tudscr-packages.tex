...
%  \Package{amssymb}'CTAN:amsfonts' zu verzichten und anstelle dessen 
...
\NewExpandableDocumentCommand\RecPackage{d() m}{%
  {"|Kurzbeschreiung Paket #2}@{%
    \hyperref[sec:packages:recommended]{Kurzbeschreiung Paket}: 
...
\item[widetable]
  ...
  Umgebung \Environment{widetable}(\Package{widetable}) zur Verfügung, die als 
...
\item[tabularx]
  ...
  \InlineDeclaration{\Environment{tabularx}(\Package{tabularx})}
...
\item[tabulary]
  ...
  \InlineDeclaration{\Environment{tabulary}(\Package{tabulary})}
  gewichtet vergeben.
