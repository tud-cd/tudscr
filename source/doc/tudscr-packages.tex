...
>%
  \ChangedAt(){%
    v2.02:Paket \Package{scrlayer-scrpage} ist für 
...
\itempkg{microtype}
  ...
  im Dokument. Es funktioniert nicht mit der klassischen \Format*{TeX}"~Engine, 
  wohl jedoch mit \Format*{pdfTeX} oder \Format*{LuaTeX} sowie \Format*{XeTeX}.
...
\itempkg{setspace}
  ...
  \Macro{setstretch|\MPName{Faktor}} zur Verfügung, mit 
  ...
  \Macro{setstretch|\MPValue{\baselinestretch}} für den 
...
\itempkg{multicol}
  ...
  \KOMAScript-Option \Option{twocolumn}~-- für einen 
...
\itempkg{isodate,datetime2}
  \index{Datum|?}%
  Mit \Macro{printdate|\MPName{Datum}} formatiert das 
...
\itempkg{quoting}
  ...
  \Environment{quote} und 
  \Environment{quotation}~-- für Zitate und ähnliches 
  an. Allerdings werden durch beide Umgebungen die \KOMAScript-Option 
  \Option{parskip=\PMisc} ignoriert. Mit der Umgebung 
...
\itempkg{csquotes}
  ...
  \Macro{enquote|\MPName{Zitat}} zur Verfügung, welcher 
...
\itempkg{enumitem}
  ...
  \Environment{itemize},
  \Environment{enumerate} sowie
  \Environment{description} und
  ...
  \Macro{setlist|\MPValue{noitemsep}}.
\end{DeclarePackages}
...
\itempkg{caption}
  ...
  \InlineDeclaration{\Option{captions=\PMisc}(\Bundle{koma-script})}
...
\itempkg{floatrow}
  ...
  \Macro{floatsetup|\MPValue{\dots}} vorgenommen werden. 
  So lässt sich unter anderem die verwendete Schrift innerhalb der 
...
\itempkg{array}
  Dieses Paket ermöglicht mit \Macro{newcolumntype} das 
...
\itempkg{widetable}
  ...
  \Environment{widetable} zur Verfügung, die als Alternative 
...
\itempkg{tabularx}
  ...
  der \Environment{tabularx}"~Umgebung beliebig 
...
\itempkg{tabulary}
  ...
  \Environment{tabularx} auf alle Spalten 
  gleichmäßig verteilt sondern in der 
  \Environment{tabulary}"~Umgebung für die 
...
\itempkg{longtable,xltabular,ltxtable,ltablex}
  ...
  \Environment{tabularx}"~Tabelle ist das Paket 
...
\itempkg{multirow}
  Es wird der Befehl \Macro{multirow} definiert, der 
...
\itempkg{pstricks}
  ...
  \Environment{pspicture} zunächst über den Pfad 
...
\Package{hyperref} besser noch mit \Macro{autoref} 
...
\itempkg{cleveref}
  ...
  Objekte mit dem Befehl \Macro{autoref} aus dem Paket 
...
\itempkg{subfiles}[v2.02]
  ...
  Die darin mit \Macro{subfiles} eingebundenen Unterdateien 
...
\itempkg{marginnote}
  \index{Randnotizen}%
  Mit dem Befehl \Macro{marginpar} lassen sich 
  ...
  \Macro{marginnote} für nicht"~gleitende Randnotizen zur 
...
\itempkg{xkeyval,keyval,kvoptions,pgfkeys}[v2.02]
  ...
  unterstützen, lässt sich das Paket \Package{scrbase}  
...
\itempkg{pagecolor}
  ...
  werden.
\itempkg{afterpage}
  Der Befehl \Macro{afterpage|\MPValue{\dots}} 
...
\itempkg{mparhack}
  ...
  \Macro{marginpar} bereitgestellt. Alternativ dazu 
...
\itempkg{morewrites,scrwfile}
  ...
  \Package{scrwfile} nutzen. Mehr dazu in 
