...
\begin{DeclareEntity}{\Bundle{koma-script}}
...
\itempkg{}(\Bundle{koma-script})
  ...
  \ChangedAt(){%
    v2.02:Paket \Package(\Bundle{koma-script}){scrlayer-scrpage} ist für 
    \TUDScript unabdingbar%
  }%
  Die zentrale Grundlage für \TUDScript sind die Klassen \Class{scrbook}, 
  \Class{scrreprt} und \Class{scrartcl} aus dem \scrguide[\KOMAScript-Bundle]. 
  Weiterhin wird das Paket \Package{scrbase} genutzt, was das Definieren von 
  ...
  Befehlen \Macro{TUDoption} und \Macro{TUDoptions} geändert werden können. 
  Für die \TUDScript-Klassen werden die \PageStyle{tudheadings}"=Seitenstile 
  mithilfe des Paketes \Package{scrlayer-scrpage} bereitgestellt. Wenn es nicht 
  durch den Anwender~-- mit beliebigen Optionen~-- geladen wird, erfolgt dies 
  am Ende der Präambel automatisch durch \TUDScript.
\end{DeclarePackages}
\end{DeclareEntity}
\begin{DeclarePackages}
\itempkg{opensans,iwona}[v2.06]
  \index{Schriftart|!}%
  \ChangedAt{%
  ...
  wird empfohlen, auf das Paket \Package'CTAN:amsfonts'{amssymb} zu verzichten 
...
\itempkg{graphicx}
  ...
  wird \Macro(\Package{graphicx}){includegraphics} genutzt.
\itempkg{xcolor}
  \index{Farben}%
...
\itempkg{etoolbox,xpatch,letltxmacro}
...
\itempkg{kvsetkeys}
  Das von \Package(\Bundle{koma-script}){scrbase} geladene Paket 
...
vorgestellt, welche sich für mich bei der Arbeit mit \Lettering{LaTeX} bewährt 
haben. Einige davon werden außerdem im Tutorial \TUDScriptTutorial{treatise} in 
...
\itempkg{setspace}
  ...
  \Macro(\Package{setspace}){setstretch|\MPName{Faktor}} zur Verfügung, mit 
  ...
  \Macro(\Package{setspace}){setstretch|\MPValue{\baselinestretch}} für den 
...
\itempkg{multicol}
  ...
  \KOMAScript-Option \Option(\Package{typearea}){twocolumn}~-- für einen 
...
\itempkg{isodate,datetime2}
  \index{Datum|?}%
  Mit \Macro(\Package{isodate}){printdate|\MPName{Datum}} formatiert das 
  ...
  \footnote{%
    ...
    \Package{tudscrsupervisor} \Macro(\Package{tudscrsupervisor}){duedate} und 
    \Macro(\Package{tudscrsupervisor}){issuedate}%
  }
...
\itempkg{quoting}
  ...
  \Environment(\Bundle{koma-script}){quote} und 
  \Environment(\Bundle{koma-script}){quotation}~-- für Zitate und ähnliches 
  an. Allerdings werden durch beide Umgebungen die \KOMAScript-Option 
  \Option(\Bundle{koma-script}){parskip=\PMisc} ignoriert. Mit der Umgebung 
  \Environment(\Package{quoting}){quoting} aus dem gleichnamigen Paket lässt 
  sich dieses Problem umgehen. Wird das Paket geladen, wird diese innerhalb der 
  \Environment{abstract}"~Umgebung verwendet.
\itempkg{csquotes}
  \index{Zitate}%
  Das Paket stellt unter anderem den Befehl 
  \Macro(\Package{csquotes}){enquote|\MPName{Zitat}} zur Verfügung, welcher 
...
\itempkg{xspace,xpunctuate}
  ...
  \Macro(\Package{xspace}){xspace} genutzt werden. Dieser setzt ein 
...
\itempkg{ellipsis}
  \index{Befehlsdeklaration}%
  In \Lettering{LaTeX} folgen den Befehlen für Auslassungspunkte (\Macro{dots} und 
  ...
  Satzzeichen und kein Buchstabe folgt, \seeplain*{\autoref{sec:tips:dots}}.
\itempkg{selnolig}(\Application{DeLig})
  \index{Ligaturen}%
  \ToDo[doc]{Hyperlink selnolig}[v2.07]%
  ...
  Mit \Format{LuaLaTeX} als Textsatzsystem kann auch \Package{selnolig} 
...
\itempkg{lmodern}'lm'
  \index{Schriftart}%
  Soll mit den klassischen \Lettering{LaTeX}"=Standardschriften gearbeitet werden, 
...
\begin{DeclarePackages}[Mathematiksatz]
...
\itempkg{mathtools,amsmath}
  Dieses Paket erweitert das De"~facto"=Standardpaket \Package{amsmath} und 
...
von \Lettering{LaTeX}. Insbesondere, wenn in einem deutschsprachigen Dokument 
...
\itempkg{enumitem}
  ...
  \Lettering{LaTeX}"=Standardlisten
  \Environment(\Bundle{koma-script},\Package{enumitem}){itemize},
  \Environment(\Bundle{koma-script},\Package{enumitem}){enumerate} sowie
  \Environment(\Bundle{koma-script},\Package{enumitem}){description} und
  ...
  \Macro(\Package{enumitem}){setlist|\MPValue{noitemsep}}.
\end{DeclarePackages}
...
\itempkg{caption}
  ...
  \InlineDeclaration{\Option(\Bundle{koma-script}){captions=\PMisc}}
...
\itempkg{floatrow}
  ...
  \Macro(\Package{floatrow}){floatsetup|\MPValue{\dots}} vorgenommen werden. 
  So lässt sich unter anderem die verwendete Schrift innerhalb der Umgebungen 
  \Environment{figure} und \Environment{table} mit  
  \Macro*{floatsetup|\MPValue{font=\dots}} einstellen. Mit 
  \begin{Code}
  ...
  \Macro(\Bundle{koma-script},\Package{caption}){caption} innerhalb der 
  ...
  \Option(\Bundle{koma-script}){captions=tableheading} genutzt werden.
\itempkg{placeins}
...
\itempkg{array}
  Dieses Paket ermöglicht mit \Macro(\Package{array}){newcolumntype} das   
  ...
  sich mit \Macro(\Package{array}){extrarowheight} ändern. 
\itempkg{booktabs}
...
\itempkg{widetable}
  ...
  \Environment(\Package{widetable}){widetable} zur Verfügung, die als Alternative 
...
\itempkg{tabularx}
  ...
  der \Environment(\Package{tabularx}){tabularx}"~Umgebung beliebig häufig 
  angegeben werden kann 
  (\Macro*{begin|\MPValue{tabularx}\MPName{Breite}\MPName{Spalten}}). Die 
  \PValue{X}"~Spalten ähneln denen vom Typ~\PValue{p}\MPName{Breite}, wobei die 
...
\itempkg{tabulary}
  ...
  \Environment(\Package{tabularx}){tabularx} auf alle Spalten gleichmäßig 
  verteilt sondern in der \Environment(\Package{tabulary}){tabulary}"~Umgebung 
  für die Spaltentypen~\PValue{LCRJ} anhand ihres Zellinhaltes gewichtet 
  vergeben. 
  (\Macro*{begin|\MPValue{tabulary}\MPName{Breite}\MPName{Spalten}}). 
\itempkg{longtable,ltxtable,ltablex}
  \ToDo{\Package{xltabular} als Empfehlung}[v2.07]%
  ...
  \Environment(\Package{tabularx}){tabularx}"~Tabelle ist das Paket 
...
\itempkg{multirow}
  Es wird der Befehl \Macro(\Package{multirow}){multirow} definiert, der das 
...
\itempkg{tabu}
  [
    v2.02:\Package{tabu} ist nur bedingt empfehlenswert;
    v2.07:\Package{tabu} in der aktuellen Version~v2.9 nicht zu empfehlen;
  ]
  ...
  \Environment(\Package{tabu}){tabu} und \Environment(\Package{tabu}){longtabu} 
...
Für das Erstellen eigener Vektorgrafiken, welche die \Lettering{LaTeX}"~Schriften 
...
Text an \Lettering{LaTeX} unterstützen, werden genutzt. Für das Programmieren von 
...
\itempkg{tikz}'pgf'
  ...
  Verwendung von \Format{pdfLaTeX}.
\itempkg{pstricks}
  ...
  \Environment(\Package{pstricks}){pspicture} zunächst über den Pfad 
  \Path{latex\,>\,dvips\,>\,ps2pdf} in PDF"~Dateien gewandelt werden. Diese 
  lassen sich von \Format{pdfLaTeX} anschließend als Abbildungen einbinden. Um 
  ...
  \itempkg{auto-pst-pdf,pdftricks2}
    ...
    dem Paket \Package{pst-pdf}. Hierfür muss \Format{pdfLaTeX} per Option mit 
...
Im Tutorial \TUDScriptTutorial{treatise} wird für \Package{pstricks} und 
\Package{tikz} jeweils ein Beispiel gegeben. Um bei der Erstellung von Grafiken 
...
Hierfür sind die beiden Pakete \Package{standalone} oder \Package{subfiles} 
sehr nützlich.
...
\itempkg{svg}
  ...
  SVG"~Grafik in ein \Lettering{LaTeX}"~Dokument automatisiert durchgeführt 
...
\Package{hyperref} besser noch mit \Macro(\Package{hyperref}){autoref} 
...
\itempkg{cleveref}
  ...
  Objekte mit dem Befehl \Macro(\Package{hyperref}){autoref} aus dem Paket 
...
\Lettering{TikZ}- oder \Package{pstricks}"~Grafik~-- zu kompilieren. Die meiner 
...
\itempkg{standalone}<\Class{standalone}>
  ...
  \ToDo[doc]{standalone als Bundle?}[v2.07]%
...
\itempkg{subfiles}[v2.02]
  ...
  Die darin mit \Macro(\Package{subfiles}){subfiles} eingebundenen Unterdateien 
...
leeres \Lettering{LaTeX}"~Dokument erzeugen und es unter \File*{mypreamble.sty} 
...
\itempkg{marginnote}
  \index{Randnotizen}%
  Mit dem Befehl \Macro(\Bundle{koma-script}){marginpar} lassen sich 
  Randnotizen erzeugen. Diese sind in \Lettering{LaTeX} spezielle Gleitobjekte, 
  ...
  \Macro(\Package{marginnote}){marginnote} für nicht"~gleitende Randnotizen zur 
...
\itempkg{xparse}
  ...
  \index{Umgebungen}%
  \index{Befehlsdeklaration}%
  Dieses mächtige Paket entstammt dem \Lettering{LaTeX3}"~Projekt und bietet für 
  die Erstellung eigener Befehle und Umgebungen einen alternativen Ansatz zu 
  den bekannten \Lettering{LaTeX}"=Deklarationsbefehlen \Macro*{newcommand} und 
...
\itempkg{xkeyval,keyval,kvoptions,pgfkeys}[v2.02]
  \index{Befehle}%
  \index{Umgebungen}%
  \index{Parameter}%
  \index{Befehlsdeklaration}%
  \ToDo{Hinweis auf Kernel-Funktionalitäten}[v2.07]%
  ...
  \Macro*{newcommand|\Macro*{Befehl|%
    \OPName{Schlüssel-Wert-Liste}\MPName{Argument}}}
  ...
  unterstützen, lässt sich das Paket \Package(\Bundle{koma-script}){scrbase}  
...
\itempkg{scrlfile}[v2.05]'koma-script'
  \index{Kompatibilität!Pakete}%
...
\itempkg{filecontents}
  \index{Minimalbeispiel|?}%
  Dieses Paket erweitert die im \Lettering{LaTeX}"~Kernel definierte Umgebung 
  \Environment(\Package{filecontents}){filecontents} dahingehend, dass bereits 
...
\itempkg{pagecolor}
  ...
  werden.
\itempkg{afterpage}
  Der Befehl \Macro(\Package{afterpage}){afterpage|\MPValue{\dots}} kann 
...
\itempkg{selinput,inputenc}
  ...
  festgelegt, was von (fast) allen \Lettering{LaTeX}"~Editoren unterstützt wird. 
  ...
  Paket \Package{selinput} (automatisch) für \Lettering{LaTeX} festgelegt werden:
  \begin{Code}
    \usepackage{selinput}
...
\itempkg{mparhack}
  ...
  \Macro(\Bundle{koma-script}){marginpar} bereitgestellt. Alternativ dazu 
  lässt sich auch \Package{marginnote} verwenden.
\itempkg{morewrites,scrwfile}
  \ToDo{Links beachten!}[v2.06]
  Falls der Fehler 
  \begin{Code}
  ...
  \Package(\Bundle{koma-script}){scrwfile} nutzen. Mehr dazu in 
