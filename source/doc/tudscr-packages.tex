...
\itempkg{microtype}
  ...
  im Dokument. Es funktioniert mit sowohl \Format*{pdfTeX} als auch 
  \Format*{LuaTeX} sowie \Format*{XeTeX} jedoch nicht mit der klassischen 
  \Format*{TeX}"~Engine.
...
\item[%
  \Application'CTAN:delig'{DeLig}, \InlineDeclaration{\Package{selnolig}}%
  ]\index{Ligaturen}% 
...
\itempkg{mathtools,amsmath}
  Dieses Paket erweitert das De"~facto"~Standard"~Paket \Package{amsmath} und 
...
\itempkg{widetable}
  ...
  Umgebung \Environment(\Package{widetable}){widetable} zur Verfügung, die als 
...
\itempkg{filecontents}
  \index{Minimalbeispiel|?}%
  Dieses Paket erweitert die im \Lettering{LaTeXe}"~Kernel definierte Umgebung 
