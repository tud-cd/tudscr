...
Pakete selber nutzen, können Sie dieses auch wie gewohnt mit
\Macro{usepackage}[\OPName{Paketoptionen}\MPName{Paket}] einbinden. 
Alternativ ist es auch möglich, einfach die gewünschten Optionen bereits 
\emph{vor} dem Laden der Dokumentklasse an das entsprechende Paket 
weiterzureichen, falls bei der folgenden Beschreibung des Paketes nichts 
anderweitiges angegeben wird. Dies wird in \autoref{sec:tips:options} genauer 
beschrieben.
%
\ToDo{koma-script kein Paket, Links beachten! warum fehlt Randnotiz?}[v2.06]
\begin{packages}
\item[%
...
dies in keinem Fall. 
\newcommand*\RecPackage[1]{%
  {"|Empfehlenswertes Paket #1}@%
  {\hyperref[sec:packages:recommended]{Empfehlenswertes Paket}: \Package{#1}}%
}
...
\item[scrlfile](\Bundle{koma-script})<koma-script>
  ...
  \ChangedAt{v2.05:\RecPackage{scrlfile}(\Bundle{koma-script})}
