...
\item[pstricks]
  ...
  \item[auto-pst-pdf]
    ...
    \hologo{MiKTeX} mit \Path{-{}-enable-write18} notwendig. Das Paket  
    \Package{pdftricks2} ist eine Alternative dazu, verzichtet jedoch auf die 
    Verwendung von \Package{pst-pdf}.
  \end{packages}
\end{packages}
%
...
\item[standalone]
  ...
  Dieses Paket ist dafür gedacht, komplett eigenständige Dokumente zu erzeugen, 
  welche später in ein Hauptdokument eingebunden werden können. Optional lassen 
  sich Pakete aus der Präambel der Unterdokumente automatisch in das 
  Hauptdokument 
  einbinden.
\item[subfiles]
\ChangedAt{v2.02!\Package{subfiles}: Beschreibung hinzugefügt}
  Dieses Paket wählt einen etwas anderen Ansatz. Es ist von Anfang an dafür 
  gedacht, ein Hauptdokument zu verwenden, sämtliche Unterdateien nutzen dessen 
  Präambel.
\end{packages}
