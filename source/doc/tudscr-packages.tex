...
dies in keinem Fall. 
\NewExpandableDocumentCommand\RecPackage{d() m}{%
  {"|Empfehlenswertes Paket #2}@{%
    \hyperref[sec:packages:recommended]{Empfehlenswertes Paket}: 
    \Package{#2}\IfValueT{#1}{(#1)}%
  }%
}
...
\item[fontenc,fontspec]
  ...
  \ChangedAt{v2.02:\RecPackage{fontspec}}%
...
\item[csquotes]
  ...
  geladen werden.
\item[noindentafter]
  \ChangedAt{v2.02:\RecPackage{noindentafter}}%
...
\item[fnpct]
  \index{Fußnoten|?}%
  \ChangedAt{v2.05:\RecPackage{fnpct}}%
...
\stditem[%
  \Application{DeLig}'CTAN:delig', \InlineDeclaration{\Package{selnolig}}%
  ]\index{Ligaturen}%
...
\item[longtable,xltabular,ltxtable,ltablex]
  \ChangedAt{v2.07:\RecPackage{xltabular}}%
...
\item[tabu]
  \ChangedAt{%
    v2.02:\RecPackage{tabu} ist nur bedingt empfehlenswert;
    v2.07:\RecPackage{tabu} in der aktuellen Version~v2.9 nicht zu empfehlen;
  }%
...
\item[subfiles]
\ChangedAt{v2.02:\RecPackage{subfiles}}%
...
\item[xkeyval,keyval,kvoptions,pgfkeys]
  ...
  \index{Parameter}%
  \index{Befehlsdeklaration}%
  \ChangedAt{v2.02:\RecPackage{xkeyval}}%
...
\item[scrlfile](\Bundle{koma-script})<koma-script>
  \index{Pakete!Abhängigkeiten}%
  \ChangedAt{v2.05:\RecPackage(\Bundle{koma-script}){scrlfile}}%
...
\item[mwe,blindtext]
  ...
  \ChangedAt{v2.02:\RecPackage{mwe}}%
...
\item[listings]
  ...
  erstellen.
  \ChangedAt{v2.02:\RecPackage{listings}}%
...
\begin{packages}
\item[scrhack](\Bundle{koma-script})<koma-script>
...
\stditem[%
  ...
  \InlineDeclaration{\Package{scrwfile}(\Bundle{koma-script})<koma-script>}%
]
