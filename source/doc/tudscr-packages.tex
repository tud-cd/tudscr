...
\item[koma-script,typearea,scrlayer-scrpage,scrbase,scrwfile]
  ...
  \ChangedAt{%
    v2.02!Paket \Package{scrlayer-scrpage} für Verwendung von \TUDScript 
    notwendig%
  }%
...
\item[quoting]\index{Zitate}
  ...
  beispielsweise die Einstellungen der \KOMAScript-Option \Option{parskip}. 
  Dies wird durch die Umgebung \Environment{quoting} verbessert. Wird das Paket 
  geladen, kommt diese innerhalb der \Environment{abstract}"=Umgebung zum 
  Einsatz.
...
\item[noindentafter]
  \ChangedAt{v2.02!Paket \Package{noindentafter}: Beschreibung hinzugefügt}
...
\item[fontspec]\index{Zeichensatzkodierung}
  \ChangedAt{v2.02!Paket \Package{fontspec}: Beschreibung hinzugefügt}%
  ...
  womit sich für ein \hologo{LaTeX}"=Dokument die Auswahl der verwendbaren 
  Schriften stark erweitert. Das Paket wird durch \TUDScript unterstützt.
...
\item[tabu]
  \ChangedAt{v2.02!Paket \Package{tabu}: Verwendung nur bedingt empfehlenswert}
...
\item[standalone]
  \ChangedAt{%
    v2.02!Paket \Package{standalone}: Probleme mit der dazugehörigen Klasse 
    \Class{standalone} behoben%
  }
...
\item[subfiles]
\ChangedAt{v2.02!Paket \Package{subfiles}: Beschreibung hinzugefügt}
...
\item[listings]\index{Quelltextdokumentation}%
  ...
  erstellen.
  \ChangedAt{v2.02!Paket \Package{listings}: Beschreibung hinzugefügt}
...
\item[xkeyval,keyval,kvoptions,pgfkeys]\index{Befehlsdeklaration}
  \ChangedAt{v2.02!Paket \Package{xkeyval}: Beschreibung hinzugefügt}
...
\item[mwe]\index{Minimalbeispiel|?}
  \ChangedAt{v2.02!Paket \Package{mwe}: Beschreibung hinzugefügt}
