...
%  \Package'CTAN:amsfonts'{amssymb} zu verzichten und anstelle dessen 
%  \Package{mdsymbol} zu laden.
...
\itempkg{setspace}
  ...
  \Macro(\Package{setspace}){setstretch|\MPName{Faktor}} zur Verfügung, 
  mit dem der Durchschuss respektive Zeilenabstand angepasst werden kann. Der 
  Wert des Faktors ist standardmäßig auf~1 gestellt und sollte maximal bis~1.25 
  ...
  \Macro(\Package{setspace}){setstretch|\MPValue{\baselinestretch}} für 
  den Zeilenabstand gewählt. Weitere Tipps sind in \autoref{sec:tips:headings} 
  und \autoref{sec:tips:headline} zu finden.
\itempkg{multicol}
...
\itempkg{isodate,datetime2}
  \index{Datum|?}%
  Mit \Macro(\Package{isodate}){printdate|\MPName{Datum}} formatiert 
  ...
  genutzt werden, welches für \Macro{DTMDate|\MPName{ISO-Datum}} die Angabe im 
...
\itempkg{quoting}
  ...
  \Environment(\Bundle{koma-script}){quotation}~-- für Zitate und 
  ähnliches an. Allerdings werden durch beide Umgebungen die \KOMAScript-Option 
  \Option(\Bundle{koma-script}){parskip=\PMisc} ignoriert. 
  Mit der Umgebung \Environment(\Package{quoting}){quoting} 
...
\itempkg{csquotes}
  ...
  \Macro(\Package{csquotes}){enquote|\MPName{Zitat}}
...
\itempkg{ellipsis}
  ...
  Zusätzliche Hinweise zur Verwendung sind in \autoref{sec:tips:dots} zu finden.
\itempkg{selnolig}(\Application{DeLig})
  ...
  Die Java-Anwendung \Application{DeLig} trennt anhand eines Wörterbuches 
...
\itempkg{caption}
  Mit der Option \InlineDeclaration{\Option(\Bundle{koma-script}){captions=\PMisc}}
  bieten die \KOMAScript-Klassen bereits einige Möglichkeiten zum Formatieren 
  der Beschriftungen für Gleitobjekte. Sollten diese nicht ausreichen, lässt sich 
...
\itempkg{floatrow}
  ...
  \Macro(\Package{floatrow}){floatsetup|\MPValue{\dots}} vorgenommen 
  ...
  Tabellenüberschriften zusätzlich die \KOMAScript-Option 
  \Option(\Bundle{koma-script}){captions=tableheading}
  genutzt werden.
\itempkg{placeins}
...
\itempkg{widetable}
  ...
  Umgebung \Environment(\Package{widetable}){widetable} zur Verfügung, 
...
\itempkg{tabularx}
  ...
  Dafür wird der Spaltentyp \PValue{X} definiert, welcher für die Umgebung 
  \InlineDeclaration{\Environment(\Package{tabularx}){tabularx}}
  beliebig häufig als Argument
  (\Macro*{begin|\MPValue{tabularx}\MPName{Breite}\MPName{Spalten}})
  angegeben werden kann.
  Die \PValue{X}"~Spalten ähneln denen vom Typ~\PValue{p}\MPName{Breite}, 
  wobei die Breite dieser aus der gewünschten Tabellengesamtbreite abzüglich 
  des benötigten Platzes der gegebenenfalls vorhandenen Standardspalten 
  \PValue{lcr} etc. automatisch berechnet wird.
\itempkg{tabulary}
  ...
  \Package{tabularx} auf alle Spalten gleichmäßig verteilt sondern in der 
  Umgebung 
  \InlineDeclaration{\Environment(\Package{tabulary}){tabulary}}
  für die Spaltentypen~\PValue{LCRJ} im Argument
  (\Macro*{begin|\MPValue{tabulary}\MPName{Breite}\MPName{Spalten}})
  anhand ihres Zellinhaltes gewichtet vergeben.
\itempkg{longtable,xltabular,ltxtable}[v2.06:\Package{xltabular}]
  ...
  \Environment(\Package{tabularx}){tabularx}-Tabelle
  ist das Paket \Package{xltabular} empfehlenswert. Alternativ dazu lässt sich 
  auch \Package{ltxtable} nutzen.
\itempkg{multirow}
  Es wird der Befehl \Macro(\Package{multirow}){multirow} definiert, der 
...
\itempkg{tabu}
  [
    ...
    v2.06:\Package{tabu} in der aktuellen Version~v2.9 nicht zu empfehlen;
  ]
  ...
  definiert. Es lässt sich alternativ zum Paket \Package{tabularx} verwenden und 
  ist insbesondere anstelle des Paketes \Package{ltxtable} eine Möglichkeit zur 
...
Text an \Lettering{LaTeX} unterstützen, werden genutzt. Für das Programmieren 
...
\itempkg{cleveref}
  ...
  Objekte mit dem Befehl \Macro(\Package{hyperref}){autoref} aus dem 
...
\Lettering{TikZ}- oder \Package{pstricks}"~Grafik~-- zu kompilieren. Die 
meiner Meinung nach besten Pakete für dieses Unterfangen werden folgend 
vorgestellt.
...
\itempkg{subfiles}[v2.02]
  ...
  Die darin mit \Macro(\Package{subfiles}){subfiles} eingebundenen 
...
leere Textdatei erzeugen und diese unter \File*{mypreamble.sty}~-- oder einem 
beliebigen anderen Namen mit der Dateiendung \File*{.sty}~-- im gleichen Ordner 
...
\itempkg{marginnote}
  ...
  \Macro(\Package{marginnote}){marginnote} für nicht"~gleitende 
...
\itempkg{xkeyval,keyval,kvoptions,pgfkeys}[v2.02]
  ...
  \Macro*{newcommand|\Macro*{Befehl|\OPName{Schlüssel-Wert-Liste}\MPName{Argument}}}
  ...
  unterstützen, lässt sich das Paket \Package{scrbase} nutzen. Soll aus einem 
...
\itempkg{filecontents}
  ...
  \Environment(\Package{filecontents}){filecontents} dahingehend, 
...
\itempkg{afterpage}
  Der Befehl \Macro(\Package{afterpage}){afterpage|\MPValue{\dots}} 
...
\itempkg{mparhack}
  ...
  \Macro(\Bundle{koma-script}){marginpar} bereitgestellt. Alternativ 
...
\itempkg{fix-cm}
  ...
  \begin{Code}
    Font shape `T1/cmr/m/n' in size <§\dots§> not available
  \end{Code}\vspace{-\baselineskip}%
