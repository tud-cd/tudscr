...
\begin{Code}
\PassOptionsToPackage§\MPName{Paketoptionen}\MPName{Paket}§
\documentclass§\OPName{Klassenoptionen}\MPValue{tudscr\dots}§
\end{Code}
\end{Example}
%
...
\itempkg{graphicx}
  ...
  wird \Macro'none'(\Package{graphicx}){includegraphics} genutzt.
\itempkg{xcolor}
  \index{Farben}%
...
\itempkg{etoolbox,xpatch,letltxmacro,etexcmds}
...
\itempkg{hyphsubst,dehyph-exptl}
  ...
  \Format{XeLaTeX} nutzen diese Trennmuster automatisch, für \Format{pdfLaTeX} 
...
\itempkg{setspace}
  ...
  \Macro'none'(\Package{setspace}){setstretch|\MPName{Faktor}} zur 
  ...
  \Macro'none'(\Package{setspace}){setstretch|\MPValue{1.1}} für den 
...
\itempkg{multicol}
  ...
  \KOMAScript-Option \Option'none'(\Package{typearea}){twocolumn}~-- für einen 
...
\itempkg{isodate,datetime2}
  \index{Datum|?}%
  Mit \Macro'none'(\Package{isodate}){printdate|\MPName{Datum}} formatiert 
...
\itempkg{quoting}
  ...
  \Environment'none'(\Bundle{koma-script}){quote} und 
  \Environment'none'(\Bundle{koma-script}){quotation}~-- für Zitate und 
  ähnliches an. Allerdings werden durch beide Umgebungen die \KOMAScript-Option 
  \Option|declare|'none'(\Bundle{koma-script}){parskip=\PName{Methode}}
  ignoriert. Mit der Umgebung \Environment'none'(\Package{quoting}){quoting}
...
\itempkg{csquotes}
  ...
  \Macro'none'(\Package{csquotes}){enquote|\MPName{Zitat}}
...
\itempkg{xspace,xpunctuate}
  ...
  \Macro'none'(\Package{xspace}){xspace} genutzt werden. Dieser setzt ein 
...
\itempkg{ellipsis}
  ...
  \seeplain*{\autoref{sec:tips:dots}}.
...
\itempkg{enumitem}
  ...
  \Environment'none'(\Bundle{koma-script},\Package{enumitem}){itemize},
  \Environment'none'(\Bundle{koma-script},\Package{enumitem}){enumerate} sowie
  \Environment'none'(\Bundle{koma-script},\Package{enumitem}){description} und
  ...
  \Macro'none'(\Package{enumitem}){setlist|\MPValue{noitemsep}}.
\end{DeclarePackages}
...
\itempkg{caption}
  Mit der Option \Option|declare|'none'(\Bundle{koma-script}){captions=\PMisc} 
...
\itempkg{floatrow}
  ...
  \Macro'none'(\Package{floatrow}){floatsetup|\MPValue{\dots}} vorgenommen 
  ...
  \Macro'none'(\Bundle{koma-script},\Package{caption}){caption} 
  ...
  \KOMAScript-Option \Option'none'(\Bundle{koma-script}){captions=tableheading}
  genutzt werden.
\itempkg{placeins}
...
\itempkg{array}
  Dieses Paket ermöglicht mit \Macro'none'(\Package{array}){newcolumntype} das 
  ...
  sich mit \Macro'none'(\Package{array}){extrarowheight} ändern. 
\itempkg{booktabs}
  ...
  Befehle \Macro'none'(\Package{booktabs}){toprule}, 
  \Macro'none'(\Package{booktabs}){midrule} sowie 
  \Macro'none'(\Package{booktabs}){cmidrule} und 
  \Macro'none'(\Package{booktabs}){bottomrule}
  für unterschiedliche horizontale Linien bereit.
\itempkg{widetable}
  Mit der \Lettering{LaTeX}-Umgebung \Environment{tabular*} kann eine 
  Tabelle mit einer definierten Breite gesetzt werden. Dieses Paket stellt die 
  Umgebung \Environment'none'(\Package{widetable}){widetable} zur Verfügung, 
...
\itempkg{tabularx}
  ...
  der \Environment'none'(\Package{tabularx}){tabularx}-Umgebung beliebig 
...
\itempkg{tabulary}
  ...
  \Environment'none'(\Package{tabularx}){tabularx} auf alle Spalten 
  gleichmäßig verteilt sondern in der 
  \Environment'none'(\Package{tabulary}){tabulary}-Umgebung für die 
...
\itempkg{longtable,ltxtable,ltablex}
  ...
  mit einer \Environment'none'(\Package{tabularx}){tabularx}-Umgebung können 
...
\itempkg{multirow}
  Es wird der Befehl \Macro'none'(\Package{multirow}){multirow} definiert, der 
...
\itempkg{tabu}[v2.02:\Package{tabu} ist nur bedingt empfehlenswert]
  ...
  die Umgebungen \Environment'none'(\Package{tabu}){tabu} und 
  \Environment'none'(\Package{tabu}){longtabu} definiert. Es 
  ...
  Durch Änderungen am \Lettering{LaTeXe}"~Kernel wurde das Paket unbrauchbar, 
  weshalb das \GitHubRepo<tabu-fixed/tabu>[\Lettering{LaTeX3}"~Projektteam] 
...
Text an \Lettering{LaTeX} unterstützen, werden genutzt. 
...
\itempkg{pstricks}
  ...
  \Environment'none'(\Package{pstricks}){pspicture} zunächst über den Pfad 
  ...
  \itempkg{auto-pst-pdf,pdftricks2}
    ...
    \Path{pdflatex} mit \Path{-{}-shell-escape} notwendig. Bitte beachten Sie 
    dazu ebenfalls die Hinweise in \autoref{sec:tips:auto-pst-pdf}. Alternativ
    dazu lässt sich das Paket \Package{pdftricks2} nutzen.
  \end{DeclarePackages}
\end{DeclarePackages}
...
\Macro{label|\MPName{Label}} ein \emph{eindeutiges} Label erzeugt werden, 
auf welches im Dokument entweder mit \Macro{ref} oder nach dem Laden von 
\Package{hyperref} besser noch mit \Macro'none'(\Package{hyperref}){autoref} 
...
\itempkg{cleveref}
  ...
  Objekte mit dem Befehl \Macro'none'(\Package{hyperref}){autoref} aus dem 
...
\itempkg{subfiles}[v2.02]
  ...
  Die darin mit \Macro'none'(\Package{subfiles}){subfiles} eingebundenen 
...
\Macro*{usepackage|\MPValue{mypreamble}} in das Dokument einbinden. Dies 
...
\itempkg{marginnote}
  \index{Randnotizen}%
  Mit dem Befehl \Macro'none'(\Bundle{koma-script}){marginpar} lassen sich 
  ...
  \Macro'none'(\Package{marginnote}){marginnote} für nicht"~gleitende 
...
\itempkg{filecontents}
  ...
  \Environment'none'(\Package{filecontents}){filecontents} dahingehend,
...
\itempkg{pagecolor}
  ...
  werden.
\itempkg{afterpage}
  Der Befehl \Macro'none'(\Package{afterpage}){afterpage|\MPValue{\dots}} 
...
\itempkg{selinput,inputenc}
  ...
  \Macro*{usepackage|\OPName{Eingabekodierung}\MPValue{inputenc}} 
...
\itempkg{mparhack}
  ...
  \Macro'none'(\Bundle{koma-script}){marginpar} bereitgestellt. Alternativ 
