...
\item[textcase]
  ...
  Mit \Macro*{MakeTextUppercase}(\Package{textcase})'none' wird die 
  Großschreibung der Überschriften in \DIN erzwungen. Im \emph{Ausnahmefall} 
...
\item[quoting]
  ...
  \Environment{quoting}(\Package{quoting})'none' behoben. Wird das Paket 
...
\item[selinput,inputenc]
  ...
  sich mit dem Paket \Package{inputenc} die Eingabekodierung manuell einstellen
...
\item[setspace]
  ...
  \Macro{setstretch}[\PParameter{1.1}](\Package{setspace})'none' gewählt. Nach 
  einer Einstellung des Zeilenabstandes sollte der Satzspiegel unbedingt mit 
  \Macro{recalctypearea} neu berechnet werden. Weitere Tipps sind in 
...
\stditem[\Application{DeLig}<delig>]
  \index{Ligaturen}%
...
\item[floatrow]
  ...
  der Umgebungen \Environment{float} und \Environment{table} eingestellt werden 
  (\Macro*{floatsetup}[\PParameter{font=\dots}]). Das typografisch richtige 
  Setzen von Abbildungs\textbf{unterschriften} 
...
\item[marginnote]
  ...
  stellt den Befehl \Macro{marginnote}(\Package{marginnote})'none' für 
  nicht"~gleitende Randnotizen zur Verfügung. Alternativ dazu lässt sich auch 
  \Package{mparhack} verwenden.
\item[todonotes]
  \index{Randnotizen}%
