...
\itempkg{setspace}
  ...
  \Macro{setstretch}[\MPName{Faktor}](\Package{setspace})'none' zur 
  ...
  \Macro{setstretch}[\MPValue{1.1}](\Package{setspace})'none' für den 
...
\itempkg{isodate,datetime2}
  \index{Datum|?}%
  Mit \Macro{printdate}[\MPName{Datum}](\Package{isodate})'none' formatiert 
...
\itempkg{quoting}
  ...
  \Environment{quote}(\Bundle{koma-script})'none' und 
  \Environment{quotation}(\Bundle{koma-script})'none'~-- für Zitate und 
  ähnliches an. Allerdings werden durch beide Umgebungen die \KOMAScript-Option 
  \Option{parskip=\PName{Methode}}(\Bundle{koma-script})'none'|declare|
...
\itempkg{csquotes}
  ...
  \Macro{enquote}[\MPName{Zitat}](\Package{csquotes})'none'
...
\itempkg{enumitem}
  ...
  \Environment{itemize}(\Bundle{koma-script},\Package{enumitem})'none',
  \Environment{enumerate}(\Bundle{koma-script},\Package{enumitem})'none' sowie
  \Environment{description}(\Bundle{koma-script},\Package{enumitem})'none' und
  ...
  \Macro{setlist}[\MPValue{noitemsep}](\Package{enumitem})'none'.
\end{DeclarePackages}
...
\itempkg{caption}
  Mit der Option \Option{captions=\PMisc}(\Bundle{koma-script})'none'|declare| 
...
\itempkg{floatrow}
  ...
  \Macro{floatsetup}[\MPValue{\dots}](\Package{floatrow})'none' vorgenommen 
  ...
  \Macro{caption}(\Bundle{koma-script},\Package{caption})'none' 
  ...
  \KOMAScript-Option \Option{captions=tableheading}(\Bundle{koma-script})'none'
  genutzt werden.
\itempkg{placeins}
...
\itempkg{marginnote}
  \index{Randnotizen}%
  Mit dem Befehl \Macro{marginpar}(\Bundle{koma-script})'none' lassen sich 
...
\itempkg{xkeyval,keyval,kvoptions,pgfkeys}[v2.02]
  ...
  \Macro*{newcommand}[%
    \Macro*{Befehl}[\OPName{Schlüssel"=Wert"=Liste}\MPName{Argument}]%
  ]
...
\itempkg{pagecolor}
  ...
  werden.
\itempkg{afterpage}
  Der Befehl \Macro{afterpage}[\MPValue{\dots}](\Package{afterpage})'none' 
...
\itempkg{selinput,inputenc}
  ...
  nicht geändert werden, so ist es für \Format{pdfLaTeX} möglich, eine 
...
\itempkg{mparhack}
  ...
  \Macro{marginpar}(\Bundle{koma-script})'none' bereitgestellt. Alternativ 
