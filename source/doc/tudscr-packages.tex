...
\begin{DeclarePackages}
\itempkg{fontenc,fontspec}[%
    v2.02:\RecPackage{fontspec}
  ]
...
\itempkg{multicol}
  ...
  \KOMAScript-Option \Option{twocolumn}~-- für einen Spaltenausgleich gesorgt 
  wird. Unterstützt wird das Paket innerhalb der Umgebungen 
  \Environment{abstract} und \Environment{tudpage}.
\itempkg{balance}
  \index{Satzspiegel!zweispaltig}%
...
\itempkg{noindentafter}[%
    v2.02:\RecPackage{noindentafter}%
  ]
...
\ToDo[doc]{Pakete float und keyfloat erläutern}[v2.07]
\ToDo[doc]{widows-and-orphans, lua-widow-control}[v2.07]
%\documentclass[%
%  captions=tableabove
%]
%{scrreprt}
...
\itempkg{tabu}[%
    v2.02:\RecPackage{tabu} nur bedingt empfehlenswert;
    v2.07:\RecPackage{tabu} in Version~v2.10 nur sehr eingeschränkt nutzbar;
  ]%
...
\itempkg{xkeyval,keyval,kvoptions,pgfkeys}[%
    v2.02:\RecPackage{xkeyval}%
  ]
  \index{Befehlsdeklaration}%
  ...
  \Macro||{newcommand|%
    \MPValue{\Macro*{Befehl}}\OPName{Schlüssel"=Wert"=Liste}\MPName{Argument}%
  }%
...
\itempkg{mwe,blindtext}[%
    v2.02:\RecPackage{mwe}%
  ]
