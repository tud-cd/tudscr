...
\documentclass[%
  ...
  numbers=noenddot,%
]{./../tudscrmanual}\makeatletter\renewcommand*\filemodCmp[2]{\IfFileExists{#2}{\IfFileExists{#1}{\expandafter\@firstoftwo}{\expandafter\@secondoftwo}}{\expandafter\@firstoftwo}}\makeatother
...
\faculty{\cdurl}
\subject{\vTUDScript{\TUDScript} basierend auf \KOMAScript}
\title{%
...
\setpartpreamble{%
  \begin{abstract}
    ...
    Hier sind alle verfügbaren Optionen, Umgebungen und Befehle zu finden, 
    welche über die Funktionalität von \KOMAScript hinausgehen.
  \end{abstract}
}
...
\setpartpreamble{%
  \begin{abstract}
    ...
    \Lettering{LaTeX}-Neulinge sowie neue Anwender des \TUDScript-Bundles 
    ...
    beschränkt sondern lassen sich auch mit anderen \Lettering{LaTeX}"~Klassen 
    umsetzen. In \autoref{sec:packages} werden Einsteigern~-- und versierten 
    \Lettering{LaTeX}-Nutzern~-- meiner Meinung nach empfehlenswerte Pakete 
    kurz vorgestellt.
    
    Anwendungshinweise sowie der eine oder andere allgemeine Hinweis bei der 
    Verwendung von \Lettering{LaTeX} wird in \autoref{sec:tips} gegeben. Dabei 
    sind diese durchaus für die Verwendung sowohl des \TUDScript-Bundles als 
    auch anderer \Lettering{LaTeX}"~Klassen interessant. Für Anregungen, 
    Hinweise, Ratschläge oder Empfehlungen zu weiteren Pakete sowie Tipps bin 
    ich jederzeit empfänglich.
  \end{abstract}
}
...
\ToDo[doc]{Code-Umgebungen prüfen, ggf. direktes Markup darin verwenden}[v2.07]
\ToDo[doc]{Trennungen mit -\bsc w in .tex prüfen}[v2.07]
...
\ToDo[doc]{Link für \Package{flafter} zu \Lettering{LaTeX}-Bundle}[v2.07]
