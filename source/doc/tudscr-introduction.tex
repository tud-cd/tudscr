...
von \Distribution{\hologo{MiKTeX}}|?| durchgeführt werden. Andernfalls wird es 
mit großer Sicherheit zu Problemen kommen. Außerdem ist es sehr ratsam, die 
Schriften des \CDs~-- wie generell sämtliche, manuell installierte Pakete und 
Schriftarten auch~-- nicht in das Verzeichnis des Standardbenutzers oder gar in 
den Windows"=Programmpfad von \Distribution{\hologo{MiKTeX}}|?| selbst sondern 
in einen separaten Ordner zu installieren und diesen bei 
\Distribution{\hologo{MiKTeX}}|?| als lokalen \Path{texmf}-Baum anzumelden. Das 
Installationsskript führt besagten Schritt automatisiert aus. Am besten ist es 
bei der Installation, den darin voreingestellten Installationspfad zu nutzen. 
...
\Path{updmap -{}-syncwithtrees} und \Path{updmap -{}-force} ausführen.
...
\Attention{%
  ...
  beziehungsweise \Distribution{Mac\hologo{TeX}}|?| über die Pakete 
  \Package*{fontware}() sowie \Package*{collection-fontutils}() 
...
hierfür in \label{sec:local:install} entsprechende Skripte bereitgestellt. Soll 
eine bereits vorhandene, lokale Nutzerinstallation aktualisiert werden, finden 
Sie die entsprechenden Skripte in \label{sec:local:update}.
...
%
\ChangedAt{%
  v2.04!Installationsskripte für \Distribution*{\hologo{TeX}~Live~Portable} und 
