...
explizit angegeben wird. 
\Attention{%
  Dieses \textbf{muss} mit \Path{bash \PName{Skript}.sh} im Terminal 
  aufgerufen werden. Hierbei sollte \Path{sudo} \textbf{nicht} verwendet 
  werden, da ansonsten die Registrierung der Schriften nicht für den aktuellen 
  Benutzer sondern für \Path{root} aufgerufen wird.
}
