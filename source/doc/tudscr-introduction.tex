\chapter{Einleitung}
%
\ToDo[doc]{Name des Paketes für \OpenSans ergänzen}[v2.06]
...
Hausschrift des \CDs \OpenSans zwingend benötigt. Außerdem müssen weitere 
Pakete durch die genutzte \hologo{LaTeX}"=Distribution bereitgestellt werden, 
welche unter \autoref{sec:packages:needed} aufgeführt sind. Beim Einsatz einer 
der Distributionen
\index{Distribution}%
...
\Class{beamer}"=Klasse allerdings bereits mehrere Stile. Dieses ist sowohl bei 
\hrfn{https://github.com/tud-cd/tud-cd}{GitHub} als auf der 
\hrfn{https://tu-dresden.de/cd/4_latex}{\hologo{LaTeX}-Seite der \TnUD} zu 
finden. Für das Erstellen von Briefen zusammen mit den \TUDScript"=Klassen 
ließe sich auch das Paket \Package{scrletter} nutzen.
...
beschrieben sind. Zur Verwendung des \TUDScript-Bundles ist~-- neben 
...
Version~v2.06 lediglich die Schrift \OpenSans, welche durch das Paket 
\ToDo[doc]{Name des Paketes für \OpenSans ergänzen/korrigieren}[v2.06]
\Package{opensans} zur Verfügung gestellt werden. Eine manuelle Installation 
der Schriften wie in vorherigen Versionen ist nicht notwendig. Lediglich für 
den Fall, dass gezielt die alten Schriften \Univers und \DIN eingesetzt werden 
sollen, müssen diese auch installiert sein. Weitere Hinweise dazu sind in 
\autoref{sec:install:fonts} zu finden.
...
werden zusätzlich zu \hologo{LaTeXe} ein \textsc{Perl}"=Interpreter sowie 
