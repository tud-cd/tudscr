...
\Macro*{up\Parameter{Letter}} oder \Macro*{it\Parameter{Letter}} auch der 
Präfix \Macro*{other\Parameter{Letter}} genutzt werden. Damit wird~-- abhängig 
von der Option \Option{slantedgreek}~-- die komplementäre Schriftlage der 
angegebenen Letter gesetzt.
\index{Griechische Lettern|)}%
\end{Declaration}
\end{Declaration}
\end{Declaration}
...
\footnote{%
  \Macro{extratitle}[\Parameter{Schmutztitel}](\Bundle{koma-script}),
  \Macro{frontispiece}[\Parameter{Frontispiz}](\Bundle{koma-script}) für eine 
  Seite vor dem eigentlichen Haupttitel sowie
  \Macro{titlehead}[\Parameter{Kopf}](\Bundle{koma-script}),
  ...
  \Macro{and}, \Macro{thanks}[\Parameter{Fußnote}](\Bundle{koma-script}), 
  \Macro{publishers}[\Parameter{Verlag}](\Bundle{koma-script}) und 
  \Macro{date}[\Parameter{Datum}] für den Titel selbst sowie 
  \Macro{uppertitleback}[\Parameter{Titelrückseitenkopf}](\Bundle{koma-script})
  und
  \Macro{lowertitleback}[\Parameter{Titelrückseitenfuß}](\Bundle{koma-script}) 
  für die Rückseite des Titels sowie abschließend 
  \Macro{dedication}[\Parameter{Widmung}](\Bundle{koma-script}) für eine 
  Danksagung.
}
...
lediglich mit \Macro{pagestyle}[\Parameter{Seitenstil}](\Bundle{koma-script}) 
...
Sobald mit \Macro{pagestyle}[\Parameter{Seitentil}](\Bundle{koma-script}) 
...
\Macro*{pagestyle}[\PParameter{empty.tudheadings}] manuell aufgerufen werden. 
Um auf das normale Verhalten von \KOMAScript{} zurückzuschalten, muss mit 
\Macro*{pagestyle}[\Parameter{Seitentil}] einer der beiden Seitenstile 
...
auch zuvor durchgeführte Anpassungen der Schrift unverändert bleiben.
\end{Declaration}
\end{Declaration}
...
(\Macro{frontispiece}[\Parameter{Frontispiz}](\Bundle{koma-script})), der 
...
\Macro*{renewcommand*}[%
  \PParameter{\Macro{partpagestyle}(\Bundle{koma-script})}%
...
Bezeichnung des mit 
\Macro{part}[\Parameter{Bezeichnung}](\Bundle{koma-script}) erzeugten Teils 
...
Mit \Macro*{renewcommand*}[%
  \PParameter{\Macro{chapterpagestyle}(\Bundle{koma-script})}%
  \Parameter{Seitenstil}%
]
...
gleicher Weise wie \Macro{setchapterpreamble}(\Bundle{koma-script}) 
\emph{vor} der Verwendung von \Macro{chapter}(\Bundle{koma-script}) oder 
...
](\Bundle{koma-script}) angepassen. Mehr dazu ist im \scrguide innerhalb des 
...
\begin{Code}[escapechar=§]
\AtBeginDocument{\addtokomafont§\Parameter{Schriftelement}\Parameter{Einstellungen}§}
\end{Code}
\end{quoting}
...
der Verwendung von \Key{\Environment{abstract}}{markboth=\PParameter{}} auch 
...
\Macro{thesis}, \Macro{subject}[\Parameter{\autoref{tab:thesis}}] 
beziehungsweise mit der Option \Option{subjectthesis} angegeben wurde~-- 
aufgeführt. Über den Parameter \Key{\Macro{confirmation}}{supporter} oder 
\emph{zuvor} mit dem Befehl \Macro{supporter} können weitere an der Arbeit 
beteiligte Personen angegeben werden. Mehrere zu nennende Personen sind auch 
hier durch \Macro{and} zu trennen. Das Feld der Unterstützer kann auch mit dem 
bloßen optionalen Argument ohne die Angabe eines Parameters angepasst werden. 
\ChangedAt{v2.05}
...
\footnote{%
  \Macro{blocking}[%
    \POParameter{\Key{\Macro{blocking}}{company=\PSet{Firma}}}%
  ]
