...
oder \Package{datetime2} geladen sein, so wird die damit eingestellte 
Formatierung des Datums durch \Macro{dateofbirth}~-- wie übrigens bei jedem 
anderem Datumsfeld der \TUDScript-Klassen auch~-- verwendet. Hierfür wird 
entweder \Macro{printdate}(\Package{isodate})'none' von \Package{isodate} oder 
\Macro{DTMDate}(\Package{datetime2})'none' aus \Package{datetime2} genutzt.
...
\end{values}
\ToDo[doc,imp]{\PValue{nocolor}}[v2.06]
\end{Declaration}

\ToDo[doc]{%
  \Option*{clearcolor} weg, wenn Funktion in \Option*{cleardoublespecialpage}%
}[v2.06]
