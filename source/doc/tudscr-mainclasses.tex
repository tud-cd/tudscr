...
erläutert. Der Titel~-- bestehend aus dem möglichen Schmutztitel 
(\Macro{extratitle}), der eigentlichen Titelseite respektive des Titelkopfes 
und der nachgelagerten Elementen~-- kann mit \Macro{maketitle} ausgegeben 
werden. Außerdem kann im zweispaltigen Satz \Macro{maketitleonecolumn} 
verwendet werden, womit eine einspaltige Ergänzung nach dem Titel selbst 
ermöglicht wird.
...
Beginn von Teilen und Kapiteln in Dokumenten zu finden.\footnote{%
  Voreinstellungen
  \Class{tudscrbook}: 
  \Option{twoside=true}(\Package{typearea})'none', 
  \Option{open=right}(\Package{koma-script})'none';
  \Class{tudscrartcl} und \Class{tudscrreprt}: 
...
\ToDo[doc,imp]{%
  ...
  davor (\Option*{open=right}(\Package{koma-script})) sowie danach 
  (\Option*{open=left}(\Package{koma-script})). Setzen von speziellen Inhalten 
  auf diesen Seiten äquivalent zu \Macro*{setpartpreamble} bzw. 
  \Macro*{setchapterpreamble}; ggf. temporär umschalten bzw. Warnung bei 
  Konflikt.
...
\Environment{quotation}"=Umgebung ausgegeben, wobei die Überschrift 
\emph{nicht} in der Form einer Gliederungsebene gesetzt wird. Die verwendete 
Umgebung hat jedoch den Nachteil, dass in dieser die \KOMAScript-Option 
