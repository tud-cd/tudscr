...
Des Weiteren besteht die Möglichkeit, auf das Standardverhalten von \KOMAScript
...
\begin{Declaration}[v2.05]{\Option{cdsection=\PSet}}<\cdalias>
\printdeclarationlist%
%
...
definiert. Damit verwenden Optionen oder Befehlen, welche normalerweise 
...
\Option{abstract=multiple} werden mehrere Zusammenfassungen auf einer Seite 
ausgeben~-- sofern genügend Platz vorhanden ist. Außerdem lässt sich die 
vertikale Zentrierung der \Environment{abstract}"~Umgebung auf einer Seite 
deaktivieren. Diese Einstellungen zur Positionierung der Zusammenfassungen 
innerhalb der \Environment{abstract}"~Umgebung werden nur wirksam, wenn eine 
Titelseite (\KOMAScript-Option \Option{titlepage=true}(\Package{koma-script})) 
und \emph{keine} Überschriften in Form von Kapiteln (\Option{abstract=chapter}) 
verwendet werden.
%
\begin{values}{\Option{abstract}}
