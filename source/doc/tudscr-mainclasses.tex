...
Säumniswert\footnote{engl.: default value}. Sollte die Angabe eines Wertes 
versäumt werden~-- es wird einfach die Form \PName{Option} verwendet~-- so wird 
...
%
\begin{Declaration}[%
  v2.02!\Option{cdfont=\PValue{head}};
  v2.02!\Option{cdfont=\PValue{heavyhead}};
  v2.03!\Option{cdfont=\PValue{din}};
  v2.03!\Option{cdfont=\PValue{nodin}};
...
erzwungenen Kleinschreibung kann \Macro{MakeTextLowercase} genutzt werden.
\end{Declaration}
\end{Declaration}
%
\begin{Example}
...
\index{Layout!Überschriften|(}%
%
\ToDo[imp]{protect sollte nicht notwendig sein, Werte direkt in Liste}[v2.05]
\begin{Declaration}[%
  v2.03!\Option{cd=\PValue{bicolor}}:%
    Farbeinsatz nur im Kopf mit farbig abgesetztem Querbalken;
  v2.03!\Option{cd=\PValue{fullcolor}}:%
    voller Farbeinsatz mit farbig abgesetztem Querbalken;%
  v2.04!\Option{cd=\PValue{barcolor}}:nur farbig abgesetzter Querbalken;%
]{\Option{cd=\PSet}}[true]
\printdeclarationlist%
%
...
\begin{Declaration}[%
  v2.03!\Option{cdtitle=\PValue{bicolor}}:%
    Farbeinsatz nur im Kopf mit farbig abgesetztem Querbalken;
  v2.03!\Option{cdtitle=\PValue{fullcolor}}:%
    voller Farbeinsatz mit farbig abgesetztem Querbalken;
  v2.04!\Option{cdtitle=\PValue{barcolor}}:nur farbig abgesetzter Querbalken;%
]{\Option{cdtitle=\PSet}}
\printdeclarationlist%
\index{Titel|?}%
%
...
\begin{Declaration}[%
  v2.02;%
  v2.03!\Option{cdcover=\PValue{bicolor}}:%
    Farbeinsatz nur im Kopf mit farbig abgesetztem Querbalken;
  v2.03!\Option{cdcover=\PValue{fullcolor}}:%
    voller Farbeinsatz mit farbig abgesetztem Querbalken;
  v2.04!\Option{cdcover=\PValue{barcolor}}:nur farbig abgesetzter Querbalken;%
]{\Option{cdcover=\PSet}}
\printdeclarationlist%
\index{Umschlagseite|?}%
%
...
\begin{Declaration}[%
  v2.03!\Option{cdpart=\PValue{bicolor}}:%
    Farbeinsatz nur im Kopf mit farbig abgesetztem Querbalken;
  v2.03!\Option{cdpart=\PValue{fullcolor}}:%
    voller Farbeinsatz mit farbig abgesetztem Querbalken;
  v2.04!\Option{cdpart=\PValue{barcolor}}:nur farbig abgesetzter Querbalken;%
]{\Option{cdpart=\PSet}}
\printdeclarationlist%
...
\begin{Declaration}[%
  v2.03!\Option{cdchapter=\PValue{bicolor}}:%
    Farbeinsatz nur im Kopf mit farbig abgesetztem Querbalken;
  v2.03!\Option{cdchapter=\PValue{fullcolor}}:%
    voller Farbeinsatz mit farbig abgesetztem Querbalken;
  v2.04!\Option{cdchapter=\PValue{barcolor}}:nur farbig abgesetzter Querbalken;%
...
\index{Layout!Kapitelseiten|?}%
%
\ToDo[doc]{Hinweis auf \Macro*{setlength}}[v2.05]
...
(\KOMAScript-Option \Option{titlepage=true}) als auch die Überschriften von 
Teilen und separaten Kapitelseiten (\Option{chapterpage=true}) lassen sich mit 
\Length{pageheadingsvskip} vertikal verschieben. Demgegenüber erlaubt es die 
Länge \Length{headingsvskip}, sowohl den Titel eines Titelkopfes 
(\KOMAScript-Option \Option{titlepage=false}) als auch die Kapitelüberschrift 
bei deaktivierter Kapitelseite (\Option{chapterpage=false}) in ihrer 
...
\begin{Declaration}[%
  v2.03;%
  v2.04!\Option{cdhead=\PValue{barcolor}}:nur farbig abgesetzter Querbalken;%
...
\begin{Declaration}[%
  v2.03!\Option{cdfoot=\PValue{color}}:farbiger Hintergrund der Fußzeile;%
...
die Option \Option{manualmark}|declare| nicht explizit angegeben, so werden mit 
\Option{cdfoot=true} über die \KOMAScript-Option \Option{automark}|declare| 
...
\item[color/bicolor/bichrome]
  Die Fußzeile wird farbig abgesetzt, falls entweder der zweifarbige Kopf
...
\ToDo[doc]{Hinweis auf \Option{ddcfoot}}[v2.05]
...
Separator (\Macro{footlogo}[\PParameter{\PName{Dateiname}:\PName{Parameter}}]) 
zu übergeben, wobei diese \emph{nach} den allgemeinen Einstellungen für alle 
Logos angewendet werden. Die möglichen Parameter und Werte für die optionalen 
...
für die farbliche Ausprägung (Option \Option{cd}) automatisch gewählt. Soll 
dies manuell erfolgen, können folgende Werte verwendet werden:
%
\begin{values}{\Option{ddc}}
\item[color]
...
verwendet. Dabei wird durch \Option{ddchead} ein mit \Macro{headlogo} 
angegebenes Zweitlogo definitiv unterdrückt. Die möglichen Werte der beiden 
Optionen entsprechen denen der Option \Option{ddc}.
\end{Declaration}
\end{Declaration}
...
\printdeclarationlist%
%
\ToDo[imp]{\Option{extratopmargin}}[v2.05]
...
\TUDScript-Klassen auch an das Paket \Package{geometry} weitergereicht, so dass 
...
%
\begin{Declaration}[%
  v2.02!\Option{abstract=\PValue{multiple}}:%
    ersetzt \Option{abstract=\PValue{double}};%
  v2.02!\Option{abstract=\PValue{tocleveldown}};%
  v2.02!\Option{abstract=\PValue{markboth}};%
  v2.04!\Option{abstract=\PValue{tocmultiple}}%
...
\KOMAScript-Option \Option{automark} aktiviert wurden und sind von der 
aktuellen Gliederungsebene der Überschrift abhängig. Werden jedoch mit Hilfe 
der \KOMAScript-Option \Option{manualmark} manuelle Kolumnentitel genutzt, 
müssen diese normalerweise auch für die Zusammenfassung durch den Anwender 
gesetzt werden. Mit \Option{abstract=markboth} lässt sich allerdings das 
...
geladen werden, wodurch stattdessen die Umgebung \Environment{quoting} 
verwendet wird.
...
%
\begin{Declaration}[%
  v2.02!\Option{declaration=\PValue{multiple}}:%
    ersetzt \Option{declaration=\PValue{double}};%
  v2.02!\Option{declaration=\PValue{tocleveldown}};%
  v2.02!\Option{declaration=\PValue{markboth}};%
  v2.04!\Option{declaration=\PValue{tocmultiple}}%
...
die \KOMAScript-Option \Option{automark}. Werden manuelle Kolumnentitel 
genutzt, müssen diese auch für die Erklärungen manuell gesetzt werden. Mit 
\Option{declaration=markboth} lässt sich unabhängig davon das Setzen der 
...
innerhalb der \Environment{declarations}"=Umgebung als auch außerhalb direkt im 
Dokument verwendet werden und akzeptiert im optionalen Argument dabei alle für 
die \Environment{declarations}"=Umgebung beschriebenen Parameter.
\end{Declaration}
\end{Declaration}
\end{Declaration}
\end{Declaration}
...
\ToDo{optionales Argument?!}[v2.05]
\footnote{%
  \Macro{confirmation}[%
...
\item[\PValue{2.02}/\PValue{first}]
  ...
  gleiche Satzspiegel genutzt. Mit \Option{tudscrver=2.02} kann dieses 
...
\item[\PValue{2.03}]
  ...
  Schriftgröße definiert. Von dieser sind sowohl die drei dehnbaren Längen 
  \Length{smallskipamount}, \Length{medskipamount} und   
  \Length{bigskipamount}, welche von den Befehlen \Macro{smallskip},   
  ...
  Layout. Die im Dokument verwendete Schriftgröße kann durch den Anwender über 
  die \KOMAScript-Option \Option{fontsize=\PName{Schriftgröße}}|declare| 
  festgelegt werden.
  \Attention{%
    Dabei ist zu beachten, dass diese immer als Klassenoption angegeben 
    werden sollte.%
  } Mit der Wahl \Option{tudscrver=2.03} lässt sich diese Funktionalität 
  deaktivieren.
\item[\PValue{2.04}]
  \index{Satzspiegel}%
  \ChangedAt{%
