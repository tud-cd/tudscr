...
\Macro{KOMAoptions}(\Bundle{koma-script}) sowie 
\Macro{KOMAoption}(\Bundle{koma-script}).
...
\itemtrue*[light/lightfont/noheavyfont]
  ...
  (\seeref{\Option{headings=\PMisc}'ppage'}), darunter liegende Ebenen%
  \footnote{%
...
(\seeref{\Option{cdfont}}) definiert. Fette Mathematikschriften 
...
weiterhin sinnvoll verwendet werden zu können.

Eine automatische Anpassung an die Schriftgröße erfolgt für sowohl 
die dehnbaren Längen \Length{bigskipamount}, \Length{medskipamount} und 
...
\Length{abovecaptionskip}(\Bundle{koma-script}) und 
\Length{belowcaptionskip}(\Bundle{koma-script}), welche den vertikalen 
Abstand vor und nach einer, für ein Gleitobjekt~-- beispielsweise Abbildung 
oder Tabelle~-- mit \Macro{caption}(\Bundle{koma-script},\Package{caption}) 
...
\itemfalse
  ...
  \Attention{Nur in diesem Fall wirkt die Option \Option{DIV}(\Package{typearea}).}
...
\item[custom]
  ...
  \InlineDeclaration{\Macro{geometry}(\Package{geometry})} und 
  \InlineDeclaration{\Macro{newgeometry}(\Package{geometry})} definieren. 
  Für Hinweise weitern Verwendung des Paketes \Package{geometry} ist dessen 
...
\begin{Declaration}
  [v2.03]
  {\Option{extrabottommargin=\PLength}}[0pt]%
\printdeclarationlist
%
...
\begin{Declaration}
  [v2.05]
  {\Option{pageheadingsvskip=\PLength}}[0pt]%
\begin{Declaration}
  [v2.05]
  {\Option{headingsvskip=\PLength}}[0pt]%
\printdeclarationlist[%
  \index{Layout!Teileseiten|?}%
...
\begin{Declaration}
  [v2.03]
  {\Option{cdhead=\PMisc}}[true,nocolor,textwidth]
\printdeclarationlist[%
  \index{Layout!Kopfzeile}%
...
\itemfalse
  ...
  ist \emph{zwingend} das Paket \Package{scrlayer-scrpage}(\Bundle{koma-script})
  zu verwenden.
\itemtrue*
...
\item[color/bicolor/bichrome]
  ...
  Variante genutzt wird (\seeref{\Option{cdhead}}) oder ein 
...
\begin{Declaration}
  [v2.05]
  {\Option{footlogoheight=\PLength}}[0pt]%
\printdeclarationlist%
%
...
\begin{Declaration}
  [v2.02]
  {\Key{\Environment{tudpage}}{ddchead=\PMisc}}(\seeref{\Option{ddchead}'ppage'})
\begin{Declaration}
  [v2.02]
...
\begin{Declaration}
  [v2.02]
  {\Key{\Macro{makecover}}{cdfont=\PMisc}}(\seeref{\Option{cdfont}'ppage'})
\begin{Declaration}
  [v2.03]
...
\begin{Declaration}
  [v2.03]
  {\Key{\Macro{makecover}}{ddchead=\PMisc}}(\seeref{\Option{ddchead}'ppage'})
\begin{Declaration}
  [v2.03]
...
\itemtrue*
  ...
  \seeref*{\Option{cleardoublespecialpage}}.
\end{values}
%
...
\Macro*{renewcommand*}[\MPValue{\Macro{chapterpagestyle}(\Bundle{koma-script})}\MPName{Seitenstil}]
...
Nutzung von \Macro{chapter}(\Bundle{koma-script}) oder den davon 
...
\ToDo[doc]{normale Deklaration (Tabelle) für parskip und open?}
...
\item[color]
  Im farbigen Layout (\seeref{\Option{cd}}) ist auch die 
...
Wird ein Titelkopf (\KOMAScript-Option \Option{titlepage=false}(\Bundle{koma-script}))
und keine Titelseite verwendet, wird einer Zusammenfassung~-- identisch zu den 
\KOMAScript-Klassen~-- in einer \Environment{quotation}(\Bundle{koma-script})"~Umgebung 
...
(\Option{parskip=\PMisc}(\Bundle{koma-script})) \emph{keine Beachtung findet}. 
Um dies zu beheben, kann das Paket \Package{quoting} geladen werden, wodurch 
stattdessen die Umgebung \Environment{quoting}(\Package{quoting}) 
genutzt wird.
...
mehrspaltig gesetzt werden. An den Parameter \Key{\Environment{abstract}}{option} 
...
wurde. Werden mit \Option{manualmark}(\Package{scrlayer-scrpage}) manuelle 
...
\begin{Declaration}
  {\Macro{nextdeclaration}[\OPList{Sprache}\MPName{Überschrift}\MPName{Erklärung}]}
\begin{Declaration}
...
\ChangedAt{v2.02}%
Die Kolumnentitel lassen sich durch \Key{\Environment{declarations}}{markboth} 
...
auf die Erläuterung von \Macro*{thesis} und \Macro*{subject}'full' verwiesen.
...
\item[\PValue{first}/\PValue{2.02}]
  ...
  Der Satzspiegel (\seeref{\Option{cdgeometry}}) im Layout des 
  ...
  Seitenfußbereich (\seeref{\Option{ddcfoot}}) wird der identische Satzspiegel 
  genutzt. Dieses Verhalten lässt sich mit \Option{tudscrver=2.02} deaktivieren.
\item[\PValue{2.03}]
  \index{Leerraum}%
  \index{Schriftgröße}%
