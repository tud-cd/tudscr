...
und Optionen für den Mathematiksatz sind in \autoref{sec:math} erläutert. In 
...
%
\begin{values}
\item[nomath/serifmath/serif/nosansmath/nosans]  
  ...
  Mathematikmodus nutzen, siehe \Option{cdmath}[false].
\item[math/sansmath/sans]
  Es werden serifenlose Mathematikschriften für lateinische und griechische 
  Lettern genutzt, siehe \Option{cdmath}[true].
...
\begin{Declaration}{\Option{cdmath}[\PBoolean]}%
...
Tabelle~-- durch \Macro{TUDoptions}\PParameter{\Option{cdmath}[true]} und 
\Macro{TUDoptions}\PParameter{\Option{cdmath}[false]} möglich. Mit
\Macro{boldmath} kann auf fette Mathematikschriften umgeschaltet werden.

Mit der Einstellung \Option{cdmath}[false] wird auf die Standardschriften
...
\end{values}
%
\ToDo[doc]{Werte für slantedgreek}[v2.03]
\end{Declaration}
...
Allen Seitenstilen ist der typische Kopf mit dem charakteristischen Querbalken 
gemein. Die Gestalt des Kopfes kann mit der Option \Option{cdhead} für alle 
Seitenstile gleichermaßen angepasst werden. Die Ausprägung des Fußes ist für 
die einzelnen Seitenstile unterschiedlich. Dieser ist bei der Verwendung von
\PageStyle{empty.tudheadings} immer leer, die Stile \PageStyle{tudheadings} und 
...
Sobald einer der neu definierten Seitenstile aktiviert wurde, sind die
Stile \PageStyle{tudheadings} beziehungsweise \PageStyle{plain.tudheadings}
zusätzlich unter den Namen \PageStyle{headings} respektive \PageStyle{plain} 
verwendbar. Das hat den Vorteil, dass bei Optionen oder Befehlen, welche 
automatisch zwischen \PageStyle{headings} und \PageStyle{plain} umschalten, 
durch die einmalige Auswahl von einem der Seitenstile \PageStyle{tudheadings} 
oder \PageStyle{plain.tudheadings} nun zwischen diesen Stilen umgeschaltet 
wird. Der Seitenstil \PageStyle{empty} erzeugt allerdings weiterhin eine 
komplett leere Seite. Um auf das normale Verhalten zurückzuschalten, muss einer 
der Seitenstile \PageStyle{scrheadings} oder \PageStyle{plain.scrheadings} 
aktiviert werden.
\end{Declaration}
\end{Declaration}
\end{Declaration}
...
\begin{Declaration}{\Option{cdhead}[\PSet]}%
  [true][\Option{cdfont}[false]:false]
\printdeclarationlist%
...
%
Mit dieser Option lassen sich für Seiten, welche mit einem der 
\PageStyle{tudheadings}"=Seitenstilen gesetzt werden, sowohl die Gestalt des 
Querbalkens als auch die darin verwendete Schrift beeinflussen. Die folgenden 
Werte können für eine Anpassung der Schriftart im Balken verwendet werden:
%
\begin{values}
\itemfalse
...
\end{values}
%
Bei der Ausprägung des Querbalkens gibt es mehrere Varianten. Dieser kann 
farbig abgesetzt oder durch Außenlinien begrenzt erscheinen, wobei diese 
entweder bis zum Text"~ oder bis zum Blattrand laufen. Für den Fall, dass ein 
randloser Ausdruck technisch nicht möglich ist, können die Varianten bei denen 
der Querbalken bis an den Blattrand reicht Probleme bereiten. Deshalb kann mit 
der Option \Option{cdhead} die Breite des Querbalkens angepasst werden. 
...
%
\begin{values}
\item[textwidth/slim/normal/standard]
...
\item[color]
  \dots
  \ToDo[imp]{Implementierung fehlt}[v2.03]
  \ToDo[doc]{Doku vervollständigen}[v2.03]
\end{values}
\end{Declaration}
...
Die Ausprägung von Kopf"~ und Fußzeile ist nicht explizit durch das \CD 
vorgegeben. Eine Möglichkeit zur Gestaltung dieser zeigt das Handbuch für das 
\CD der \TnUD. Dieses wird ohne Kopf"~ und mit einer einfachen Fußzeile 
gesetzt, welche den aktuellen Kolumnentitel sowie die Paginierung enthält. 
...
Kopf"~ und Fußzeilen sehr flexibel und individuell anpassbar. Dennoch kann mit 
der Option \Option{cdfoot} das beschriebene Verhalten aktiviert werden, was 
auch für das hier vorliegende Anwenderhandbuch geschehen ist.
%
\begin{values}
\itemfalse
...
wobei diese nach den allgemeinen Einstellungen für alle Logos gesetzt werden.
Die möglichen Parameter für die globalen respektive individuellen optionalen 
...
\begin{Declaration}[%
  ...
  v2.03!Parameter \Key{\Environment{tudpage}}{color} entfernt
...
der \PageStyle{tudheadings}"=Seitenstile. Soll ein anderes Zweitlogo als das 
mit \Macro{headlogo} gegebene für die jeweilige \Environment{tudpage}-Umgebung 
erscheinen, so kann \Key{\Environment{tudpage}}{headlogo}[\PName{Dateiname}] 
verwendet werden.
...
\Option{cdfont}'full', \Option{cdhead}, \Option{cdfoot} sowie \Option{ddc}, 
\Option{ddchead} und \Option{ddcfoot} als auch die Befehle \Macro{headlogo} und 
\Macro{footlogo} in \autoref{sec:tudheadings}. Das Verhalten sowie die jeweils 
gültigen Wertzuweisungen können in den entsprechenden Abschnitten des Handbuchs 
nachgelesen werden.
\end{Declaration}
\end{Declaration}
\end{Declaration}
...
\begin{Declaration}[v2.03]{\Key{\Macro{maketitle}}{headlogo}[\PName{Dateiname}]}
\begin{Declaration}[v2.03]{%
...
(\Option{cdtitle}[true/lite/color]), können zusätzlich verschiedene Parameter 
im optionalen Argument verwendet werden, um die Nutzung der Schriften des \CDs 
zu regulieren sowie die Gestaltung von Kopf und Fuß zu beeinflussen. Alle 
Parameter entsprechen in ihrem Verhalten den gleichnamigen Klassenoptionen. Die 
jeweils gültigen Wertzuweisungen können den Beschreibungen der Option 
\Option{cdfont}'full' respektive der Optionen \Option{cdhead}, \Option{cdfoot}, 
\Option{ddc}, \Option{ddchead} und \Option{ddcfoot} sowie der Befehle 
\Macro{headlogo} und \Macro{footlogo} in \autoref{sec:tudheadings} entnommen 
werden. Die Einstellungen der Parameter wirken sich nur lokal und einzig auf 
die Titelseite aus.
\end{Declaration}
\end{Declaration}
\end{Declaration}
...
Befehl \Macro{maketitle} für die Änderung der Seitenzahl, der verwendeten 
Schrift sowie zur Anpassung von Kopf und Fuß verwendet werden.
\end{Declaration}
...
]{\Macro{makecover}\OLParameter{Seitenzahl}}{\Macro{maketitle}'auto'}
...
Des Weiteren können alle optionalen Parameter zur Anpassung der Seitenzahl, der 
verwendeten Schrift sowie der Gestalt von Kopf und Fuß genutzt werden, welche 
für den Befehl \Macro{maketitle} bereits erläutert wurden.
\end{Declaration}
\end{Declaration}
...
wenn der Satzspiegel des \CDs (\Option{geometry}[true/symmetric/balanced]) 
...
Einstellung zur Satzspiegelgestaltung für die Option \Option{geometry}~-- 
verwendet werden.
\index{Satzspiegel|)}\index{Layout!Satzspiegel|)}%
...
die Klassen \Class{tudscrreprt} und \Class{tudscrbook} in der Form eines 
...
Auch die beiden Umgebungen \Environment{abstract} und \Environment{declaration} 
...
Umgebung \Environment{abstract}'full' nachgelesen werden.
...
für die Kolumnentitel auf linker und rechter Seite \Term{confirmationname} 
respektive \Term{blockingname} verwendet. Außerdem kann der Anwender diese mit 
...
\section{Kompatibilitätseinstellungen für Umbruch und Layout}
...
\item[\PValue{v2.02}\textOR\PValue{last}]
  \ChangedAt{%
    ...
    wird mit ohne vergrößerten Seitenrand verwendet
  }
  Der Satzspiegel im Layout des \CDs (siehe Option~\Option{geometry}) wurde in 
  ...
  folglich identisch. Bei der Aktivierung des \DDC-Logos im Fußbereich 
  (siehe Option~\Option{ddcfoot}) wird im Gegensatz zur Version~v2.02 der 
  gleiche Satzspiegel genutzt. Mit \Option{tudscrver}[v2.02] kann dieses 
