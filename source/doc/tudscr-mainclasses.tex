...
die Absatzeinstellungen oder \Option{BCOR=\PName{Länge}} zur Festlegung der 
Bindekorrektur~-- unterstützen. Zusätzlich zu den \KOMAScript"=Klassen werden 
weitere Pakete zwingend benötigt, welche unter \autoref{sec:packages:needed} 
aufgeführt sind und auf jeden Fall durch \TUDScript geladen werden.
...
\itemtrue*
  ...
  (Klassenoption \Option{twoside=true}) wird die Paginierung außen platziert.
\end{values}
%
...
\end{values}
%
Wird der Option mit \Option{cdfoot=\protect\PName{Höhe}} ein Längenmaß 
übergeben, entspricht dies der Verwendung von \Option{extrabottommargin}.
\end{Declaration}
...
beziehungsweise \Option{cdfoot=\protect\PName{Höhe}} angepasst werden.
\end{Declaration}
\end{Declaration}
\end{Declaration}
...
ausreichen, so kann diese über die Option \Option{cdfoot=\protect\PName{Höhe}} 
beziehungsweise \Option{extrabottommargin} angepasst werden.
\end{Declaration}
\end{Declaration}
\end{Declaration}
...
Wird eine Titelseite (\KOMAScript-Option \Option{titlepage=true}) im \CD der 
\TnUD gesetzt (\Option{cdtitle=true}), können auch die weiterhin aufgeführten 
...
%
Im zweispaltigen Satz (Klassenoption~\Option{twocolumn=true}) wird mit 
...
\KOMAScript-Optionen \Option{twoside} und \Option{open=left} beziehungsweise 
\Option{open=right}\footnote{Standard bei \Class{tudscrbook}}. Für diese kann 
der Seitenstil mit der \KOMAScript"=Option \Option{cleardoublepage} eingestellt 
werden.
...
\Option{cdfoot=\protect\PName{Höhe}} mit einer Längenangabe verwendet werden. 
...
\Package{typearea} angebotene \KOMAScript-Option \Option{BCOR=\PName{Länge}} 
...
\Package{geometry} weitergereicht, so dass der Benutzer unabhängig von der 
Satzspiegelgestaltung (\Option{cdgeometry}) die Option \Option{BCOR} nutzen 
