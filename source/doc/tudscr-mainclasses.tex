...
\printdeclarationlist
Diese beiden Befehle dienen ebenfalls zu gezielten Auswahl einer Schriftart des 
\CDs in Stärke und Schnitt. Dabei muss im ersten obligatorischen Argument der 
Name der zu verwendenden Schriftart angegeben werden. Dieser ist der Tabelle zu 
entnehmen. Dabei muss zum einen der Vorsatz \emph{Univers} nicht zwingend 
verwendet werden, zum anderen sind weder Leerzeichen noch die passende Groß- 
und Kleinschreibung notwendig. Für die Wahl der Schriftstärke ist entweder die 
Bezeichnung\footnote{Light, Regular, Bold, Black} oder die entsprechende Zahl 
allein ausreichend. Für die geneigten Schriftschnitte sind für das Suffix 
\enquote{Oblique} auch \enquote{Italic} oder \enquote{Slanted} als Alias 
möglich.

Der Befehl \Macro{cdfont} entspricht einem Textschalter und ändert die aktuell 
verwendete Schriftart, wohingegen \Macro{textcdfont} als Textkommando fungiert 
und den im zweiten Argument gegebenen \PName{Text} in der gewünschten 
\PName{Schriftart} setzt.
\end{Declaration}
\end{Declaration}
...
\ToDo[imp,nxt]{%
  \Environment{tudpage} als obsolet deklarieren und in \File{tudscr-obsolete}
  dokumentieren
}[v2.xx]
...
%
Mit dieser Option wird es möglich, auf das Verhalten einer älteren Version von 
\TUDScript umzuschalten, um nach der Kompilierung das erwartete Ergebnis zu 
erhalten. Neue Möglichkeiten, die sich nicht auf den Umbruch oder das Layout 
auswirken, sind auch für den Fall verfügbar, dass per Option die Kompatibilität 
zu einer älteren Version ausgewählt wurde. 
...
%
\begin{values}
\item[\PValue{v2.02}\textOR\PValue{first}]
  \ChangedAt{%
