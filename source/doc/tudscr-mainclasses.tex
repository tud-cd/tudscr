...
\Option{parskip=\PName{Methode}} für die 
Absatzeinstellungen oder 
\Option{BCOR=\PName{Längenwert}} zur Festlegung der 
...
beiden \KOMAScript-Befehlen \Macro{KOMAoptions} 
beziehungsweise \Macro{KOMAoption}.
...
die beiden Längen \Length{abovecaptionskip} und 
\Length{belowcaptionskip} für den Abstand zwischen 
einem Gleitobjekt~-- beispielsweise eine Abbildung oder eine Tabelle~-- und 
dessen mit \Macro{caption} 
...
\InlineDeclaration{\Option{fontsize=\PLength}} festgelegt 
werden.
\Attention{%
...
dem Paket \Package{typearea}, wobei insbesondere die 
Klassenoption \Option{DIV=\PMisc} maßgeblichen Einfluss auf 
...
\itemval{false}
  ...
  Fall wirkt die Option \Option{DIV}.}
\itemval*{true,asymmetric,cd}
  ...
  (Klassenoption \Option{twoside=true}) genutzt.%
...
\itemval{twoside,balanced}
  ...
  (\Option{twoside=true}) und ändert den Satzspiegel 
  ...
  der \KOMAScript-Option \Option{open=left} immer auf 
...
\itemval{custom}
  ...
  Befehlen \InlineDeclaration{\Macro{geometry}} und 
  \InlineDeclaration{\Macro{newgeometry}} kann der Anwender 
...
\InlineDeclaration{\Option{BCOR=\PLength}} zu erwähnen. Mit 
...
(\Option{cdgeometry}) die Option \Option{BCOR} nutzen kann. 
...
\InlineDeclaration{\Option{headinclude=\PBoolean}} sowie 
\InlineDeclaration{\Option{footinclude=\PBoolean}}. Diese 
...
\footnote{%
  ...
  \Macro{frontispiece|\MPName{Frontispiz}} für eine 
...
(\KOMAScript-Option \Option{titlepage=false}) als auch die Kapitelüberschrift 
bei deaktivierter Kapitelseite (\Option{chapterpage=false}) in ihrer vertikalen 
Position anzupassen.
...
\printdeclarationlist[Layout!Kopfzeile,Layout!Querbalken]
...
Sobald mit \Macro{pagestyle|\MPName{Seitentil}}
...
[%
  ...
  v2.03!\Option{cdfoot=\PLength};%
]
\printdeclarationlist[Layout!Fußzeile,Layout!Kolumnentitel,Layout!Farbgestaltung]
...
die Option \InlineDeclaration{\Option{manualmark}} 
nicht explizit angegeben, so werden mit \Option{cdfoot=true} über die 
Option \InlineDeclaration{\Option{automark}} auch 
...
\Macro(\Package{graphicx}){includegraphics} weitergereicht. Dies 
...
\KOMAScript-Option \Option{titlepage=\PBoolean} festgelegt, ob 
...
(\Macro{frontispiece|\MPName{Frontispiz}}), der 
...
\footnote{%
  \KOMAScript-Optionen \Option{twoside=true} und 
  \Option{open=right}, Standard für \Class{tudscrbook}%
}
...
Im zweispaltigen Satz (Klassenoption~\Option{twocolumn}) 
...
(\KOMAScript-Option~\Option{titlepage=false}) genutzt, so 
...
(\Option{titlepage=true}) erfolgt die Ausgabe des 
...
Argument wird an \Macro{hypersetup} aus besagtem Paket 
...
\Macro{printdate} von \Package{isodate} oder 
\Macro{DTMDate} aus \Package{datetime2} genutzt. Mit 
...
sein, so wird mit \Macro{printdate} beziehungsweise 
\Macro{DTMDate} die durch das jeweilige Paket eingestellte 
...
dieser (\Macro{partpagestyle}) auf 
...
Weise wie \Macro{setpartpreamble} \emph{vor} der 
Verwendung von \Macro{part} oder den davon abgeleiteten 
...
Mit \Macro*{renewcommand*|\MPValue{\Macro{chapterpagestyle}}\MPName{Seitenstil}}
...
\InlineDeclaration{\Option{chapterprefix=\PBoolean}} 
...
gleicher Weise wie \Macro{setchapterpreamble} \emph{vor} der Verwendung von 
\Macro{chapter} oder den davon abgeleiteten Varianten angegeben werden. Mit 
dem Schriftelement \Font{chaptersubtitle} lässt sich die Schrift für den 
gegebenen Untertitel verändern. In \autoref{sec:fonts:elements} ist zu finden, 
wie es angepasst werden kann.
\end{Declaration}
\end{Declaration}
\index{Layout!Überschriften|)}%
\index{Kapitel|?)}%
...
\InlineDeclaration{\Option{cleardoublepage=\PMisc}} der 
...
\footnote{%
  \KOMAScript-Option \Option{twoside=true} und 
  \Option{open=right}%
}
...
\Option{cleardoublepage} immer als vollständig leere 
...
\Option{cleardoublepage} zu übernehmen. Des Weiteren 
...
\itemval*{true}
  ...
  der Einstellung von \Option{cleardoublepage} für 
  Vakatseiten.
\itemval{current}
  Für die erzeugte Rückseite wird der aktuell definierte Seitenstil 
  (\Macro{pagestyle}) verwendet.
...
\itemval{color}
  ...
  des Titels aus.\footnote{%
    \seeplain{\Macro{uppertitleback} und 
    \Macro{lowertitleback}} im \scrguide%
  }%
\itemval{nocolor}
...
\Macro{addtokomafont|\MPName{Schriftelement}\MPName{Einstellungen}} 
...
Diese Option wird für die Klassen \Class{scrartcl} 
und \Class{scrreprt} standardmäßig bereitgestellt. Für 
die Klasse \Class{scrbook} geschieht dies nicht. Dazu 
...
\itemval{chapter,addchap}(\Class{tudscrbook})
  ...
  Es wird der Befehl \Macro{chapter} für das Setzen der 
...
(\KOMAScript-Option~\Option{titlepage=true}) verwendet 
werden.
\begin{DeclareValues}
...
\itemval{multiple,multi,all,aggregate}
  ...
  \Option{twocolumn} aktiviert, erfolgt die Ausgabe aller 
...
(\KOMAScript-Option \Option{titlepage=false}), so wird 
eine Zusammenfassung~-- identisch zu den \KOMAScript-Klassen~-- in einer 
\Environment{quotation}"~Umgebung ausgegeben, wobei die 
...
\Option{parskip=\PMisc} nicht beachtet wird. Um dies zu 
...
\Macro{setchapterpreamble} verwendet, so wird die 
...
Mit dem \KOMAScript-Befehl \Macro{renewcaptionname}
...
\Macro{renewcaptionname|\MPName{Sprache}\MPName{Makro}\MPName{Inhalt}} 
zu verwenden:%
\begin{Code}
...
\Option{titlepage=true} eine Titelseite verwendet wird.
\begin{DeclareValues}
\itemval{single,one,simple}
...
\itemval{fill,fil,vfil,vfill}
  ...
  (Klassenoption \Option{twocolumn}) steht diese 
...
automatische (\Option{automark}) oder manuelle 
(\Option{manualmark}) Kolumnentitel genutzt. Mit 
\InlineDeclaration{%
...
\Macro{pdfbookmark} aus \Package{hyperref}. Die Lesezeichen 
...
\begin{Declaration}
  ...
  [v2.02]
\printdeclarationlist
  [Abschlussarbeit,Betreuer,Gutachter,Hochschullehrer,Referent]
