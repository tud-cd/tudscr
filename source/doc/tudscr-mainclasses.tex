...
\Option(\Bundle{koma-script}){parskip=\PMisc}
für die Absatzeinstellungen oder 
\Option(\Package{typearea}){BCOR=\PLength}
zur Festlegung der 
...
von \KOMAScript bereitgestellten Befehlen 
\Macro(\Bundle{koma-script}){KOMAoptions} sowie 
\Macro(\Bundle{koma-script}){KOMAoption}.
...
\Lettering{LaTeX}"=Standardschriften beziehungsweise ein anderes Schriftpaket 
zu verwenden. Die Erläuterungen dazu sind in \autoref{sec:text} zu finden. Zur 
Verwendung der Schriften mit \Lettering{LaTeX}-Klassen, welche nicht zum 
...
der Schriften im OpenType-Format ist auch beim Einsatz von \Format{LuaLaTeX} 
oder \Format{XeLaTeX} nicht zwingend erforderlich. Dennoch ist dies problemlos 
...
%
\begin{DeclareValues}[Layout!Kopfzeile,Layout!Überschriften]{\Option{cdfont}}
\itemval{false}
  Es werden die \Lettering{LaTeX}"=Standardschriften und nicht die Hausschrift 
  des \CDs verwendet. Der Anwender kann beliebige Schriftpakete nutzen.%
  \footnote{%
    Für die Verwendung der klassischen \Lettering{LaTeX}"~Schriften, ist das 
    Paket \Package{lmodern} sehr empfehlenswert.%
  }
\itemval*{true,light,lightfont,noheavyfont}
  Die Hausschrift \OpenSans des \CDs wird genutzt. Die Überschriften 
  der obersten Gliederungsebenen bis einschließlich \Macro*{subsubsection} 
  werden in (normal"~)fetter oder extra"~fetter Schriftstärke gesetzt
  (\seeplain{\Option'ppage'{headings=\PMisc}}), darunter liegende Ebenen%
  \footnote{%
    \Macro(\Bundle{koma-script}){paragraph} und 
    \Macro(\Bundle{koma-script}){subparagraph}%
  }
...
Ziffern anpassen.
%
\begin{DeclareValues}[Ziffernform]{\Option{cdfont}}
\itemval{liningfigures,normalfigures}
  \ChangedAt{v2.06}%
  ...
  aktiviert.
\itemval{oldstylefigures,osf,oldnumbers}
  \ChangedAt{v2.06}%
...
%
\begin{DeclareValues}{\Option{cdfont}}
\itemval{nomath,nocdmath} 
  Diese Einstellung deaktiviert die \OpenSans für den mathematischen Satz. Es 
  werden die \Lettering{LaTeX}"=Standardschriften verwendet und es lassen sich 
  beliebige Schriftpakete für den Mathematikmodus nutzen, 
  \seeplain{\Option{cdmath=false}}.
\itemval{math,cdmath}
  Die \OpenSans wird für die Mathematikschriften genutzt, 
  \seeplain{\Option{cdmath=true}}.
\itemval{upgreek,uprightgreek}
  Griechische Majuskeln und Minuskeln sind aufrecht, 
  \seeplain{\Option{slantedgreek=false}}.
\itemval{slgreek,slantedgreek}
  Die Ausgabe der griechischen Lettern erfolgt kursiv, 
  \seeplain{\Option{slantedgreek=true}}.
\itemval{texgreek,standardgreek}
  \ChangedAt{v2.06}%
  Es kommt der \Lettering{LaTeX}-Standard zum Einsatz, bei dem griechische 
  Majuskeln aufrecht und Minuskeln kursiv ausgegeben werden, 
  \seeplain{\Option{slantedgreek=standard}}.
\end{DeclareValues}

\ChangedAt{v2.02}%
Für die \TUDScript-Klassen sind bei den \PageStyle{tudheadings}"=Seitenstilen 
folgende Einstellungen für die Schriftart im Querbalken möglich:
%
\begin{DeclareValues}[Layout!Kopfzeile]{\Option{cdfont}}
\itemval{nohead,noheadfont}
  Bei deaktivierter Hausschrift für den Fließtext können diese ebenfalls im 
  Querbalken deaktiviert werden, \seeplain{\Option{cdhead=false}}.
\itemval{head,lighthead,lightfonthead,noheavyfonthead}
  ...
  \seeplain{\Option{cdhead=true}}.
\itemval{heavyhead,heavyfonthead}
  Im Querbalken wird~-- unabhängig von der Dokumentschrift~-- \OpenSans in 
  erhöhter Stärke genutzt, \seeplain{\Option{cdhead=heavy}}.
\end{DeclareValues}
\end{Declaration}
...
ist nicht notwendig.
%
\begin{DeclareValues}{\Option{ttfont}}
\itemval{false}
...
\subsection{%
  ...
  \ChangedAt*{%
    v2.06:Glyphen im Mathematiksatz werden passend zu den mit der Option 
...
(\seeplain{\Option{cdfont}}) definiert. Fette Mathematikschriften 
...
%
\begin{DeclareValues}{\Option{cdmath}}
\itemval{false}
  Es werden die normalen \Lettering{LaTeX}"=Serifenschriften beziehungsweise 
...
\printdeclarationlist[Griechische~Lettern]
...
%
\begin{DeclareValues}{\Option{slantedgreek}}
\itemval{false}
...
\itemval{standard,latex}
  Die Ausgabe entspricht mit aufrechten Majuskeln und kursiven Minuskeln der 
  griechischen Lettern dem \Lettering{LaTeX}-Standard beim mathematischen Satz.
\end{DeclareValues}
\end{Declaration}
...
zahlreiche zusätzliche für die \Lettering{LaTeX}"=Standardschriften zur 
Verfügung. Diese sind in Verbindung mit der \OpenSans aus typografischer Sicht 
jedoch keine ideale Lösung. Die Symbole aus dem Paket \Package{mdsymbol} passen 
...
\Length(\Bundle{koma-script}){abovecaptionskip} und 
\Length(\Bundle{koma-script}){belowcaptionskip} 
für den Abstand zwischen einem Gleitobjekt~-- beispielsweise eine Abbildung 
oder eine Tabelle~-- und dessen mit 
...
\InlineDeclaration{\Option(\Bundle{koma-script}){fontsize=\PLength}} festgelegt 
werden.
\Attention{%
...
%
\begin{DeclareValues}{\Option{relspacing}}
...
\begin{Declaration}
  ...
  (A4)
\begin{Declaration}
  {\Option{twoside=\PMisc}}
  (false|\Class{tudscrbook}:true)
  'true,false,semi'
\begin{Declaration}
  {\Option{twocolumn=\PBoolean}}
  (false)
\printdeclarationlist[Satzspiegel|?,Papierformat|?]

Die Einstellung des Satzspiegels erfolgt durch \KOMAScript normalerweise mit 
dem Paket \Package(\Bundle{koma-script}){typearea}, wobei insbesondere die 
Klassenoption \Option(\Package{typearea}){DIV=\PMisc} maßgeblichen Einfluss auf 
diesen hat. Mit 
...
festzulegen.
%
\begin{DeclareValues}[Satzspiegel!doppelseitig]{\Option{cdgeometry}}
\itemval{false}
  Die Satzspiegelberechnung erfolgt via \Package{typearea}, die Vorgaben des 
  \CDs bezüglich der Seitenränder werden ignoriert.
  \Attention{Nur in diesem Fall wirkt die Option \Option(\Package{typearea}){DIV}.}
\itemval*{true,asymmetric,cd}
  ...
  (Klassenoption \Option'none'(\Package{typearea}){twoside=true}) genutzt.%
...
\itemval{twoside,balanced}
  ...
  (\Option'none'(\Package{typearea}){twoside=true}) und ändert den Satzspiegel 
  ...
  der \KOMAScript-Option \Option(\Bundle{koma-script}){open=left} immer 
...
%
\begin{DeclareValues}{\Option{cdgeometry}}
\itemval{restricted}
...
\InlineDeclaration{\Option(\Package{typearea}){BCOR=\PLength}}
zu erwähnen. Mit dieser kann bei der Satzspiegelberechnung ein Heftrand respektive 
...
(\Option{cdgeometry}) die Option \Option(\Package{typearea}){BCOR} nutzen 
kann. So lässt sich eine Bindekorrektur von beispielsweise 5\,mm mit der 
\emph{Klassenoption} \Option*(\Package{typearea}){BCOR=5mm} festlegen.
...
\InlineDeclaration{\Option(\Package{typearea}){headinclude=\PBoolean}} sowie 
\InlineDeclaration{\Option(\Package{typearea}){footinclude=\PBoolean}}.
...
wenn der Satzspiegel des \CDs verwendet wird (\seeplain{\Option{cdgeometry}}),
...
\printdeclarationlist
  [Titel|?,Umschlagseite|?,Teil|?,Kapitel|?,Layout!Überschriften]
...
\itemval*{true,nocolor,monochrome}
  ...
  schwarze Schrift für Titel, Teil- und Kapitelüberschriften verwendet.
...
Die Option \Option(\Bundle{koma-script}){headings=\PMisc} 
wird bereits von \KOMAScript definiert und dient unter anderem zum Festlegen der
Größe von Überschriften. Diese wird von \TUDScript erweitert, um im Layout des 
\CDs die Schriftstärke der Überschriften anpassen zu können. Die Einstellungen 
gelten für die Gliederungsebenen bis einschließlich \Macro*{subsubsection}.
%
\begin{DeclareValues}{\Option{headings}}
...
\footnote{%
  \Macro(\Bundle{koma-script}){extratitle|\MPName{Schmutztitel}},
  \Macro(\Bundle{koma-script}){frontispiece|\MPName{Frontispiz}} für eine 
  Seite vor dem eigentlichen Haupttitel sowie
  \Macro(\Bundle{koma-script}){titlehead|\MPName{Kopf}},
  \Macro{subject|\MPName{Typisierung}}, \Macro{title|\MPName{Titel}},
  \Macro{subtitle|\MPName{Untertitel}}, \Macro{author|\MPName{Autor}},
  \Macro{and}, \Macro(\Bundle{koma-script}){thanks|\MPName{Fußnote}}, 
  \Macro(\Bundle{koma-script}){publishers|\MPName{Verlag}} und 
  \Macro{date|\MPName{Datum}} für den Titel selbst sowie 
  \Macro(\Bundle{koma-script}){uppertitleback|\MPName{Titelrückseitenkopf}} und
  \Macro(\Bundle{koma-script}){lowertitleback|\MPName{Titelrückseitenfuß}} 
  für die Rückseite des Titels sowie abschließend 
  \Macro(\Bundle{koma-script}){dedication|\MPName{Widmung}} für eine 
  Danksagung.
}
...
\Macro(\Bundle{koma-script}){part} und 
\Macro(\Bundle{koma-script}){addpart} sowie deren Sternversionen 
...
Verwendung von \Macro(\Bundle{koma-script}){chapter} beziehungsweise 
\Macro(\Bundle{koma-script}){addchap} und den dazugehörigen 
...
\Macro(\Bundle{koma-script}){subsection}, 
\Macro(\Bundle{koma-script}){subsubsection} sowie 
\Macro(\Bundle{koma-script}){paragraph} und 
\Macro(\Bundle{koma-script}){subparagraph}
...
\begin{Declaration}
  ...
  (0pt)
  [v2.05]
\printdeclarationlist[Teil|?,Kapitel|?]

\ToDo{Standardeinstellung für \Option*{pageheadingsvskip} dokumentieren}[v2.06]%
...
(\KOMAScript-Option \Option(\Bundle{koma-script}){titlepage=true}) als 
...
(\KOMAScript-Option \Option(\Bundle{koma-script}){titlepage=false}) als 
...
\Macro(\Bundle{koma-script}){pagestyle|\MPName{Seitenstil}} 
...
anverwandten Stile \PageStyle{tudheadings} und \PageStyle{plain.tudheadings} 
übernehmen die Einstellungen für die Fußzeile aus der Anwenderschnittstelle 
von \Package{scrlayer-scrpage}.%
\footnote{%
...
Sobald mit \Macro(\Bundle{koma-script}){pagestyle|\MPName{Seitentil}} 
...
\PageStyle(\Package{scrlayer-scrpage}){scrheadings} oder 
\PageStyle(\Package{scrlayer-scrpage}){plain.scrheadings}
aktiviert werden.
\end{Declaration}
\end{Declaration}
\end{Declaration}
...
\printdeclarationlist[Layout!Kopfzeile,Layout!Farbgestaltung,Datum]
...
%
\begin{DeclareValues}{\Option{cdhead}}
\itemval{false}
...
%
\begin{DeclareValues}[Layout!Farbgestaltung]{\Option{cdhead}}
...
Seitenbreite, \seeplain{\Option{cdhead=paperwidth}}.
%
\begin{DeclareValues}[Layout!Farbgestaltung]{\Option{cdhead}}
\itemval{barcolor}
  \ChangedAt{v2.04}%
...
eingeblendet werden.
%
\begin{DeclareValues}[Datum]{\Option{cdhead}}
\itemval{date,showdate}
  Das eingestellte Datum wird rechts oberhalb vom Textbereich eingeblendet.
\itemval{nodate,hidedate}
...
\printdeclarationlist[Layout!Fußzeile]

Die \TUDScript-Klassen sind~-- insbesondere aufgrund der Möglichkeit zur 
Verwendung des Paketes \Package(\Bundle{koma-script}){scrlayer-scrpage}~-- bei 
...
%
\begin{DeclareValues}[Satzspiegel!doppelseitig]{\Option{cdfoot}}
\itemval{false}
  Die Kopf- und Fußzeilen zeigen Standardverhalten, zur manuellen Änderung 
  ist \emph{zwingend} das Paket \Package(\Bundle{koma-script}){scrlayer-scrpage}
  zu verwenden.
\itemval*{true}
  ...
  (Klassenoption \Option(\Package{typearea}){twoside=true}) die 
  Paginierung außen platziert wird.
\end{DeclareValues}

\index{Kolumnentitel}%
Wird beim Laden der Klasse respektive des Paketes \Package{scrlayer-scrpage} 
die \KOMAScript-Option \InlineDeclaration{\Option(\Package{scrlayer-scrpage}){manualmark}}
nicht explizit angegeben, so werden mit \Option{cdfoot=true} über die 
\KOMAScript-Option \InlineDeclaration{\Option(\Package{scrlayer-scrpage}){automark}}
...
%
\begin{DeclareValues}[Layout!Farbgestaltung]{\Option{cdfoot}}
\itemval{nocolor,monochrome}
  Der Fuß wird immer ohne farbigen Hintergrund gesetzt.
\itemval{color,bicolor,bichrome}
  Die Fußzeile wird farbig abgesetzt, falls entweder der Kopf in einer farbigen
  Variante genutzt wird (\seeplain{\Option{cdhead}}) oder ein farbiger 
  Seitenhintergrund in der Hausfarbe (Titel oder Kapitelseite) aktiv ist.
\end{DeclareValues}
...
%
\begin{DeclareValues}{\Option{cdfoot}}
\itemval{\PSet{Höhe}}
...
\Macro(\Package{graphicx}){includegraphics} durchgereicht werden, um 
...
\Macro(\Package{graphicx}){includegraphics}
...
von  \Option*'none'{footlogoheight=0pt} werden diese auf die Höhe des Logos der 
...
lässt sich gegebenenfalls das optionale Argument für die Schriftformatierung 
nutzen.
\ChangedAt{v2.06}%
...
%
\begin{DeclareValues}{\Option{ddc}}
\itemval{false}
...
manuell ändern:
%
\begin{DeclareValues}[Layout!Farbgestaltung]{\Option{ddc}}
\itemval{color}
...
angewandte Sprache mit \Environment{tudpage/language=\PSet{Sprache}}
...
Parameter \Environment{tudpage/columns=\PSet{Anzahl}} der Inhalt der 
gegebenen Anzahl entsprechend mehrspaltig gesetzt. Der gewünschte  
\PageStyle{tudheadings}"=Seitenstil kann mit dem Parameter 
\Environment{tudpage/pagestyle=\PSet{Seitenstil}} angepasst werden, 
...
\KOMAScript-Option 
\InlineDeclaration{\Option(\Bundle{koma-script}){titlepage=\PBoolean}}
...
(\Macro(\Bundle{koma-script}){extratitle}) inklusive dazugehöriger Rückseite 
(\Macro(\Bundle{koma-script}){frontispiece|\MPName{Frontispiz}}), der 
...
\begin{Declaration}
  ...
  [
    ...
    v2.02:Unterstützung der Schriftelemente \Font*{titlepage} und \Font*{thesis}
      ...
      \Font*(\Bundle{koma-script}){titlehead}, 
      \Font*(\Bundle{koma-script}){subject}, 
      \Font*(\Bundle{koma-script}){title}, 
      \Font*(\Bundle{koma-script}){subtitle}, 
      \Font*(\Bundle{koma-script}){author}, 
      \Font*(\Bundle{koma-script}){publishers}, 
      \Font*(\Bundle{koma-script}){date} und 
      \Font*(\Bundle{koma-script}){dedication};
  ]
\begin{Declaration}
...
\begin{Declaration}
  ...
  <\seeplain{\Option'ppage'{cdgeometry}}>
  [v2.06]
\begin{Declaration}
...
\footnote{%
  \KOMAScript-Optionen \Option(\Package{typearea}){twoside=true} und 
  \Option(\Bundle{koma-script}){open=right},
  Standard für \Class{tudscrbook}%
}
so wird zusätzlich die Option \Option{cleardoublespecialpage} einbezogen. Dies 
ist insbesondere bei den Befehlen \Macro(\Bundle{koma-script}){uppertitleback} 
respektive \Macro(\Bundle{koma-script}){lowertitleback} für die Titelrückseite 
zu beachten.
...
(\KOMAScript-Option \Option(\Bundle{koma-script}){titlepage=true}) im
...
(Klassenoption~\InlineDeclaration{\Option(\Package{typearea}){twocolumn}})
...
(\KOMAScript-Option~\Option(\Bundle{koma-script}){titlepage=false})
genutzt, so folgt diesem die einspaltige Textpassage aus dem obligatorischen 
...
(\KOMAScript-Option~\Option(\Bundle{koma-script}){titlepage=true})
...
\begin{Declaration}
  ...
  [
    ...
    v2.02:Unterstützung der Schriftelemente \Font*{titlepage} und \Font*{thesis}
      ...
      \Font*(\Bundle{koma-script}){title}, 
      \Font*(\Bundle{koma-script}){subtitle}, 
      \Font*(\Bundle{koma-script}){author} und
      \Font*(\Bundle{koma-script}){publishers};
  ]
\begin{Declaration}
...
die Autoren und gegebenenfalls der mit \Macro(\Bundle{koma-script}){publishers}
...
\begin{Entity}{\Bundle{koma-script}}
\begin{Declaration}
  {\Option{titlepage=\PMisc}}%
  (true|\Class{tudscrartcl}:false)
  'true,false,firstiscover'
\begin{Declaration}
...
von der Option \Option{headings} abhängt und der Untertitel \emph{immer} 
...
Argument wird an \Macro(\Package{hyperref}){hypersetup} aus besagtem
...
\Macro(\Package{isodate}){printdate} von \Package{isodate} oder 
\Macro(\Package{datetime2}){DTMDate} aus \Package{datetime2} genutzt. Mit 
...
Umgebung \Environment(\Package{tudscrsupervisor}){task} genutzt werden, falls 
...
Dieser Befehl wird sowohl bei den \Lettering{LaTeX}"=Standardklassen als auch 
bei den \KOMAScript-Klassen lediglich auf der Titelseite dazu verwendet, 
mehrere Autoren im Argument von \Macro{author} voneinander zu trennen.
...
\Macro{advisor} verwendet werden, um mehrere Betreuer respektive Gutachter 
und Fachreferenten anzugeben. Er ist dabei nicht allein auf die Titelei 
beschränkt sondern lässt sich auch bei der Angabe von Personen in den 
...
\Macro{date/before} und \Macro{date/after} genutzt werden, um 
...
der Parameter \Macro{date/place} verwendet wird.
...
sein, so wird mit \Macro(\Package{isodate}){printdate} beziehungsweise 
\Macro(\Package{datetime2}){DTMDate} die durch das jeweilige Paket 
...
außerdem zur Erkennung von Abschlussarbeiten, da für diese spezielle Felder 
...
%
\begin{DeclareValues}{\Option{subjectthesis}}
\itemval{false}
...
Ausgabe kleiner und in geringerer Schriftstärke.
\end{Declaration}
...
Beschreibung (\seeplain{\autoref{sec:localization}}) vorangestellt. Dazwischen 
...
Dokumenttitels durch das \CD fixiert ist, kann es~-- im Gegensatz zu 
...
muss gegebenenfalls vom Anwender kontrolliert werden. Mit \Macro{publishers} lassen 
sich ein Verlag oder auch andere Informationen am Ende der Titelseite ausgeben.

Fußnoten werden auf dem Titel nicht mit 
\Macro(\Bundle{koma-script}){footnote}, 
sondern mit der Anweisung \Macro{thanks} erzeugt. Diese dienen in der Regel 
für Anmerkungen bei Titel oder den Autoren. Als Fußnotenzeichen werden dabei 
Symbole statt Zahlen verwendet. Der Befehl \Macro{thanks} kann nur innerhalb 
des Arguments einer der Anweisungen für die Titelseite wie beispielsweise 
\Macro{author} oder \Macro{title} verwendet werden.
...
\ChangedAt[\Macro(\Bundle{koma-script}){partpagestyle}]{%
...
dieser (\Macro(\Bundle{koma-script}){partpagestyle}) auf 
\PageStyle{plain.tudheadings} gesetzt. Möchten Sie stattdessen einen anderen 
Seitenstil nutzen, so kann dieser mit 
\Macro*{renewcommand*|%
  \MPValue{\Macro(\Bundle{koma-script}){partpagestyle}}\MPName{Seitenstil}%
}
angepasst werden.
...
Weise wie \Macro(\Bundle{koma-script}){setpartpreamble}
\emph{vor} der Verwendung von 
\Macro(\Bundle{koma-script}){part} 
...
Bezeichnung des mit 
\Macro(\Bundle{koma-script}){part|\MPName{Bezeichnung}} 
...
\notudscrartcl
%
\begin{DeclareValues}{\Option{parttitle}}
\itemval{false}
...
\itemval*{true}
  ...
  werden auf einer separaten Seite gesetzt. Der folgende Text erscheint auf der 
  nächsten respektive übernächsten Seite, \seeplain*{\Option{cleardoublespecialpage}}.
\end{DeclareValues}

Mit \Macro*{renewcommand*|%
  \MPValue{\Macro(\Bundle{koma-script}){chapterpagestyle}}%
  \MPName{Seitenstil}%
}
...
\InlineDeclaration{\Option(\Bundle{koma-script}){chapterprefix=\PBoolean}}
...
können nur im doppelseitigem Satz (\Option(\Package{typearea}){twoside=true}) 
vor dem Beginn eines Teils oder Kapitels abhängig von der \KOMAScript-Option 
\InlineDeclaration{\Option(\Bundle{koma-script}){open=\PMisc}}<any,left,right> 
auftreten. Wird ein neuer Teil oder ein neues Kapitel immer auf einer rechten 
Seite erzeugt (\Option(\Bundle{koma-script}){open=right}),%
\footnote{%
  Voreinstellungen
  \Class{tudscrbook}: 
  \Option(\Package{typearea}){twoside=true}, 
  \Option(\Bundle{koma-script}){open=right};
  \Class{tudscrartcl}, \Class{tudscrreprt}: 
  \Option(\Bundle{koma-script}){open=any}, 
  \Option(\Package{typearea}){twoside=false}%
}
...
\InlineDeclaration{\Option(\Bundle{koma-script}){cleardoublepage=\PMisc}}
der Seitenstil eingestellt werden.
...
\footnote{%
  \KOMAScript-Option \Option(\Package{typearea}){twoside=true} und 
  \Option(\Bundle{koma-script}){open=right}%
}
...
\Option(\Bundle{koma-script}){cleardoublepage}
...
\Option(\Bundle{koma-script}){cleardoublepage} zu übernehmen. Des 
...
\notudscrartcl
%
\begin{DeclareValues}{\Option{cleardoublespecialpage}}
\itemval{false}
...
\itemval*{true}
  ...
  der Einstellung von \Option(\Bundle{koma-script}){cleardoublepage} für 
  Vakatseiten.
\itemval{current}
  Für die erzeugte Rückseite wird der aktuell definierte Seitenstil verwendet.
\itemval{\PSet{Seitenstil}}
...
\itemval{color}
  ...
  \seeplain{\Option{cd}}. Die Einstellung wirkt sich ebenfalls auf die 
  Rückseite des Titels aus.\footnote{%
    \seeplain{\Macro(\Bundle{koma-script}){uppertitleback} und 
    \Macro(\Bundle{koma-script}){lowertitleback}} im \scrguide%
  }%
\itemval{nocolor}
...
Bedarfsfall mit 
\Macro(\Bundle{koma-script}){addtokomafont|\MPName{Schriftelement}\MPName{Einstellungen}}
anpassen. Mehr dazu ist im \scrguide innerhalb des Abschnitts 
...
\printdeclarationlist[Satzspiegel!zweispaltig]
...
Neben den Auswahlmöglichkeiten, welche \KOMAScript bereits für die Klassen 
...
ausgegeben werden.
%
\begin{DeclareValues}[Layout!Überschriften]{\Option{abstract}}
\itemval{false}<nur \Class{tudscrartcl} und \Class{tudscrreprt}>
  Es wird keine Überschrift für die \Environment{abstract}-Umgebung ausgegeben.
\itemval*{true}<nur \Class{tudscrartcl} und \Class{tudscrreprt}>
...
\itemval{chapter,addchap}(\Class{tudscrbook})
  ...
  Es wird der Befehl \Macro(\Bundle{koma-script}){chapter} für das 
...
%
\begin{DeclareValues}[Inhaltsverzeichnis]{\Option{abstract}}
...
\itemval{tocleveldown,leveldown,totocleveldown}
  \ChangedAt{v2.02}%
...
\itemval{tocmultiple,totocmultiple,tocaggregate,totocaggregate}
  \ChangedAt{v2.04}%
...
\ChangedAt{v2.02}%
\index{Kolumnentitel}%
Außerdem lässt sich mit dem Paket \Package{scrlayer-scrpage} das Verhalten der 
Kolumnentitel beeinflussen. Diese werden normalerweise automatisch gesetzt, 
wenn diese über die Option \Option(\Package{scrlayer-scrpage}){automark} 
aktiviert wurden und sind von der aktuellen Gliederungsebene der Überschrift 
abhängig. Werden jedoch mit Hilfe der Option 
\Option(\Package{scrlayer-scrpage}){manualmark} manuelle Kolumnentitel 
...
%
\begin{DeclareValues}[Kolumnentitel]{\Option{abstract}}
\itemval{markboth}
...
Seite ausgeben~-- sofern genügend Platz vorhanden ist. Außerdem kann 
...
Zusammenfassungen innerhalb der \Environment{abstract}-Umgebung werden 
ausschließlich wirksam, wenn eine Titelseite 
(\KOMAScript-Option \Option'none'(\Bundle{koma-script}){titlepage=true}) und 
\emph{keine} Überschriften in Form von Kapiteln (\Option{abstract=chapter}) 
verwendet werden.
%
\begin{DeclareValues}{\Option{abstract}}
...
\itemval{multiple,multi,all,aggregate}
  ...
  \Option(\Package{typearea}){twocolumn} aktiviert, erfolgt die Ausgabe 
...
\itemval{fill,fil,vfil,vfill}
  ...
  (Klassenoption \Option(\Package{typearea}){twocolumn}) nicht zur Verfügung.
...
\begin{Declaration}
  ...
  <\seeplain{\Option'ppage'{abstract}}>
\printdeclarationlist
...
Wird ein Titelkopf 
(\KOMAScript-Option \Option(\Bundle{koma-script}){titlepage=false})
und keine Titelseite verwendet, so wird eine Zusammenfassung~-- identisch zu den 
\KOMAScript-Klassen~-- in einer 
\Environment(\Bundle{koma-script}){quotation}-Umgebung 
ausgegeben, wobei die Überschrift \emph{nicht} in der Form einer Gliederungsebene 
gesetzt wird. Gleiches gilt für die Präambeln von Teilen und Kapiteln. Die 
\Environment{quotation}-Umgebung
hat allerdings den Nachteil, dass die Einstellung zur Absatzauszeichnung 
(\Option(\Bundle{koma-script}){parskip=\PMisc}) \emph{keine Beachtung findet}. 
Um dies zu beheben, kann das Paket \Package{quoting} geladen werden, wodurch 
stattdessen die Umgebung \Environment(\Package{quoting}){quoting} 
genutzt wird.
...
optionalen Parameter \Environment{abstract/language=\PSet{Sprache}} die 
...
auch ohne die Verwendung des Parameters \Environment{abstract/language} 
direkt als optionales Argument übergeben werden.

\ChangedAt{v2.02}%
\index{Kolumnentitel}%
Mit \Environment{abstract/markboth} können die gesetzten Kolumnentitel 
beeinflusst werden. Wird
\InlineDeclaration{\Environment{abstract/markboth=false}}
angegeben, werden automatische respektive manuelle Kolumnentitel verwendet. 
Die Einstellung
\InlineDeclaration{\Environment{abstract/markboth=true}}
wiederum setzt diese für linke und rechte Seiten auf \Term{abstractname}. 
Zusätzlich lässt sich mit
\InlineDeclaration{\Environment{abstract/markboth=\PSet{Kolumnentitel}}}
der Kolumnentitel direkt festlegen. So können die Kolumnen beispielsweise mit 
der Verwendung von \Environment{abstract/markboth=\MPValue{}} auch 
gelöscht werden. Sollte \Environment{abstract/markboth} aktiviert 
werden, so wird in der Umgebung automatisch der Seitenstil \PageStyle{headings} 
genutzt~-- falls eine Titelseite
(\KOMAScript-Option \Option(\Bundle{koma-script}){titlepage=true}) 
verwendet wird. Mit dem Parameter \Environment{abstract/pagestyle} kann 
...
\Environment{abstract/columns=\PSet{Anzahl}} die Zusammenfassung 
mehrspaltig gesetzt werden. An den Parameter \Environment{abstract/option} 
...
\Macro(\Bundle{koma-script}){setchapterpreamble} verwendet, so wird die 
...
Mit dem \KOMAScript-Befehl \Macro(\Bundle{koma-script}){renewcaptionname}
...
\Macro(\Bundle{koma-script}){renewcaptionname|\MPName{Sprache}\MPName{Makro}\MPName{Inhalt}}
zu verwenden:%
\begin{Code}
...
%
\begin{DeclareValues}[Layout!Überschriften]{\Option{declaration}}
\itemval{false}
...
%
\begin{DeclareValues}[Inhaltsverzeichnis]{\Option{declaration}}
\itemval{notoc,nottotoc}
...
beiden Gliederungsebenen gesetzt, falls dies über die 
\KOMAScript-Option \Option(\Package{scrlayer-scrpage}){automark} aktiviert 
wurde. Werden mit \Option(\Package{scrlayer-scrpage}){manualmark} manuelle 
Kolumnentitel genutzt, müssen diese auch für die Erklärungen manuell gesetzt 
werden. Mit \Option{declaration=markboth} lässt sich unabhängig davon das 
Setzen der Kolumnentitel auf linken und rechten Seiten forcieren, wobei 
hierfür der Titel der Überschrift genutzt wird.
%
\begin{DeclareValues}[Kolumnentitel]{\Option{declaration}}
\itemval{markboth}
...
\itemval{nomarkboth}
  Die Einstellung für manuelle oder automatische Kolumnentitel werden beachtet.
\end{DeclareValues}
Für die Befehle \Macro{declaration} beziehungsweise \Macro{confirmation} und 
\Macro{blocking} sowie die \Environment{declarations}-Umgebung lässt sich mit 
dem Parameter \Environment{declarations/markboth} ein beliebiger Kolumnentitel 
setzen.
...
\Option(\Bundle{koma-script}){titlepage=true} eine Titelseite verwendet wird.
%
\begin{DeclareValues}{\Option{declaration}}
\itemval{single,one,simple}
...
\itemval{fill,fil,vfil,vfill}
  ...
  (Klassenoption \Option(\Package{typearea}){twocolumn}) steht diese 
...
\Environment{declarations/language=\PSet{Sprache}} für die 
...
\ChangedAt{v2.02}%
\index{Kolumnentitel}%
Die Kolumnentitel lassen sich durch \Environment{declarations/markboth} 
beeinflussen. Mit
\InlineDeclaration{\Environment{declarations/markboth=true}}
...
\InlineDeclaration{\Environment{declarations/markboth=false}}
werden je nach gewählter Option für das Paket \Package{scrlayer-scrpage} 
automatische (\Option(\Package{scrlayer-scrpage}){automark}) oder manuelle
(\Option(\Package{scrlayer-scrpage}){manualmark}) Kolumnentitel genutzt. Mit 
\InlineDeclaration{\Environment{declarations/markboth=\PSet{Kolumnentitel}}}
lassen sich diese auch direkt festlegen.

Wird der Parameter \Environment{declarations/markboth} in irgendeiner 
Form genutzt, wird der Seitenstil \PageStyle{headings} automatisch aktiviert. 
Alternativ kann mit dem Parameter \Environment{declarations/pagestyle}
...
\Environment{declarations/columns=\PSet{Anzahl}} mehrspaltig gesetzt. 
...
\Environment{declarations/supporter} sowie 
\Environment{declarations/place} und 
\Environment{declarations/closing} sind in der Dokumentation des Befehls 
\Macro{confirmation} zu finden, der Parameter 
\Environment{declarations/company} ist für \Macro{blocking} erläutert.
\end{Declaration}
\end{Declaration}
\end{Declaration}
\end{Declaration}
...
Parameter \Macro{confirmation/supporter} oder \emph{zuvor} mit dem 
...
\Macro{confirmation/place} beziehungsweise \Macro{place} angegebene Ort 
...
mit dem Parameter \Macro{confirmation/closing} oder zuvor mit dem 
Befehl \Macro{confirmationclosing} angepasst werden. Die Parameter 
\Environment{declarations/language}, 
\Environment{declarations/markboth}, 
\Environment{declarations/pagestyle}, 
\Environment{declarations/columns} und 
...
respektive durch die Option \Option{subjectthesis} angegeben wurde~-- aufgeführt. 
Mit \Macro{blocking/company} oder \emph{vorher} mit \Macro{company} kann 
...
%
\begin{DeclareValues}{\Option{tudbookmarks}}
\itemval{false}
...
\Macro(\Package{hyperref}){pdfbookmark} aus \Package{hyperref}. Die Lesezeichen 
...
\Macro(\Bundle{koma-script}){providecaptionname|\MPName{Sprache}\MPName{Makro}\MPName{Inhalt}}
...
\footnote{%
  \Macro{confirmation|%
    \OPValue{\Macro{confirmation/supporter=\PSet{Unterstützer}}}}
...
\begingroup
  ...
  \newcommand*\showfield[1]{%
    \Macro*'none'{getfield|\MPValue{#1}}~\textrightarrow~\getfield{#1}%
  }%
  \noindent%
