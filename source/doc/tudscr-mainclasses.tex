...
\end{Declaration}
\end{Declaration}
\ToDo[doc]{\Macro{cdfont} und \Macro{textcdfont}}[v2.04]
...
Kompilierung eines \hologo{LaTeX}"~Dokumentes über den klassischen Prozesspfad 
...
\Package{iwona} entnommen werden müssen. Zusätzlich kann es auch beim Kerning 
der Schriften es zu Problemen kommen.

\ToDo[doc]{So nicht richtig, \Package{fontspec} für Umlaute notwendig}[v2.04]
...
beim Einsatz von \hologo{LuaLaTeX} oder \hologo{XeLaTeX} keinen triftigen 
Grund, das Paket \Package{fontspec} zu verwenden. Wird es dennoch geladen, 
...
\begin{Declaration}[%
  ...
  v2.04!farbiger Querbalken möglich%
]{\Option{cd}[\PSet]}[true]
\printdeclarationlist%
\index{Layout}%
%
...
Option \Option{cdhead} angepasst werden kann. Die Ausprägung des Fußes 
...
\ToDo[doc]{%
  Hinweis auf Verwendung erst nach dem Laden von \Package{scrlayer-scrpage}
  bzw. nach \Macro*{begin}\PParameter{document} mit \Macro*{Attention}%
}[v2.04]
\end{Declaration}
\end{Declaration}
\end{Declaration}
...
\ToDo[doc]{\Macro{footcontent} und \Macro{footlayer} dokumentieren}[v2.04]
%\footcontent[\color{red}]{\bla}[\bla]
\ToDo[doc]{\Font{tudheadings} dokumentieren}[v2.04]
\ToDo[doc]{Längenangabe entspricht \Option{extrabottommargin}}[v2.04]
\begin{Declaration}[%
