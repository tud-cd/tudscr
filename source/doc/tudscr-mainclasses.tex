...
Orientierung festlegen. Der doppelseitige Satz kann mit \Option{twoside=\PMisc} 
...
(Klassenoption~\InlineDeclaration{\Option{twocolumn}(\Package{typearea})}) wird 
mit \Macro{maketitle} die Titelseite selbst immer einspaltig gesetzt. Direkt 
nach dem Titel folgt normalerweise der zweispaltige Fließtext. Mit dem Befehl 
...
(\KOMAScript-Option~\Option{titlepage=true}(\Bundle{koma-script})) erfolgt die 
Ausgabe des Argumentes \MPName{Einspaltentext} direkt nach dieser auf einer 
...
Titelkopf (\KOMAScript-Option~\Option{titlepage=false}(\Bundle{koma-script})) 
...
Der Befehl \Macro{thesis} dient den \TUDScript-Klassen außerdem zum Erkennen 
