...
auf einer Seite unterdrückt werden. Damit kann der Anwender gegebenenfalls die 
Positionierung selbstständig vornehmen. Es ist zu beachten, dass die hier 
vorgestellten Einstellungen zur Positionierung der Zusammenfassungen innerhalb 
der \Environment{abstract}"=Umgebung nur wirksam sind, wenn eine Titelseite 
(\Option{titlepage}[true]) und \emph{keine} Überschriften in Form von Kapiteln 
(\Option{abstract}[chapter]) verwendet werden.
%
\begin{values}
...
als optionales Argument anpassen. Damit lassen sich Einstellungen der Option 
\Option{abstract} für eine bestimmte gegebenenfalls Umgebung anpassen. 
...
dem Parameter \Key{\Environment{abstract}}{markboth} beeinflusst werden. Sollte 
dieser verwendet werden, wird \Key{\Environment{abstract}}{pagestyle}[headings] 
automatisch gesetzt. Mit \Key{\Environment{abstract}}{markboth}[false] werden 
automatische respektive manuelle Kolumnentitel verwendet, die Einstellung 
\Key{\Environment{abstract}}{markboth}[true] setzt diese für linke und rechte 
Seiten auf \Term{abstractname}. Außerdem lässt sich der Kolumnentitel mit 
\Key{\Environment{abstract}}{markboth}[\PName{Kolumnentitel}] auch direkt 
festlegen. So können die Kolumnen beispielsweise mit der Verwendung von
\Key{\Environment{abstract}}{markboth}[\PParameter{}] auch gelöscht werden. 
...
\begin{Declaration}[v2.02]{%
  \Key{\Environment{declarations}}{markboth}[\PBoolean|\PName{Kolumnentitel}]%
}
\begin{Declaration}[v2.02]{%
...
Wurde das Paket \Package{babel} geladen, kann mit dem optionalen Parameter 
\Key{\Environment{declarations}}{language}[\PName{Sprache}] die Sprache 
innerhalb der \Environment{declarations}"=Umgebung geändert werden. Dadurch 
werden die Bezeichner \Term{confirmationname} und \Term{blockingname} sowie
die Trennungsmuster innerhalb der Umgebung sprachspezifisch angepasst. 

\ChangedAt{v2.02}
Der für die Umgebung zu verwendende Seitenstil lässt sich mit dem Parameter 
\Key{\Environment{declarations}}{pagestyle} angegeben. Die Kolumnentitel können 
mit \Key{\Environment{declarations}}{markboth} beeinflusst werden. Wird dieser 
Parameter verwendet, wird \Key{\Environment{declarations}}{pagestyle}[headings] 
automatisch gesetzt. Mit \Key{\Environment{abstract}}{markboth}[true] werden 
für die Kolumnentitel auf linker und rechter Seite auf \Term{confirmationname} 
beziehungsweise \Term{blockingname} verwendet. Außerdem kann der Anwender diese 
mit \Key{\Environment{declarations}}{markboth}[\PName{Kolumnentitel}] auch 
direkt festlegen.
...
\begin{Declaration}[v2.02]{%
  \Key{\Macro{declaration}}{markboth}[\PBoolean|\PName{Kolumnentitel}]%
}
...
\begin{Declaration}[v2.02]{%
  \Key{\Macro{confirmation}}{markboth}[\PBoolean|\PName{Kolumnentitel}]%
}
\begin{Declaration}[v2.02]{%
...
\begin{Declaration}[v2.02]{%
  \Key{\Macro{blocking}}{markboth}[\PBoolean|\PName{Kolumnentitel}]%
}
\begin{Declaration}[v2.02]{%
...
\ToDo[doc]{%
  Inhaltlich passt soweit alles, Formatierung muss noch angefasst werden
}[v2.02]
