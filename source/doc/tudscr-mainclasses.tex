...
\PValue{Light/Regular/Semi-Bold/Bold/Extra-Bold} ausreichend. Anstelle des 
Suffix' \PValue{Italic} ist auch die Nutzung von \PValue{Oblique} oder 
\PValue{Slanted} als Alias für die kursiven Schriftlagen möglich. Es sind weder 
Leerzeichen noch die passende Groß- und Kleinschreibung oder Bindestriche 
notwendig, ebenso ist der Vorsatz \PValue{OpenSans} optional.
%
\begin{center}%
...
\itemval{twoside,balanced}<\marginformat*{20}{30}{20}{30}>
  ...
  Alternativ kann der gewünschte Satzspiegel mit \Option{cdgeometry=custom} 
...
Einstellung erscheinen, so kann eine der folgenden Optionen genutzt werden, um 
dieses individuell anzupassen und die eingestellten Vorgaben für das jeweilige 
...
Die möglichen Wertzuweisungen entsprechen denen der Option \Option{cd=\PMisc}.
...
\itemval{false}
  Bei den \PageStyle{tudheadings}"=Seitenstilen erscheint kein Logo von \DDC.
\itemval*{true}
...
durch \Macro{thesis} und/oder \Macro{subject} sowie der Autor oder 
respektive die Autoren und gegebenenfalls der mit \Macro{publishers} angegebene 
Verlag ausgegeben.
\ChangedAt{v2.02}%
...
\begin{tabular}{llll}
  ...
  \typecast{diss\slc phd}{dissertationname}
...
\Environment{abstract/markboth} folgende Einstellungen:%
\index{Kolumnentitel}%
...
%
\ChangedAt{v2.02}%
Für den Parameter \Environment{declarations/markboth} existieren folgende 
Einstellungen:%
\index{Kolumnentitel}%
...
und tabellarisch aufgeführt. Diese enden auf entweder \Term*{\dots name} oder 
\Term*{\dots text} für einzelne Begriffe respektive beschreibende Texte. Um 
diese oder auch von anderen Paketen bereitgestellte Bezeichner zu ändern, ist 
\Macro{renewcaptionname|\MPName{Sprache}\MPName{Makro}\MPName{Inhalt}} zu 
