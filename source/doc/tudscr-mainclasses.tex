...
\ToDo[doc]{%
  \Macro{footlogo}, \Macro{footlogosep} und \Length{footlogoheight}   
  dokumentieren%
}[v2.03]
...
beispielsweise von kooperierenden Unternehmen oder Sponsoren
...
%auf die Höhe der Erstlogos skaliert. Über das optionale Argument können 
%weitere Formatierungsbefehle an den verwendeten Befehl \Macro{includegraphics} 
%durchgereicht werden, um beispielsweise die Größe des Zweitlogos anzupassen.
%Welche Parameter angepasst werden können, ist der Dokumentation des
%\Package{graphicx}-Paketes zu entnehmen.
%
%Sollte die Option \Option{ddc} aktiviert sein, wird das \DDC-Logo nicht im 
%Kopf sondern automatisch im Fuß gesetzt. Die Option \Option{ddchead} setzt
%dieses auf jeden Fall im Kopf und überschreibt damit das mit \Macro{headlogo} 
%angegebene Zweitlogo.
\end{Declaration}
\end{Declaration}
\end{Declaration}
...
\ToDo[imp]{cdhead}[v2.03]
\ToDo[doc]{cdhead aus barfont und widehead zusammenführen}[v2.03]
\ToDo[doc]{widehead in changedat dazuy}
\begin{Declaration}[v2.03/\Option{barfont}]{\Option{cdhead}[\PSet]}%
  [true][\Option{cdfont}[false]:false]
\printdeclarationlist%
...
\Option{cdfont}'full' sowie \Option{cdhead} und \Option{cdfoot}'page' als auch 
\Option{ddc}, \Option{ddchead} und \Option{ddcfoot}'full'. Das Verhalten sowie 
die jeweils gültigen Wertzuweisungen können in den entsprechenden Abschnitten 
...
\ToDo[imp,nxt]{%
  Titelseite und Cover auf (Drittlogos und) freie Gestaltung mit scrlayer 
  erweitern.
}[v2.x]
...
\begin{Declaration}[v2.02/\Macro{degree}]{%
...
\index{Fußzeile|?}
%
\ToDo[doc]{Option \Option{extrabottommargin} dokumentieren}[v2.03]
\end{Declaration}
...
\begin{Declaration}[v2.02/\Macro{restriction}]{%
  \Macro{blocking}\OLParameter{Firma}%
}
...
\begin{Declaration}[v2.02/\Term{degreetext}]{\Term{graduationtext}}
\printdeclarationlist%
\index{Titel}\index{Abschlussarbeit}\index{Typisierung}%
%
...
\begin{Declaration}[v2.02/\Term{restrictionname}]{\Term{blockingname}}
\printdeclarationlist%
\index{Selbstständigkeitserklärung}\index{Sperrvermerk}%
%
...
\begin{Declaration}[v2.02/\Term{restrictiontext}]{\Term{blockingtext}}
\printdeclarationlist%
%
...
\item[\PValue{v2.02}\textOR\PValue{last}]
  \ChangedAt{%
    v2.03!Satzspiegel geändert{,} insbesondere bei der Verwendung des 
    \protect\DDC-Logos im Fußbereich mit der Option \Option{ddcfoot}
  }
