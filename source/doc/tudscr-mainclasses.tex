...
\autoref{sec:exmpl:mathswap} sind zusätzliche Hinweise zum typographisch guten 
...
\printdeclarationlist%
\label{sec:text}%
\index{Schrift!Fließtext}\index{Schrift!Stärke}%
%
...
Wird das Paket \Package{fontspec} geladen so werden die Schriften des \CDs im 
OpenType"=Format verwendet. Hierfür müssen diese für das Betriebssystem nutzbar 
sein und entweder \hologo{LuaLaTeX} oder \hologo{XeLaTeX} als Dokumentprozessor 
verwendet werden. Allerdings sollte dies nur in Ausnahmefällen geschehen, wobei 
die Hinweise in \fullref{sec:fonts:fontspec} unbedingt zu beachten sind.
\end{Declaration}
...
lokal begrenzt wird~-- global auf das Dokument aus. Bei einem Textkommando 
hingegen erfolgt die Änderung der Schriftart nur für das angegebene Argument. 
Deshalb ist die Nutzung der letzteren Variante vorzuziehen. 
\ChangedAt{v2.04} Alternativ zu den genannten Textschaltern und -befehlen 
können seit der Version~v2.04 auch die beiden Befehle \Macro{cdfont} und 
\Macro{textcdfont} verwendet werden, die sich in der Anwendung etwas 
unterscheiden, allerdings die gleiche Funktionalität bereitstellen.
\end{Declaration}
\end{Declaration}
\end{Declaration}
\end{Declaration}
...
\printdeclarationlist
Diese beiden Befehle können alternativ zu den zuvor beschriebenen verwendet 
werden. Dabei muss im ersten obligatorischen Argument für \PName{Schriftart} 
der Name der zu verwendenden Schriftart des \CDs angegeben werden. Dieser ist 
der Tabelle zu entnehmen, wobei der Vorsatz \emph{Univers} entfallen kann und 
für die Wahl der Schriftstärke entweder die Bezeichnung oder die Zahl allein 
ausreichend ist. Der Befehl \Macro{cdfont} entspricht einem Textschalter und 
ändert die aktuell verwendete Schriftart, wohingegen \Macro{textcdfont} als 
Textkommando fungiert und den im zweiten Argument gegebenen \PName{Text} in der 
gewünschten \PName{Schriftart} setzt.
\end{Declaration}
\end{Declaration}
...
\begin{Declaration}[%
  v2.03!neue Farbvarianten (\PValue{bicolor}{,} \PValue{fullcolor}),%
...
\printdeclarationlist%
\index{Umschlagseite|!}\index{Layout!Umschlagseite}%
%
...
\subsubsection{Verwendung von Schriftelementen}
\index{Schriftelemente}
Für die Schriftauswahl der Überschriften aller Gliederungsebenen sind die durch 
\KOMAScript{} bereitgestellten Schriftelemente verantwortlich. Genaueres hierzu 
ist im \KOMAScript"=Handbuch \scrguide im Abschnitt \emph{Textauszeichnungen} 
zu finden.

Mit \Macro{addtokomafont}\Parameter{Schriftelement}\Parameter{Einstellungen} 
lassen sich sowohl die durch \KOMAScript{} definierten als auch alle in diesem 
Handbuch zusätzlich aufgeführten Schriftelemente individuell durch den Anwender 
anpassen.


\subsubsection{Auszeichnung von Überschriften}
...
\index{Schriftelemente}
%
Die \TUDScript-Klassen definieren das Schriftelement~\Font{parttitle}. Mit 
diesem lässt sich~-- bei aktivierter \Option{parttitle}-Option~-- die Schrift 
für die Bezeichnung des Teils beeinflussen.
\end{Declaration}
...
\Attention{%
  ...
  \Package{scrlayer-scrpage} definiert. Wird es nicht durch den Anwender 
...
sowie \Option{cdfoot} realisiert. 
\end{Declaration}
...
\itemtrue*[light/lightfont/noheavyfont]
  Im Querbalken wird \textubn{Univers~65~Bold} für \Macro{faculty} verwendet, 
  die Felder \Macro{department}, \Macro{institute}, \Macro{chair} und 
  \Macro{extraheadline} werden mit \textuln{Univers~45~Light} gesetzt.
\item[heavy/heavyfont]
...
\begin{values}
\item[barcolor]
  \ChangedAt{v2.04} Im Gegensatz zur vorherigen Einstellung wird der Querbalken 
  mit farbigem Hintergrund verwendet.
\item[bicolor/bichrome]
...
\end{values}
%
Für den Fall, dass der Querbalken lediglich über Außenlinien realisiert wird, 
kann zusätzlich dessen Laufweite festgelegt werden:
%
\begin{values}
\item[textwidth/slim]
...
ändern, indem der Wert der Länge \Length{footlogoheight} auf einen von 
\PValue{0pt} verschiedenen mit \Macro*{setlength} gesetzt wird. Sollte die Höhe 
...
\index{Fußzeile}\index{Layout!Fußzeile}%
%
Mit diesem Befehl kann durch den Anwender beliebiger Inhalt für den Fußbereich 
der \PageStyle{tudheadings}"=Seitenstile definiert werden. Dabei gibt es zwei 
unterschiedliche Varianten. Wird der Befehl mit einem Argument in der Form
\Macro{footcontent}\Parameter{Inhalt} verwendet, so erscheint der Inhalt über 
die komplette Textbreite im Fuß. Soll der Fußbereich zweispaltig erscheinen, so 
muss \emph{nach} dem obligatorischen Argument zusätzlich das optionale folgen. 
In diesem Fall wird der Inhalt aus dem ersten Argument in der linken Fußspalte 
gesetzt, der Inhalt aus dem zweiten entsprechend in der rechten Fußspalte
(\Macro{footcontent}\Parameter{linker Inhalt}\OParameter{rechter Inhalt}).
...
\ToDo[imp,nxt]{%
  ...
  (CD-Handbuch S. 80 ff.) über \Package{scrlayer-scrpage} bereitstellen. 
  Seitenstil: \PageStyle{special.tudheadings}
}[v2.xx]
...
Parameter entsprechen in ihrem Verhalten den gleichnamigen Optionen respektive 
Befehlen, wirken sich jedoch nur lokal und einzig auf die Umschlagseite aus.
...
Felder verwendet, welche kein spezielles Schriftelement verwenden, welches 
ohnehin durch \KOMAScript{} bereitgestellt wird. Das mit \Macro{thesis} 
angegebene Feld, in welchem der Typ einer Abschlussarbeit angegeben wird, nutzt 
das Schriftelement~\Font{thesis}. Mehr dazu ist in \see*{\autoref{sec:title}} 
zu finden. Sowohl die bereits durch \KOMAScript{} definierten als auch alle 
hier 
genannten Schriftelemente lassen sich durch den Anwender über den Befehl 
...
\ToDo[imp,nxt]{%
  Rückseite bei Teile- und Kapitelseiten gestaltbar? Möglichkeit zum Einbinden 
  von Grafiken, einem schlauen Spruch etc.?
...
\end{values}
%
Über den \KOMAScript-Befehl \Macro{renewcaptionname} kann der Bezeichner~-- 
...
\begin{Declaration}[v2.02]{\Macro{nextabstract}\OParameter{Sprache}}
...
\begin{Declaration}[v2.04]{\Macro{nextdeclaration}\OParameter{Sprache}}
...
\index{Selbstständigkeitserklärung}\index{Sperrvermerk}%
%
\ToDo[doc]{\Macro{nextdeclaration}}[v2.04]
...
\ToDo[doc]{%
  Abstände von Schriftgröße abhängig; dies betrifft \Length{smallskipamount}, 
  \Length{medskipamount}, \Length{bigskipamount}, \Length{abovecaptionskip}, 
  \Length{columnsep}
}[v2.04]