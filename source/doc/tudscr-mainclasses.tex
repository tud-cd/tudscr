...
\Option{parskip=\PMisc}(\Bundle{koma-script}) für die Absatzeinstellungen oder 
\Option{BCOR=\PLength}(\Package{typearea}) zur Festlegung der 
...
\begin{Declaration}{\Macro{TUDoptions}[\MPName{Optionenliste}]}
\begin{Declaration}{\Macro{TUDoption}[\MPName{Option}\MPName{Werteliste}]}
\printdeclarationlist[\index{Optionen|!}]%
%
...
beiden \KOMAScript-Befehlen \Macro{KOMAoptions}(\Bundle{koma-script}) 
beziehungsweise \Macro{KOMAoption}(\Bundle{koma-script}).
...
\section{%
  ...
  \ChangedAt*{%
    ...
    v2.06:Nutzung der \texttt{Roboto~Mono} als Schreibmaschinenschrift möglich;%
...
\begin{Declaration}{\Option{cdfont=\PMisc}}[true,normalbold,liningfigures]%
...
\begin{Declaration}[v2.06]{\Option{ttfont=\PMisc}}[true]%
\printdeclarationlist[\index{Schriftart}\index{Schriftstärke}]%
%
...
\Package{roboto} oder \Package{lmodern} genutzt wird und insbesondere die 
...
\itemvaltrue*=roboto,roboto-mono=
  Sollte das Paket \Package{roboto} installiert sein, wird die darin definierte 
...
%
\begin{Declaration}[v2.04]{\Macro{cdfont}[\MPName{Schriftart}]}
\begin{Declaration}[v2.04]{\Macro{textcd}[\MPName{Schriftart}\MPName{Text}]}
\printdeclarationlist
...
\begin{Declaration}{\Option{slantedgreek=\PBoolean}}[true]
\printdeclarationlist[\index{Griechische Lettern|(}]%
%
...
%
\ToDo{InlineDeclaration mit Label ohne Indexeintrag?}[v2.07]
...
Sehr oft kommt hierfür das Paket \Package{amssymb}'CTAN:amsfonts' zum Einsatz. 
Dieses stellt zahlreiche zusätzliche für die \hologo{LaTeX}"=Standardschriften 
zur Verfügung. Diese sind in Verbindung mit der \OpenSans aus typografischer 
Sicht jedoch keine ideale Lösung. Die Symbole aus dem Paket \Package{mdsymbol} 
...
die beiden Längen \Length{abovecaptionskip}(\Bundle{koma-script}) und 
\Length{belowcaptionskip}(\Bundle{koma-script}) für den Abstand zwischen 
einem Gleitobjekt~-- beispielsweise eine Abbildung oder eine Tabelle~-- und 
dessen mit \Macro{caption}(\Bundle{koma-script},\Package{caption}) 
...
\InlineDeclaration{\Option{fontsize=\PLength}(\Bundle{koma-script})} festgelegt 
werden.
\Attention{%
...
%
\begin{DeclarationValues}
\itemvalfalse=absolute,standard=
...
%
\begin{Entity}{\Package{typearea}}
\begin{Declaration}{\Option{DIV=\PMisc}}[default]
\begin{Declaration}{\Option{paper=\PMisc}}[A4]
\begin{Declaration}{\Option{twoside=\PMisc}}%
  [false|\Class{tudscrbook}:true]<true,false,semi>
\begin{Declaration}{\Option{twocolumn=\PBoolean}}[false]
\printdeclarationlist[\index{Satzspiegel}\index{Papierformat}]%
%
Die Einstellung des Satzspiegels erfolgt durch \KOMAScript normalerweise mit 
dem Paket \Package{typearea}(\Bundle{koma-script}), wobei insbesondere die 
Klassenoption \Option{DIV=\PMisc}(\Package{typearea}) maßgeblichen Einfluss auf 
diesen hat. Mit \Option{paper=\PMisc} lässt sich das Papierformat und dessen 
Orientierung festlegen. Mit \Option{twocolumn=\PBoolean} wird das 
Dokument ein- oder zweispaltig gesetzt. Der doppelseitige Satz kann mit 
\Option{twoside=\PMisc} (de-)aktiviert werden. Ist dieser aktiv, sind weiterhin 
...
\begin{Declaration}[v2.03]{\Option{cdgeometry=\PMisc}}%
  [true,restricted|\Option{cd=false}:false]%
\printdeclarationlist[%
  \index{Layout!Seitenstile}\index{Satzspiegel!doppelseitig}%
]%
...
\itemvalfalse
  ...
  \CDs bezüglich der Seitenränder werden ignoriert. \Attention{Nur in diesem 
  Fall wirkt die Option \Option{DIV}(\Package{typearea}).}
\itemvaltrue*=asymmetric,cd=%
...
\itemval=twoside,balanced=%
  ...
  der \KOMAScript-Option \Option{open=left}(\Bundle{koma-script}) immer auf 
...
\itemval=custom=[v2.05]
  ...
  Befehlen \InlineDeclaration{\Macro{geometry}(\Package{geometry})} und 
  \InlineDeclaration{\Macro{newgeometry}(\Package{geometry})} kann der Anwender 
  den Satzspiegel selbst definieren. Für Hinweise zur Verwendung der Befehle 
  sollte die Dokumentation von \Package{geometry} genutzt werden.
\end{DeclarationValues}
\end{Declaration}
...
\InlineDeclaration{\Option{BCOR=\PLength}(\Package{typearea})} zu erwähnen. Mit 
...
beziehungsweise \Option*{BCOR=6mm}(\Package{typearea}) bei diesem Beispiel zu 
wählen.%
\index{Satzspiegel|)}%
\end{Example}
...
\Package{scrlayer-scrpage}(\Bundle{koma-script}) genutzt werden. 
...
\InlineDeclaration{\Option{headinclude=\PBoolean}(\Package{typearea})} sowie 
\InlineDeclaration{\Option{footinclude=\PBoolean}(\Package{typearea})}. Diese 
können~-- unabhängig von der Einstellung zur Satzspiegelgestaltung durch die 
Option \Option{cdgeometry}~-- verwendet werden.

\begin{Declaration}[v2.03]{\Option{extrabottommargin=\PLength}}[0pt]%
\printdeclarationlist%
%
...
%
\begin{Declaration}{\Option{cd=\PMisc}}[true]
\printdeclarationlist%
%
...
\begin{Declaration}[v2.06]{\Option{headings=\PMisc}}[heavy]
\printdeclarationlist[\index{Schriftstärke}]%
%
Die Option \Option{headings=\PMisc} wird bereits von \KOMAScript{} definiert 
...
Gliederungsebenen bis einschließlich \Macro*{subsubsection}.
%
\begin{DeclarationValues}
...
und zum anderen alle Teileseiten (%
  \Macro{part}(\Bundle{koma-script}), \Macro{addpart}(\Bundle{koma-script})%
) und Kapitelseiten (%
  \Macro{chapter}(\Bundle{koma-script}),\Macro{addchap}(\Bundle{koma-script})%
...
\begin{Declaration}{\Option{cdtitle=\PMisc}}[\Option{cd=\PMisc}]<\cdalias>
\printdeclarationlist[\index{Titel|?}]%
%
...
\footnote{%
  ...
  \Macro{uppertitleback}[\MPName{Titelrückseitenkopf}](\Bundle{koma-script}) und
...
\begin{Declaration}[v2.02]{\Option{cdcover=\PMisc}}%
  [true|\Option{cd=false}:false]<\cdalias>
...
\begin{Declaration}{\Option{cdpart=\PMisc}}[\Option{cd=\PMisc}]<\cdalias>
\printdeclarationlist[\index{Layout!Teileseiten|?}]%
%
...
\Macro{part}(\Bundle{koma-script}) und \Macro{addpart}(\Bundle{koma-script}) 
sowie deren Sternversionen verwendet wird. In \autoref{sec:part} sind weitere 
Hinweise zur Teileseite im 
\CD zu finden.
\end{Declaration}

\begin{Declaration}{\Option{cdchapter=\PMisc}}[\Option{cd=\PMisc}]<\cdalias>
\printdeclarationlist[\index{Layout!Kapitelseiten|?}]%
%
...
Verwendung von \Macro{chapter}(\Bundle{koma-script}) beziehungsweise 
\Macro{addchap}(\Bundle{koma-script}) und den dazugehörigen Sternversionen 
genutzt wird. Weitere Hinweise zur Kapitelseite im \CD sind in 
\autoref{sec:chapter} zu finden.
\end{Declaration}

\begin{Declaration}[v2.05]{\Option{cdsection=\PMisc}}%
  [\Option{cd=\PMisc}]<\cdalias>
\printdeclarationlist%
%
Für Überschriften der weiteren Gliederungsebenen
\Macro{section}(\Bundle{koma-script}), 
\Macro{subsection}(\Bundle{koma-script}), 
\Macro{subsubsection}(\Bundle{koma-script}) sowie 
\Macro{paragraph}(\Bundle{koma-script}) und 
\Macro{subparagraph}(\Bundle{koma-script})
werden in der primären Hausfarbe \Color{HKS41} ausgegeben, falls über die 
Option \Option{cd=\PMisc} eine farbige Ausprägung des Layouts eingestellt 
wurde. Mit der Angabe von \Option*{cdsection=true} erscheinen die Überschriften 
...
%
\begin{Declaration}[v2.05]{\Option{pageheadingsvskip=\PLength}}[0pt]%
\begin{Declaration}[v2.05]{\Option{headingsvskip=\PLength}}[0pt]%
\printdeclarationlist[%
...
(\KOMAScript-Option \Option{titlepage=true}(\Bundle{koma-script})) als auch 
...
(\KOMAScript-Option \Option{titlepage=false}(\Bundle{koma-script})) als auch 
...
\begin{Declaration}{\Macro{faculty}[\MPName{Fakultät}]}
\begin{Declaration}{\Macro{department}[\MPName{Einrichtung}]}
\begin{Declaration}{\Macro{institute}[\MPName{Institut}]}
\begin{Declaration}{\Macro{chair}[\MPName{Lehrstuhl}]}
\begin{Declaration}{\Macro{extraheadline}[\MPName{Textzeile}]}
\printdeclarationlist[\index{Layout!Kopfzeile}]%
%
...
\Package{scrlayer-scrpage}(\Bundle{koma-script}) lassen sich entweder einzelne 
Seiten oder auch ganze Dokumente sehr einfach in diesem Stil setzen. Dazu muss 
lediglich mit \Macro{pagestyle}[\MPName{Seitenstil}](\Bundle{koma-script}) 
...
Anwenderschnittstelle von \Package{scrlayer-scrpage}(\Bundle{koma-script}).%
\footnote{%
  \begin{Entity}{\Package{scrlayer-scrpage}}%
...
Sobald mit \Macro{pagestyle}[\MPName{Seitentil}](\Bundle{koma-script}) 
einer der \PageStyle{tudheadings}"=Seitenstile aktiviert wurde, werden die 
Standardseitenstile \PageStyle{headings} beziehungsweise \PageStyle{plain} als 
Alias für \PageStyle{tudheadings} respektive \PageStyle{plain.tudheadings} 
...
entsprechenden \PageStyle{tudheadings}"=Seitenstile.
...
\Macro*{pagestyle}[\MPValue{empty.tudheadings}] manuell aufgerufen werden. 
...
\PageStyle{scrheadings}(\Package{scrlayer-scrpage}) oder 
...
\begin{Declaration}[v2.03]{\Option{cdhead=\PMisc}}[true,nocolor,textwidth]
\printdeclarationlist[%
  \index{Layout!Kopfzeile}%
...
Verwendung des Paketes \Package{scrlayer-scrpage}(\Bundle{koma-script})~-- bei 
...
\itemvalfalse
  ...
  sollte unbedingt das Paket \Package{scrlayer-scrpage}(\Bundle{koma-script}) 
  verwendet werden.
\itemvaltrue*
...
die Option \InlineDeclaration{\Option{manualmark}(\Package{scrlayer-scrpage})} 
nicht explizit angegeben, so werden mit \Option{cdfoot=true} über die 
Option \InlineDeclaration{\Option{automark}(\Package{scrlayer-scrpage})} auch 
...
\printdeclarationlist[\index{Schriftelemente}]%
%
...
\begin{Declaration}{\Macro{headlogo}[\OList\MPName{Dateiname}]}
\printdeclarationlist[%
  \index{Layout!Kopfzeile}%
  \index{Layout!Zweitlogo|?}%
...
werden können, ist der Dokumentation des Paketes \Package{graphicx} zu 
entnehmen.
...
\begin{Declaration}[v2.03]{\Macro{footlogo}[\OList\MPName{Dateinamenliste}]}
...
(\Macro{footlogo}[\MPValue{\PName{Dateiname}:\PName{Parameter}}]) zu übergeben, 
...
des Paketes \Package{graphicx} zu entnehmen.
\end{Declaration}
\end{Declaration}

\begin{Declaration}[v2.05]{\Option{footlogoheight=\PLength}}[0pt]%
\printdeclarationlist%
%
...
von  \Option*{footlogoheight=0pt}'none' werden diese auf die Höhe des Logos der 
...
\printdeclarationlist[\index{Layout!Fußzeile}]%
%
...
\Package{scrlayer-scrpage}(\Bundle{koma-script}) für den Fuß werden nicht 
berücksichtigt, hier kann es zu Überlagerungen der Inhalte kommen. Gleiches 
gilt für die Verwendung der Werte \PValue{true} und \PValue{false} für die 
Option \Option{cdfoot}.
...
]{\Option{ddc=\PMisc}}[false]
\begin{Declaration}[v2.02]{\Option{ddchead=\PMisc}}
\begin{Declaration}[v2.02]{\Option{ddcfoot=\PMisc}}
\printdeclarationlist[%
  \index{Layout!Kopfzeile}%
  \index{Layout!Zweitlogo}%
  \index{Layout!Fußzeile}%
...
angewandte Sprache mit \Key{\Environment{tudpage}}{language=\PSet{Sprache}} 
...
Parameter \Key{\Environment{tudpage}}{columns=\PSet{Anzahl}} der Inhalt der 
gegebenen Anzahl entsprechend mehrspaltig gesetzt. Der gewünschte  
\PageStyle{tudheadings}"=Seitenstil kann mit dem Parameter 
\Key{\Environment{tudpage}}{pagestyle=\PSet{Seitenstil}} angepasst werden, 
wobei \PValue{headings}, \PValue{plain} und \PValue{empty} gültig sind. 
...
\KOMAScript-Option \Option{titlepage}(\Bundle{koma-script}) festgelegt, ob 
...
\begin{Declaration}[%
  ...
  v2.02:Unterstützung der Schriftelemente \Font*{titlepage} und \Font*{thesis}
    sowie der bereits durch \KOMAScript{} bereitgestellten 
    \Font*{titlehead}(\Bundle{koma-script}), 
    \Font*{subject}(\Bundle{koma-script}), 
    \Font*{title}(\Bundle{koma-script}), 
    \Font*{subtitle}(\Bundle{koma-script}), 
    \Font*{author}(\Bundle{koma-script}), 
    \Font*{publishers}(\Bundle{koma-script}), 
    \Font*{date}(\Bundle{koma-script}) und 
    \Font*{dedication}(\Bundle{koma-script})%
]{\Macro{maketitle}[\OPList{Seitenzahl}]}
\begin{Declaration}[v2.02]{%
  \Key{\Macro{maketitle}}{pagenumber=\PSet{Seitenzahl}}%
}
\begin{Declaration}[v2.06]{\Key{\Macro{maketitle}}{cdgeometry=\PMisc}}(%
  \seeref{\Option{cdgeometry}'ppage'}%
)
\begin{Declaration}[v2.02]{\Key{\Macro{maketitle}}{cdfont=\PMisc}}(%
  \seeref{\Option{cdfont}'ppage'}%
)
\begin{Declaration}[v2.03]{\Key{\Macro{maketitle}}{cdhead=\PMisc}}(%
  \seeref{\Option{cdhead}'ppage'}%
)
\begin{Declaration}[v2.03]{\Key{\Macro{maketitle}}{cdfoot=\PMisc}}(%
  \seeref{\Option{cdfoot}'ppage'}%
)
\begin{Declaration}[v2.03]{%
  \Key{\Macro{maketitle}}{headlogo=\PSet{Dateiname}}%
}(\seeref{\Macro{headlogo}'ppage'})
\begin{Declaration}[v2.03]{%
  \Key{\Macro{maketitle}}{footlogo=\PSet{Dateinamenliste}}
}(\seeref{\Macro{footlogo}'ppage'})
\begin{Declaration}[v2.03]{\Key{\Macro{maketitle}}{ddc=\PMisc}}(%
  \seeref{\Option{ddc}'ppage'}%
)
\begin{Declaration}[v2.03]{\Key{\Macro{maketitle}}{ddchead=\PMisc}}(%
  \seeref{\Option{ddchead}'ppage'}%
)
\begin{Declaration}[v2.03]{\Key{\Macro{maketitle}}{ddcfoot=\PMisc}}(%
  \seeref{\Option{ddcfoot}'ppage'}%
)
\printdeclarationlist[%
  \index{Titel|!(}%
...
\footnote{%
  ...
  \Option{open=right}(\Bundle{koma-script}), Standard für \Class{tudscrbook}%
}
...
(\KOMAScript-Option \Option{titlepage=true}(\Bundle{koma-script})) im \TUDCD 
...
Im zweispaltigen Satz (Klassenoption~\Option{twocolumn}(\Package{typearea})) 
wird mit \Macro{maketitle} die Titelseite selbst immer einspaltig gesetzt. 
Direkt nach dem Titel folgt normalerweise der zweispaltige Fließtext. Mit dem 
Befehl \Macro{maketitleonecolumn} kann nach dem Titel zusätzlich weiterer 
...
(\KOMAScript-Option~\Option{titlepage=false}(\Bundle{koma-script})) genutzt, so 
...
(\Option{titlepage=true}(\Bundle{koma-script})) erfolgt die Ausgabe des 
Argumentes \PName{Einspaltentext} direkt nach dieser auf einer oder 
...
(\KOMAScript-Option \Option{titlepage=true}(\Bundle{koma-script})) verwendet 
wird.
\end{Declaration}
...
\begin{Declaration}[%
  ...
  v2.02:Unterstützung der Schriftelemente \Font*{titlepage} und \Font*{thesis}
    ...
    \Font*{title}(\Bundle{koma-script}), 
    \Font*{subtitle}(\Bundle{koma-script}), 
    \Font*{author}(\Bundle{koma-script}) und
    \Font*{publishers}(\Bundle{koma-script})%
]{\Macro{makecover}[\OPList{Seitenzahl}]}
\begin{Declaration}[v2.02]{%
  \Key{\Macro{makecover}}{pagenumber=\PSet{Seitenzahl}}%
}
\begin{Declaration}{\Key{\Macro{makecover}}{cdgeometry=\PBoolean}}
\begin{Declaration}[v2.02]{\Key{\Macro{makecover}}{cdfont=\PMisc}}(%
  \seeref{\Option{cdfont}'ppage'}%
)
\begin{Declaration}[v2.03]{\Key{\Macro{makecover}}{cdhead=\PMisc}}(%
  \seeref{\Option{cdhead}'ppage'}%
)
\begin{Declaration}[v2.03]{\Key{\Macro{makecover}}{cdfoot=\PMisc}}(%
  \seeref{\Option{cdfoot}'ppage'}%
)
\begin{Declaration}[v2.03]{%
  \Key{\Macro{makecover}}{headlogo=\PSet{Dateiname}}%
}(\seeref{\Macro{headlogo}'ppage'})
\begin{Declaration}[v2.03]{%
  \Key{\Macro{makecover}}{footlogo=\PSet{Dateinamenliste}}
}(\seeref{\Macro{footlogo}'ppage'})
\begin{Declaration}[v2.03]{\Key{\Macro{makecover}}{ddc=\PMisc}}(%
  \seeref{\Option{ddc}'ppage'}%
)
\begin{Declaration}[v2.03]{\Key{\Macro{makecover}}{ddchead=\PMisc}}(%
  \seeref{\Option{ddchead}'ppage'}%
)
\begin{Declaration}[v2.03]{\Key{\Macro{makecover}}{ddcfoot=\PMisc}}(%
  \seeref{\Option{ddcfoot}'ppage'}%
)
\printdeclarationlist[\index{Umschlagseite|!(}]%
%
...
\begin{Entity}{\Bundle{koma-script}}
\begin{Declaration}{\Option{titlepage=\PMisc}}%
  [true|\Class{tudscrartcl}:false]<true,false,firstiscover>%
\begin{Declaration}{\Macro{title}[\MPName{Titel}]}
\begin{Declaration}{\Macro{subtitle}[\MPName{Untertitel}]}
\printdeclarationlist[\index{Titel!Felder|(}]%
%
Die Befehle \Macro{title} und \Macro{subtitle} sind selbsterklärend. Anzumerken 
ist, dass die Schriftstärke des Titels von der Option \Option{headings}() 
abhängt und der Untertitel immer im fetten Schnitt erscheint. Die Gestalt des 
Titels (alleinstehende Titelseite oder Titelkopf) wird mit der Option 
\Option{titlepage} gesetzt und ist im \scrguide dokumentiert.
\end{Declaration}
\end{Declaration}
\end{Declaration}
\end{Entity}
...
\begin{Declaration}{\Macro{author}[\MPName{Autor(en)}]}
\begin{Declaration}{\Macro{authormore}[\MPName{Autorenzusatz}]}
\begin{Declaration}[%
  v2.02;%
...
\begin{Declaration}{\Macro{dateofbirth}[\MPName{Geburtsdatum}]}
\begin{Declaration}{\Macro{placeofbirth}[\MPName{Geburtsort}]}
\begin{Declaration}{\Macro{matriculationnumber}[\MPName{Matrikelnummer}]}
\begin{Declaration}{\Macro{matriculationyear}[\MPName{Immatrikulationsjahr}]}
\begin{Declaration}[%
...
Umgebung \Environment{task}(\Package{tudscrsupervisor}) genutzt werden, falls 
...
\printdeclarationlist[\index{Kollaboratives Schreiben|?}]%
%
...
\end{Entity}
aus dem Paket \Package{tudscrsupervisor} nutzen.
\end{Declaration}
%
\begin{Example}
...
\begin{Declaration}[v2.05]{\Macro{date*}[\OPList{Suffix}\MPName{Datum}]}
\begin{Declaration}[v2.05]{%
  \Key{\Macro{date}}{before=\PSet{Präfix}}%
}
\begin{Declaration}[v2.05]{%
  \Key{\Macro{date}}{after=\PSet{Suffix}}%
}
\begin{Declaration}[v2.05]{%
  \Key{\Macro{date}}{place=\PSet{Ort}}%
}
\begin{Declaration}{\Macro{defensedate}[\MPName{Verteidigungsdatum}]}
\printdeclarationlist[%
  \index{Datum|?}%
...
\begin{Declaration}{\Macro{thesis}[\MPName{Typisierung}]}
\begin{Declaration}{\Macro{subject}[\MPName{Typisierung}]}
\printdeclarationlist[%
...
\begin{Declaration}[v2.02]{\Macro{graduation}[\OPName{Kurzform}\MPName{Grad}]}
\printdeclarationlist%
%
...
\begin{Declaration}{\Macro{supervisor}[\MPName{Name(n)}]}
\begin{Declaration}{\Macro{referee}[\MPName{Name(n)}]}
\begin{Declaration}{\Macro{advisor}[\MPName{Name(n)}]}
\begin{Declaration}{\Macro{professor}[\MPName{Name}]}
\printdeclarationlist[%
  \index{Betreuer|?}%
  \index{Gutachter|?}%
  \index{Referent|?}%
...
\begin{Declaration}{\Macro{titledelimiter}[\MPName{Trennzeichen}]}
\printdeclarationlist[\index{Titel!Trennzeichen}]%
%
...
\begin{Entity}{\Bundle{koma-script}}
\begin{Declaration}{\Macro{extratitle}[\MPName{Schmutztitel}]}
\begin{Declaration}{\Macro{frontispiece}[\MPName{Frontispiz}]}
\begin{Declaration}{\Macro{titlehead}[\MPName{Kopf}]}
\begin{Declaration}{\Macro{publishers}[\MPName{Verlag}]}
\begin{Declaration}{\Macro{thanks}[\MPName{Fußnote}]}
\begin{Declaration}{\Macro{uppertitleback}[\MPName{Titelrückseitenkopf}]}
\begin{Declaration}{\Macro{lowertitleback}[\MPName{Titelrückseitenfuß}]}
\begin{Declaration}{\Macro{dedication}[\MPName{Widmung}]}
\printdeclarationlist%
%
...
\ChangedAt[\Macro{frontispiece}](){%
  v2.06:Schmutztitelrückseite wird unterstützt%
}%
...
dieser (\Macro{partpagestyle}(\Bundle{koma-script})) auf 
\PageStyle{plain.tudheadings} gesetzt. Möchten Sie stattdessen einen anderen 
Seitenstil nutzen, so kann dieser mit 
\Macro*{renewcommand*}[%
  \MPValue{\Macro{partpagestyle}(\Bundle{koma-script})}\MPName{Seitenstil}%
]
angepasst werden.

\begin{Declaration}[v2.06]{\Macro{setpartsubtitle}[\MPName{Untertitel}]}
...
Weise wie \Macro{setpartpreamble}(\Bundle{koma-script}) \emph{vor} der 
Verwendung von \Macro{part}(\Bundle{koma-script}) oder den davon abgeleiteten 
...
Bezeichnung des mit \Macro{part}[\MPName{Bezeichnung}](\Bundle{koma-script}) 
...
\itemvalfalse
  ...
  \Macro{setchapterpreamble}(\Bundle{koma-script}) erzeugten Kapitelpräambel 
...
Mit \Macro*{renewcommand*}[%
  \MPValue{\Macro{chapterpagestyle}(\Bundle{koma-script})}\MPName{Seitenstil}%
]
...
\InlineDeclaration{\Option{chapterprefix=\PBoolean}(\Bundle{koma-script})} 
...
\begin{Declaration}[v2.06]{\Macro{setchaptersubtitle}[\MPName{Untertitel}]}
...
Überschrift selbst ein Untertitel gesetzt werden. Der Befehl muss in gleicher 
Weise wie \Macro{setchapterpreamble}(\Bundle{koma-script}) \emph{vor} der 
Verwendung von \Macro{chapter}(\Bundle{koma-script}) oder den davon 
...
\ToDo[doc]{Abschnitt für Absatzeinstellungen (parskip)}
\ToDo[doc]{normale Deklaration (Tabelle) für open und cleardoublepage?}
\subsection{%
  Vakatseiten%
  \label{sec:vacat}%
  \index{Vakatseiten|(}%
...
können nur im doppelseitigem Satz (\Option{twoside=true}(\Package{typearea})) 
vor dem Beginn eines Teils oder Kapitels abhängig von der \KOMAScript-Option 
\InlineDeclaration{\Option{open=\PMisc}(\Bundle{koma-script})}<any,left,right> 
auftreten. Wird ein neuer Teil oder ein neues Kapitel immer auf einer rechten 
Seite erzeugt (\Option{open=right}(\Bundle{koma-script})),%
\footnote{%
  Voreinstellungen
  \Class{tudscrbook}: 
  \Option{twoside=true}(\Package{typearea}), 
  \Option{open=right}(\Bundle{koma-script});
  \Class{tudscrartcl}, \Class{tudscrreprt}: 
  \Option{open=any}(\Bundle{koma-script}), 
...
\InlineDeclaration{\Option{cleardoublepage=\PMisc}(\Bundle{koma-script})} der 
Seitenstil eingestellt werden.

\begin{Declaration}[%
  v2.06:Einstellung für farbige Rückseiten%
]{\Option{cleardoublespecialpage=\PMisc}}[true]%
\printdeclarationlist[%
  \index{Titel}%
  \index{Layout!Teileseiten}%
  \index{Layout!Kapitelseiten}%
...
\Option{cleardoublepage}(\Bundle{koma-script}) immer als vollständig leere 
...
\Option{cleardoublepage}(\Bundle{koma-script}) zu übernehmen. Des Weiteren 
...
\itemvalfalse
  ...
  \Option{cleardoublepage}(\Bundle{koma-script}).
\itemvaltrue*
  Der Seitenstil der Rückseite von Teilen und gegebenenfalls Kapiteln entspricht
  der Einstellung von \Option{cleardoublepage}(\Bundle{koma-script}) für 
  Vakatseiten.
\itemval=current=
  Für die erzeugte Rückseite wird der aktuell definierte Seitenstil 
  (\Macro{pagestyle}(\Bundle{koma-script})) verwendet.
...
](\Bundle{koma-script}) anpassen. Mehr dazu ist im \scrguide innerhalb des 
...
\begin{Declaration}[v2.06]{\Macro{getfield}[\MPName{Feldname}]}
\printdeclarationlist
...
\begin{Declaration}{\Option{abstract=\PMisc}}%
  [false,notoc|\Class{tudscrbook}:chapter,toc]%
\printdeclarationlist[\index{Satzspiegel!zweispaltig}]%
%
Diese Option wird für die Klassen \Class{scrartcl}(\Bundle{koma-script}) 
und \Class{scrreprt}(\Bundle{koma-script}) standardmäßig bereitgestellt. Für 
die Klasse \Class{scrbook}(\Bundle{koma-script}) geschieht dies nicht. Dazu 
...
Ausblick verknüpft. Daher gibt es bei \Class{scrbook}(\Bundle{koma-script}) 
generell keine \Environment{abstract}"~Umgebung. Bei Berichten im weiteren 
...
%
\begin{DeclarationValues}
\itemvalfalse(nur \Class{tudscrartcl} und \Class{tudscrreprt})
  Es wird keine Überschrift für die \Environment{abstract}"~Umgebung ausgegeben.
\itemvaltrue*(nur \Class{tudscrartcl} und \Class{tudscrreprt})
...
\itemval=section,addsec=
  ...
  \Macro{section}(\Bundle{koma-script}).
\itemval=chapter,addchap='\Class{tudscrbook}'(%
    nur \Class{tudscrreprt} und \Class{tudscrbook}%
  )
  Es wird der Befehl \Macro{chapter}(\Bundle{koma-script}) für das Setzen der 
...
\itemval=multiple,multi,all,aggregate=[%
    v2.02:ersetzt \Option{abstract=double}'none'%
  ]
...
\begin{Declaration}{\Macro{nextabstract}[\OPList{Sprache}]}
\begin{Declaration}{\Key{\Environment{abstract}}{language=\PSet{Sprache}}}
\begin{Declaration}[v2.02]{\Key{\Environment{abstract}}{markboth=\PMisc}}
\begin{Declaration}[v2.02]{%
  \Key{\Environment{abstract}}{pagestyle=\PSet{Seitenstil}}%
}
\begin{Declaration}{\Key{\Environment{abstract}}{columns=\PSet{Anzahl}}}
\begin{Declaration}{\Key{\Environment{abstract}}{option=\PMisc}}(%
...
(\KOMAScript-Option \Option{titlepage=false}(\Bundle{koma-script})), so wird 
eine Zusammenfassung~-- identisch zu den \KOMAScript-Klassen~-- in einer 
\Environment{quotation}(\Bundle{koma-script})"~Umgebung ausgegeben, wobei die 
...
\Option{parskip=\PMisc}(\Bundle{koma-script}) nicht beachtet wird. Um dies zu 
beheben, kann das Paket \Package{quoting} geladen werden, wodurch stattdessen 
die Umgebung \Environment{quoting}(\Package{quoting}) Verwendung findet.
...
optionalen Parameter \Key{\Environment{abstract}}{language=\PSet{Sprache}} die 
...
beeinflusst werden. Wird \InlineDeclaration{%
  \Key{\Environment{abstract}}{markboth=false}%
} angegeben, werden automatische respektive manuelle Kolumnentitel verwendet. 
Die Einstellung \InlineDeclaration{\Key{\Environment{abstract}}{markboth=true}} 
wiederum setzt diese für linke und rechte Seiten auf \Term{abstractname}. 
...
der Verwendung von \Key{\Environment{abstract}}{markboth=\MPValue{}} auch 
...
\Key{\Environment{abstract}}{columns=\PSet{Anzahl}} die Zusammenfassung 
...
\Macro{setpartpreamble}(\Bundle{koma-script}) beziehungsweise 
\Macro{setchapterpreamble}(\Bundle{koma-script}) verwendet, so wird die 
...
%
Mit dem \KOMAScript-Befehl \Macro{renewcaptionname}(\Bundle{koma-script})
...
](\Bundle{koma-script}) zu verwenden:%
\begin{Code}[escapechar=§]
...
\begin{Declaration}{\Option{declaration=\PMisc}}%
...
\end{DeclarationValues}
%
Weiterhin kann mit dem Parameter \Key{\Environment{declarations}}{markboth} ein 
...
\Option{titlepage=true}(\Bundle{koma-script}) eine Titelseite verwendet wird.
%
\begin{DeclarationValues}
\itemval=single,one,simple=
  ...
  beziehungsweise im zweispaltigen Satz in einer neuen Spalte ausgegeben.
\itemval=multiple,multi,all,aggregate=[%
    v2.02:ersetzt \Option{declaration=double}'none'%
  ]
...
\begin{Declaration}{%
  \Macro{nextdeclaration}[%
    \OPList{Sprache}\MPName{Überschrift}\MPName{Erklärung}%
  ]%
}
\begin{Declaration}{\Key{\Environment{declarations}}{language=\PSet{Sprache}}}
\begin{Declaration}{\Key{\Environment{declarations}}{markboth=\PMisc}}
\begin{Declaration}{%
  \Key{\Environment{declarations}}{pagestyle=\PSet{Seitenstil}}%
}
\begin{Declaration}{%
  \Key{\Environment{declarations}}{columns=\PSet{Anzahl}}%
}
\begin{Declaration}{\Key{\Environment{declarations}}{option=\PMisc}}
\begin{Declaration}{%
  \Key{\Environment{declarations}}{supporter=\PSet{Unterstützer}}
}
\begin{Declaration}{\Key{\Environment{declarations}}{place=\PSet{Ort}}}
\begin{Declaration}{\Key{\Environment{declarations}}{closing=\PSet{Ende}}}
\begin{Declaration}{\Key{\Environment{declarations}}{company=\PSet{Firma}}}
\printdeclarationlist%
%
...
Jede mit dem Paket \Package{babel} aktivierte Sprache kann mit dem Parameter 
\Key{\Environment{declarations}}{language=\PSet{Sprache}} für die 
\Environment{declarations}"~Umgebung eingestellt werden, wodurch die 
...
beeinflusst werden. Mit \InlineDeclaration{%
  \Key{\Environment{declarations}}{markboth=true}%
} werden für diese auf linker und rechter Seite \Term{confirmationname} 
beziehungsweise \Term{blockingname} verwendet. Durch die Angabe von
\InlineDeclaration{\Key{\Environment{declarations}}{markboth=false}} 
werden je nach gewählter Einstellung für das Paket \Package{scrlayer-scrpage} 
automatische (\Option{automark}(\Package{scrlayer-scrpage})) oder manuelle 
(\Option{manualmark}(\Package{scrlayer-scrpage})) Kolumnentitel genutzt. Mit 
\InlineDeclaration{%
  \Key{\Environment{declarations}}{markboth=\PName{Kolumnentitel}}%
} lassen sich diese auch direkt festlegen.

Wird der Parameter \Key{\Environment{declarations}}{markboth} in irgendeiner 
Form genutzt, wird der Seitenstil \PageStyle{headings} automatisch aktiviert. 
Alternativ kann mit dem Parameter \Key{\Environment{declarations}}{pagestyle}
auch ein beliebiger Seitenstil angegeben werden. Zusammen mit dem Paket 
\Package{multicol} wird der Inhalt der Umgebung durch die Verwendung von 
\Key{\Environment{declarations}}{columns=\PSet{Anzahl}} mehrspaltig gesetzt. 
Alle gültigen Werte der Option \Option{declaration} werden vom Parameter
\Key{\Environment{declarations}}{option} akzeptiert. Die weiteren Parameter 
\Key{\Environment{declarations}}{supporter} sowie 
\Key{\Environment{declarations}}{place} und 
\Key{\Environment{declarations}}{closing} sind in der Dokumentation des Befehls 
\Macro{confirmation} zu finden, der Parameter 
\Key{\Environment{declarations}}{company} ist für \Macro{blocking} erläutert.
\end{Declaration}
\end{Declaration}
\end{Declaration}
\end{Declaration}
...
\begin{Declaration}{\Macro{confirmation}[\OPList{Unterstützer}]}
\begin{Declaration}[v2.05]{\Macro{confirmation*}[\OList]}
\begin{Declaration}{\Key{\Macro{confirmation}}{supporter=\PSet{Unterstützer}}}
\begin{Declaration}{\Key{\Macro{confirmation}}{place=\PSet{Ort}}}
\begin{Declaration}{\Key{\Macro{confirmation}}{closing=\PSet{Ende}}}
\begin{Declaration}{\Key{\Macro{confirmation}}{language=\PSet{Sprache}}}
\begin{Declaration}[v2.02]{\Key{\Macro{confirmation}}{markboth=\PMisc}}
\begin{Declaration}[v2.02]{%
  \Key{\Macro{confirmation}}{pagestyle=\PSet{Seitenstil}}%
}
\begin{Declaration}[v2.02]{%
  \Key{\Macro{confirmation}}{columns=\PSet{Anzahl}}%
}
\begin{Declaration}{\Key{\Macro{confirmation}}{option=\PMisc}}
\printdeclarationlist[\index{Datum}]%
%
...
Parameter \Key{\Macro{confirmation}}{supporter} oder \emph{zuvor} mit dem 
Befehl \Macro{supporter} können weitere an der Arbeit beteiligte Personen 
angegeben werden. Mehrere zu nennende Personen sind auch hier durch \Macro{and} 
...
\begin{Declaration}[v2.02]{\Macro{blocking}[\OPList{Firma}]}
\begin{Declaration}{\Key{\Macro{blocking}}{company=\PSet{Firma}}}
\begin{Declaration}{\Key{\Macro{blocking}}{language=\PSet{Sprache}}}
\begin{Declaration}{\Key{\Macro{blocking}}{markboth=\PMisc}}
\begin{Declaration}{\Key{\Macro{blocking}}{pagestyle=\PSet{Seitenstil}}}
\begin{Declaration}{\Key{\Macro{blocking}}{columns=\PSet{Anzahl}}}
\begin{Declaration}{\Key{\Macro{blocking}}{option=\PMisc}}
\printdeclarationlist%
%
...
\begin{Declaration}{\Macro{declaration}[\OList]}
\begin{Declaration}[v2.05]{\Macro{declaration*}[\OList]}
\begin{Declaration}{\Key{\Macro{declaration}}{language=\PSet{Sprache}}}
\begin{Declaration}[v2.02]{\Key{\Macro{declaration}}{markboth=\PMisc}}
\begin{Declaration}[v2.02]{%
  \Key{\Macro{declaration}}{pagestyle=\PSet{Seitenstil}}%
}
\begin{Declaration}[v2.02]{\Key{\Macro{declaration}}{columns=\PSet{Anzahl}}}
\begin{Declaration}{\Key{\Macro{declaration}}{option=\PMisc}}
\begin{Declaration}{\Key{\Macro{declaration}}{supporter=\PSet{Unterstützer}}}
\begin{Declaration}{\Key{\Macro{declaration}}{place=\PSet{Ort}}}
\begin{Declaration}{\Key{\Macro{declaration}}{closing=\PSet{Ende}}}
\begin{Declaration}{\Key{\Macro{declaration}}{company=\PSet{Firma}}}
\printdeclarationlist%
%
...
\begin{Declaration}{\Macro{supporter}[\MPName{Unterstützer}]}
\begin{Declaration}{\Macro{place}[\MPName{Ort}]}
\begin{Declaration}{\Macro{confirmationclosing}[\MPName{Ende}]}
\begin{Declaration}{\Macro{company}[\MPName{Firma}]}
\printdeclarationlist%
%
...
angegeben.
\TermTable[.78\textwidth]{graduationtext}%
\end{Declaration}
...
\printdeclarationlist[\index{Lesezeichen}]%
%
...
\printdeclarationlist[\index{Zusammenfassung}]%
%
...
\footnote{%
  \Macro{confirmation}[\OPValue{%
    \Key{\Macro{confirmation}}{supporter=\PSet{Unterstützer}}%
...
\footnote{%
  \Macro{blocking}[\OPValue{\Key{\Macro{blocking}}{company=\PSet{Firma}}}]
...
\begin{Declaration}[v2.03]{\Option{tudscrver=\PMisc}}[last]
\printdeclarationlist[\index{Kompatibilität|!}]%
%
...
\itemval=2.02,first=
  ...
  gleiche Satzspiegel genutzt. Mit \Option{tudscrver=2.02} lässt sich dieses 
...
\itemval=2.06=
  ...
  vordefinierte Schriftgrößen (Option \Option{fontsize}(\Bundle{koma-script})) 
  erzielt wird.
\itemval=2.07=
...
%]{\Option{footnotes=\PMisc}}[nosymbolheadings]%
