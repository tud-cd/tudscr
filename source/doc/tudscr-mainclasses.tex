...
]{\Option{cdfont=\PMisc}}[true,normalbold,liningfigures]%
\printdeclarationlist%
%
...
\addtolength\tudscrdim{\dimexpr2\tabcolsep+2\arrayrulewidth-\textwidth\relax}%
\printdeclarationlist(%
...
\begin{Declaration}[%
  v2.05;%
  v2.06f:beachtet sämtliche Schriftgrößen (\Option{fontsize});%
...
]{\Option{cdgeometry=\PMisc}}[true,restricted][\Option{cd=false}:false]%
\printdeclarationlist%
...
\item[\PValueName{Höhe}]
  ...
  Verwendung von Option \Option{extrabottommargin=\PLength}.
\end{values}
\end{Declaration}
...
\ChangedAt[\Macro{frontispiece}(\Package{koma-script})](){%
...
]{\Option{abstract=\PMisc}}[false,notoc][\Class{tudscrbook}:chapter,toc]%
\printdeclarationlist%
\index{Satzspiegel!zweispaltig}%
%
...
]{\Option{declaration=\PMisc}}[true,notoc][\Class{tudscrbook}:chapter,toc]%
\printdeclarationlist%
%
...
Parameter \Key{\Environment{declarations}}{language=\PSet{Sprache}} für die 
...
beziehungsweise\Term{blockingname} verwendet. Automatische 
(\Option{automark}(\Package{scrlayer-scrpage})) respektive manuelle 
(\Option{manualmark}(\Package{scrlayer-scrpage})) Kolumnentitel werden 
mit \InlineDeclaration{\Key{\Environment{declarations}}{markboth=false}} 
genutzt. Mit \InlineDeclaration{%
  \Key{\Environment{declarations}}{markboth=\PName{Kolumnentitel}}%
} können diese direkt festgelegt werden. Wird 
\Key{\Environment{declarations}}{markboth} in irgendeiner Form genutzt, wird 
der Seitenstil \PageStyle{headings} automatisch gesetzt. Dieser lässt sich für 
die Umgebung mit dem Parameter \Key{\Environment{declarations}}{pagestyle} auch 
manuell angegeben. Wurde das Paket \Package{multicol} geladen, wird der Inhalt 
der Umgebung mit \Key{\Environment{declarations}}{columns=\PSet{Anzahl}} 
mehrspaltig gesetzt. Für \Key{\Environment{declarations}}{option} können alle 
gültigen Werte der Option \Option{declaration} angegeben werden. Die Verwendung 
der weiteren Parameter \Key{\Macro{confirmation}}{supporter} sowie 
