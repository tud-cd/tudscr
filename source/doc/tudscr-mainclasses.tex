...
Sollte kein Wert angegeben werden~-- es wird einfach die Form \PName{Option} 
verwendet~--, so wird dieser Option der gegebenenfalls vorgesehene Säumniswert 
zugewiesen.
...
sowie das Setzen von Überschriften vor, was in der Standardkonfiguration durch 
\TUDScript auch so umgesetzt wird. Da jedoch in längeren Texten die Verwendung 
von Serifenschriften zu empfehlen ist, gibt es jedoch die Möglichkeit, die 
...
Paket \Package{mdsymbol}. Diese Einstellung lässt sich deaktivieren, wodurch 
...
\minisec{Die Schriftformate PostScript und OpenType}
...
%
Das \TUDScript-Bundle unterstützt mit die Nutzung der Schriften des \CDs sowohl 
im PostScript- als auch im OpenType"=Format. Beide Formate werden durch das
Paket \Package{opensans} bereitgestellt, wobei unabhängig vom genutzten 
...
\Engine{LuaLaTeX} oder \Engine{XeLaTeX} nicht zwingend erforderlich. Dennoch 
ist dies problemlos möglich, indem einfach das Paket \Package{fontspec} 
eingebunden wird.
...
den klassischen Prozesspfad via \Path{latex > dvips > ps2pdf}~-- wie es 
beispielsweise für die Erstellung von Grafiken mit \Package{pstricks} notwendig 
ist~-- erforderlich. Andererseits werden die mathematische Glyphen den 
...
\end{values}
%
Weiterhin lässt sich die Gestalt der Ziffern anpassen, welche für die \OpenSans 
immer dicktengleich (äquidistant) sind. Es kann zwischen Versalziffern und 
Mediävalziffern gewählt werden.
%
\begin{values}{\Option{cdfont}}
\item[liningfigures/normalfigures]
  \ChangedAt{v2.06}
  Mit dieser Einstellung kommen Versalziffern oder auch Majuskelziffern zum 
  Einsatz. Diese ist standardmäßig aktiviert.
\item[oldstylefigures/osf/oldnumbers]
  \ChangedAt{v2.06}
  Es werden Mediävalziffern oder auch Minuskelziffern verwendet. 
\end{values}
%
...
\item[upgreek/uprightgreek]
  Griechische Groß- und Kleinbuchstaben werden aufrecht gesetzt, 
...
\begin{Declaration}[v2.06]{\Option{ttfont=\PSet}}[false]%
\printdeclarationlist%
\ToDo[doc]{Option \Option*{ttfont}}[v2.06]
\end{Declaration}
...
Schriftart angegeben werden. Dieser ist der folgenden Tabelle zu entnehmen. Für 
...
\ToDo{neue Schalter dokumentieren}[v2.06]
...
\ToDo[doc]{standardgreek rein}[v2.06]
\ToDo[doc]{hinweis zu mdsymbol}[v2.06]
%
...
%
\ToDo[doc]{\Macro*{var\dots} raus, \Macro*{it\dots} rein}[v2.06]
\ToDo[doc]{alle Befehle in eine Tabelle, keine explizite Deklaration}[v2.06]
\begin{Declaration}{\Macro{varGamma}}
...
\begin{Declaration}{\Macro{upGamma}}
...
\index{Layout|(}%
\ToDo[doc]{Erweiterung der Option \Option{headings}}[v2.06]
...
\footnote{%
  ...
  \Macro{frontispiece}[\Parameter{Frontispiz}](\Package{koma-script}),
...
\ToDo[doc]{%
  \Key*{\Macro*{maketitle}}{cd=\PSet}, \Key*{\Macro*{maketitle}}{cdfont=\PSet} 
  und \Key*{\Macro*{maketitle}}{cdgeometry=\PSet} dokumentieren%
}[v2.06]
\ToDo[doc]{\Macro*{raggedtitle} dokumentieren}[v2.06]
\begin{Declaration}[%
...
\begin{Declaration}{\Option{titlesignature=\PBoolean}}[false]%
\printdeclarationlist%
%
\ToDo[doc]{Dokumentation von \Option{titlesignature}}[v2.06]
\index{Titel!Felder|)}%
\end{Declaration}

\begin{Bundle}{\Package{koma-script}}
\begin{Declaration}{\Macro{extratitle}[\Parameter{Schmutztitel}]}
\begin{Declaration}[v2.06]{\Macro{frontispiece}[\Parameter{Frontispiz}]}
...
\index{Schriftelemente}%
%
\ToDo[doc]{Befehl \Macro{setpartsubtitle} dokumentieren}[v2.06]%
Mit dem Schriftelement~\Font{partsubtitle} lässt sich die Schrift für den 
...
%
\ToDo[doc]{%
  \Macro{setchaptersubtitle} und \Font{chaptersubtitle} dokumentieren%
}[v2.06]%
Mit dem Schriftelement~\Font{chaptersubtitle} lässt sich die Schrift für den 
gegebenen Untertitel verändern. In \autoref{sec:fonts:elements} ist zu finden, 
wie es angepasst werden kann.
\end{Declaration}
\end{Declaration}
...
\begin{Declaration}[%
  v2.06:Einstellung für farbige Rückseiten integriert%
...
Farbe wie die Vorderseite von Titel, Teil oder Kapitel gesetzt werden. 
\notudscrartcl
%
...
und \Font{chaptersubtitle}. Sowohl die bereits durch \KOMAScript{} definierten
als auch alle hier genannten und folgend erläuterten Schriftelemente sollten im
Bedarfsfall vom Anwender über den Befehl \Macro{addtokomafont}[%
...
afterdate
defensedate
\begin{Declaration}{\Macro{getfield}[\Parameter{Feldname}]}
\printdeclarationlist
\end{Declaration}
...
\begingroup
  ...
  \ToDo[doc]{auf \Macro*{getfield} verweisen}[v2.06]
  \noindent
