...
\item[\PValue{2.03}]
  ...
  Mit der Version~v2.04 werden mehrere Längen in Abhängigkeit der über die 
  Option \Option{fontsize} gewählten Schriftgröße definiert, was durch die 
  \KOMAScript-Klassen \emph{nicht} standardmäßig geschieht. Dies betrifft 
  sowohl die dehnbaren Längen \Length{smallskipamount}, \Length{medskipamount} 
  und \Length{bigskipamount}, die von den Befehlen \Macro{smallskip}, 
  ...
  \Length{belowcaptionskip} für den Abstand zwischen Gleitobjekt und dessen 
  Beschreibung sowie \Length{columnsep} als Maß für den Abstand der einzelnen 
  Textspalten im zwei- oder mehrspaltigen Layout. Mit \Option{tudscrver}[2.03] 
  kann dieses Verhalten deaktiviert werden.
\item[\PValue{2.04}]
  Dies ist Kompatibilitätseinstellung für \TUDScript in der Version~\vTUDScript 
  und wird für spätere Änderungen bereits vorgehalten. Soll ein Dokument auch 
  mit einer späteren Version von \TUDScript nach einem \hologo{LaTeXe}-Lauf das 
  gleiche Ausgabeergebnis erzeugt werden wie mit der aktuellen Version, muss 
  dies mit der Klassenoption \Option{tudscrver} angegeben werden.
\item[\PValue{last}]
