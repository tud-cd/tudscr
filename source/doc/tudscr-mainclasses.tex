...
\Option{fontsize=\PLength}(\Bundle{koma-script}) festgelegt werden.
\Attention{%
...
\Option{DIV=\PMisc}(\Package{typearea})|declare| maßgeblichen Einfluss auf den 
...
\Option{BCOR=\PLength}(\Package{typearea})|declare| zu erwähnen. Mit dieser 
...
Option \Option{BCOR}(\Package{typearea}) nutzen kann. So lässt sich eine 
Bindekorrektur von beispielsweise \unit[5]{mm} mit der \emph{Klassenoption} 
\Option*{BCOR=5mm}(\Package{typearea}) festlegen.
...
sein. Alternativ kann auch die Option \Option{cdfoot} mit einem Längenwert 
verwendet werden.
\end{Declaration}
...
Überschriften anpassen zu können.
%
\begin{DeclarationValues}
...
]{\Option{cdtitle=\PMisc}}[\Option{cd=\PMisc}]<\cdalias>
\printdeclarationlist%
\index{Titel|?}%
%
...
]{\Option{cdcover=\PMisc}}[true][\Option{cd=false}:false]<\cdalias>
\printdeclarationlist%
\index{Umschlagseite|?}%
%
...
]{\Option{cdpart=\PMisc}}[\Option{cd=\PMisc}]<\cdalias>
\printdeclarationlist%
...
]{\Option{cdchapter=\PMisc}}[\Option{cd=\PMisc}]<\cdalias>
\printdeclarationlist%
...
\KOMAScript-Option 
\Option{titlepage=\PBoolean}(\Bundle{koma-script})|declare| festgelegt, ob 
...
\Option{chapterprefix=\PBoolean}(\Bundle{koma-script})|declare| 
...
\KOMAScript-Optionen \Option{twoside=\PMisc}(\Package{typearea})|declare| 
und \Option{open=\PMisc}(\Bundle{koma-script})|declare| am Beginn von Teilen 
...
\Option{cleardoublepage=\PMisc}(\Bundle{koma-script})|declare| eingestellt 
werden.
...
\begin{Code}[escapechar=§]
\AtBeginDocument{%
  \addtokomafont§\Parameter{Schriftelement}\Parameter{Einstellungen}§%
}
\end{Code}
\end{quoting}
