...
\end{itemize}
%
Für \Application{\hologo{TeX}studio} wird über GitHub das Archiv 
...
Sämtliche Probleme von \Package{auto-pst-pdf} in Kombination mit anderen 
Paketen liegt an der Tatsache, dass dieses Paket automatisiert des aktuelle 
Dokument durchläuft und nach dem Lesen der Präambel lediglich den Inhalt der 
vorhandene \Environment{pspicture}"=Umgebungen über den PostScript-Pfad
\Path{latex \textrightarrow{} dvips \textrightarrow{} ps2pdf} kompiliert und  
PDF-Grafiken erstellt.

Mit dem Paket \Package{ifpdf} kann erkannt werden, welcher Dokumentprozessor 
gerade das Dokument kompiliert. Mit dem Makro \Macro{ifpdf} können abhängig 
davon unterschiedliche Quelltexte ausgeführt werden, was im Folgenden genutzt 
...
ausgegeben. In Kombination mit \Package{auto-pst-pdf} führt dies zu einer 
...
Diesen Befehle können bis zu drei optionale Argumente \emph{vor} den beiden 
obligatorischen übergeben werden. Dieses Verhalten ist für die standardmäßige 
Benutzerschnittstelle für die (Re-)Definition durch \hologo{LaTeXe} nicht 
vorgesehen. Deshalb wird für dieses Unterfangen das Paket \Package{xparse} 
...
Mit \Package{tikz}(pgf) und respektive auch \Package{todonotes}~-- welches 
das zuvor genannte nutzt~-- gibt es in Verbindung mit \Package{auto-pst-pdf} 
ebenfalls Probleme. Dies lässt sich jedoch einfach lösen, indem man diese 
Pakete lediglich in der Präambel einbindet, wenn \hologo{pdfLaTeX} als 
Dokumentprozessor aktiv ist.
%
\begin{Code}
\ifpdf
  \usepackage{tikz}
...
\section{Worttrennungen in nicht-englischsprachigen Texten}
...
\Package{hyphsubst} muss bereits \emph{vor} den Dokumentklassen selbst geladen 
werden. Außerdem wird das Paket \Package{babel} benötigt. Damit auch Wörter mit 
Umlauten richtig getrennt werden, ist außerdem die Verwendung des Paketes 
\Package{fontenc} mit der \PValue{T1}"~Schriftkodierung notwendig. Der Beginn 
...
Eine kurze Anmerkung zu \Macro{defaulthyphenchar}. Normalerweise sind die von 
\hologo{LaTeXe} verwendeten Zeichen für Bindestrich und Trennstrich identisch. 
Das führt wiederum
dazu, dass der Trennungsalgorithmus von \hologo{LaTeXe} bei Wörtern, welche 
bereits einen Bindestrich enthalten, außer Kraft gesetzt wird. In der Folge 
werden Wörter wie \enquote{Donaudampfschifffahrts-Gesellschafterversammlung} 
normalerweise nur direkt nach dem angegebenen Bindestrich getrennt. Im 
Normalfall ist der Trennstrich auf \Macro{defaulthyphenchar}\PValue{=45} 
gesetzt. Wird die \PValue{T1}"~Schriftkodierung verwendet, befindet sich auf 
der Position \PValue{127} glücklicherweise für gewöhnlich das gleiche Zeichen 
noch einmal. Dies ist jedoch von der verwendeten Schrift abhängig.

Sollte trotz aller Maßnahmen denoch einmal ein bestimmtes Wort falsch getrennt 
