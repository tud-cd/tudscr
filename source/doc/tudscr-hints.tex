...
\ToDo[doc,nxt]{Schnittmarken mit \Package{crop} dokumentieren}[v2.04]
...
System bekannt sein.%
\footnote{%
  Genauer gesagt, muss der Pfad zu \File{inkscape.exe} in der Umgebungsvariable 
  \Path{PATH} des Betriebssystems enthalten sein.
}
...
\ToDo[doc]{Vermeiden des Skalierens einer PDF-Datei beim Druck}[v2.04]
\url{http://www.komascript.de/node/1897}
% Einstellung der Skalierung der Druckausgabe abhängig vom PDF-Betrachter
% um Skalierung zu unterdrücken gibt es zwei Varianten
% 1.
% \usepackage{hyperref}
% \hypersetup{pdfprintscaling=AppDefault}
% 2.
% \pdfcatalog{/ViewerPreferences<</PrintScaling/None>>} 
\ToDo[doc]{Verwendung des Paketes \Package{fix-cm} erläutern}[v2.04]
% Hinweis auf Option fontsize, \index{Schriftgröße|!}
