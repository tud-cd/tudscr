\ToDo[doc]{Stern bei printdeclarationlist entfernen?}[v2.05]
...
sequentiell geprüft, ob das Makro \bsc\PName{Element}\PValue{autorefname}
oder \bsc\PName{Element}\PValue{name} existiert. Soll die Bezeichnung eines 
...
\PValue{disposition} durch das dazugehörige Schriftelement zu ersetzen.
...
möglich. Nach dem Laden des Paketes, kann das Rücksetzen des Zählers nach einem 
Kapitel mit \Macro{counterwithout*}\PParameter{footnote}\PParameter{chapter} 
deaktiviert werden. Auch andere \hologo{LaTeX}-Zähler~-- beispielsweise der 
...
\Option{paper=\PSet}|declare| sowohl möglich, lediglich die Ausrichtung in ein 
...
Optionen via \Macro*{usepackage}\OParameter{Option}\Parameter{Paket} folgender 
...
\Package{floatrow}-Befehle zusammen mit der \Environment{pspicture}"=Umgebung 
...
indem die fraglichen Pakete lediglich geladen werden, wenn \hologo{pdfLaTeX} 
aktiv ist.
%
\begin{quoting}[rightmargin=0pt]
\begin{Code}
