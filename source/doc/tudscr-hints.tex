...
\ToDo[doc]{%
  Schnittmarken mit \Package*{crop} bzw. \Package*{geometry} dokumentieren%
}[v2.05]
...
\ChangedAt{%
  v2.05:\TaT\Macro*{includesvg} Bugfix für \Application{Inkscape} Version~0.91; 
  v2.05:\TaT\Macro*{includesvg} in Funktionalität erweitert%
}
Definition von \Macro{includesvg}[\OParameter{Breite}\Parameter{Datei}] 
in der Präambel des Dokumentes wie folgt möglich:
\ToDo[imp]{Bugfix in \Package{svg} oder in tudscr oder eigenes Paket?}[v2.05]
\ToDo[doc]{erweiterte Definition bzw. Paket \Package{svg} dokumentieren}[v2.05]
%\usepackage{filemod}
