...
\Class{tudmathposter} basierende Dokumente zu überführen. In jedem Fall sollte 
bei einem Umstieg auf \Class{tudscrposter} beachtet werden, dass für diese 
Klasse eine explizite Wahl der Schriftgröße über die 
Option~\Option'none'(\Bundle{koma-script}){fontsize} notwendig ist. Um 
...
\Macro*'none'(\Package{geometry}){geometry/paper=\PName{Papierformat}},
\Macro*'none'(\Package{geometry}){geometry/layout=\PName{Zielformat}} und 
\Macro*'none'(\Package{geometry}){geometry/layoutoffset=\PName{Längenwert}} des
Befehls \Macro*'none'(\Package{geometry}){geometry} aus dem Paket 
