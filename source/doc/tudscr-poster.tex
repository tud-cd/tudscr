...
dieser wird das Erstellen von Postern im gleichen Layout mit \Logo{LaTeX} 
...
\Option{paper=\PMisc} zu verwenden. Dabei lassen sich mit 
\Option{paper=\PName{Format}} unter anderem die gängigen Klassen der 
ISO/DIN"~Reihen A~bis~D als auch Quer- oder Längsformat auswählen. Ein 
beliebiges Format kann mit der Einstellung \Option{paper=\PName{Höhe:Breite}} 
...
\Option{fontsize=\PLength} angegeben werden. Für eine passend abgestimmte 
...
\end{tabular}
    ...
    \caption{%
      Empfohlene Kombinationen für die Wahl von Papierformat (\Option{paper}) 
      und Schriftgröße (\Option{fontsize})%
...
\itemval=\PName{Farbe}=
  ...
  \Macro{color|\MPName{Farbe}} verwendet wird.
\end{DeclareValues}
\end{Declaration}
...
Beschriftung den Befehl \Macro{captionof|\MPName{Typ}\MPName{Beschriftung}}
...
Verwendung bei der Beschreibung von \Macro||'full'(\Bundle{tudscr}){author} zu 
finden. Vor der Ausgabe aller Autoreninformationen wird der Bezeichner 
\Term{authorname} in fetter Schrift gesetzt. 
