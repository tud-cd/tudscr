...
\Macro{subtitle}, \Macro{subject} und \Macro(\Bundle{koma-script}){titlehead} 
...
Befehle für den Schmutztitel \Macro(\Bundle{koma-script}){extratitle} sowie 
\Macro(\Bundle{koma-script}){frontispiece} nutzen, wobei deren Verwendung für 
...
\Macro*{date}, \Macro*(\Bundle{koma-script}){publishers} und 
\Macro*(\Bundle{koma-script}){dedication} sowie 
\Macro*(\Bundle{koma-script}){uppertitleback} und 
\Macro*(\Bundle{koma-script}){lowertitleback} haben bei der Klasse 
...
\Environment'none'(\Package{multicol}){multicols}-Umgebung aus dem Paket 
\Package{multicol} sehr empfehlenswert. 

Zur Festlegung des Papierformats ist die \KOMAScript-Option 
\Option|declare|'none'(\Package{typearea}){paper=\PMisc} zu verwenden. Dabei 
lassen sich mit \Option'none'(\Package{typearea}){paper=\PSet{Format}} 
...
\Option'none'(\Package{typearea}){paper=\PSet{Höhe}{:}\PSet{Breite}}
...
\Option|declare|'none'(\Bundle{koma-script}){fontsize=\PName{Schriftgröße}}
...
\end{tabular}
    ...
    \caption{%
      ...
      (\Option'none'(\Package{typearea}){paper}) und Schriftgröße 
      (\Option'none'(\Bundle{koma-script}){fontsize})%
...
Äquivalent zu den \TUDScript"=Hauptklassen wird mit dieser Option die 
Verwendung des \TUDCDs für das Poster festgelegt. Sie hat Einfluss auf die 
Farbgestaltung der Gliederungsüberschriften sowie des Seitenstils, welcher 
standardmäßig auf \PageStyle{empty.tudheadings} gesetzt wird.
...
\begin{Declaration}
  {\Option{bleedmargin=\PName{Längenwert}}}
  (0.2in)
...
Einstellungen an \Macro'none'(\Package{hyperref}){hypersetup} aus besagtem 
