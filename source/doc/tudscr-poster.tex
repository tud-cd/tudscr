\chapter[Die Posterklasse \ClassRaw{tudscrposter}{\BooleanFalse}]{%
  Die Posterklasse%
  \tudmarkuplabel[sec:poster]{\Class{tudscrposter}}%
  \index{Posterklasse|!}%
}
%
\begin{Entity*}[%
...
\Macro{title}, \Macro{subtitle}, \Macro{subject}() und \Macro{titlehead}
...
Befehle für den Schmutztitel \Macro{extratitle} sowie \Macro{frontispiece} 
...
die Option \Option*{subjectthesis} \emph{nicht} zur Verfügung.
\end{Entity}
Die von \KOMAScript{} für eine Titelseite bereitgestellten Befehle 
\begin{Entity}{\Bundle{koma-script}}
\Macro*{publishers} und \Macro*{dedication} sowie \Macro*{uppertitleback} und 
\Macro*{lowertitleback} 
\end{Entity}
haben für \Class{tudscrposter} keinerlei Funktionalität. Die beiden Befehle 
\Macro{author} und \Macro{authormore} existieren weiterhin, werden allerdings 
...
Umgebung \Environment*{declarations} als auch die Befehle \Macro*{declaration}, 
\Macro*{confirmation} und \Macro*{blocking}. Für eine mögliche \abstractname{} 
kann die Umgebung \Environment{abstract} weiterhin genutzt werden, allerdings 
kann mit der Option \Option{abstract} lediglich noch die Gliederungsebene der 
...
\Environment{multicols}(\Package{multicol})"~Umgebung aus dem Paket 
\Package{multicol} sehr empfehlenswert. 

Zur Festlegung des Papierformats ist die \KOMAScript-Option 
\Option{paper=\PMisc}(\Package{typearea}) zu verwenden. Dabei lassen 
sich mit \Option{paper=\PName{Format}}(\Package{typearea}) unter anderem 
...
\Option{paper=\PName{Höhe:Breite}}(\Package{typearea}) gewählt werden. Für 
...
\Option{fontsize=\PLength}(\Bundle{koma-script}) angegeben werden. Für eine 
...
\end{tabular}
    ...
    \caption{%
      ...
      (\Option{paper}(\Package{typearea})) und Schriftgröße 
      (\Option{fontsize}(\Bundle{koma-script}))%
...
\begin{Declaration}{\Option{cd=\PMisc}}[bicolor]
...
\itemval=\PName{Farbe}=
  ...
  \Macro{color}[\MPName{Farbe}](\Package{xcolor}) verwendet wird.
\end{DeclarationValues}
\end{Declaration}

\begin{Declaration}{\Option{bleedmargin=\PLength}}[0.2in]
\printdeclarationlist[\index{Beschnittzugabe|?}\index{Schnittmarken|?}]
%
...
%
\begin{Declaration}[v2.07]{\Environment{figurehere}}
\begin{Declaration}[v2.07]{\Environment{tablehere}}
\printdeclarationlist%
%
...
\Macro{captionof}[\MPName{Typ}\MPName{Beschriftung}](\Bundle{koma-script}) 
...
\begin{Declaration}{\Macro{faculty}[\OPName{Fußzeile}\MPName{Fakultät}]}
\begin{Declaration}{\Macro{department}[\OPName{Fußzeile}\MPName{Einrichtung}]}
\begin{Declaration}{\Macro{institute}[\OPName{Fußzeile}\MPName{Institut}]}
\begin{Declaration}{\Macro{chair}[\OPName{Fußzeile}\MPName{Lehrstuhl}]}
\printdeclarationlist%
%
...
\begin{Declaration'}{\Macro{emailaddress*}[\MPName{E-Mail-Adresse}]}
\begin{Declaration'}{%
  \Macro{emailaddress}[\OPName{Einstellungen}\MPName{E-Mail-Adresse}]%
}
\begin{Declaration'}{\Macro{course}[\MPName{Studiengang}]}
\begin{Declaration'}{\Macro{discipline}[\MPName{Studienrichtung}]}
\begin{Declaration}{\Macro{contactperson}[\MPName{Name(n)}]}
\begin{Declaration}{\Macro{telephone}[\MPName{Telefonnummer}]}
\begin{Declaration}{\Macro{telefax}[\MPName{Telefaxnummer}]}
\begin{Declaration}{\Macro{office}[\MPName{Adresse/Gebäude}]}
\printdeclarationlist%
%
...
Verwendung bei der Beschreibung von \Macro{author}'full' zu finden. Vor der 
...
\begin{Declaration}{\Macro{webpage}[\OPName{Einstellungen}\MPName{URL}]}
\begin{Declaration}{\Macro{webpage*}[\MPName{URL}]}
\printdeclarationlist%
%
...
Einstellungen an \Macro{hypersetup}(\Package{hyperref}) aus besagtem Paket 
