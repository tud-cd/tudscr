...
\ChangedAt{%
  ...
  v2.05:Neues Schriftpaket \Package{tudscrfonts-fix} für Dokumentklassen 
...
\section{Das Paket \Package{tudscrfonts-fix} -- Schriftkompatibilität}
\begin{Bundle*}[v2.05]{\Package{tudscrfonts-fix}}
\printchangedatlist%
%
...
\end{enumerate}
%
Wird nun das Paket \Package{tudscrfonts-fix} einer der zuvor genannten Klassen 
verwendet, hat dies den Vorteil, dass auch in diesen das angepasste Kerning der 
Schriften sowie der stark verbesserte Mathematiksatz zum Tragen kommen. 
Außerdem kann bei der Verwendung von \Package{tudscrfonts-fix} auf eine 
...
\Attention{%
  ...
  \Package{tudscrfonts-fix} können die alten Schriftfamilien auch parallel zu 
...
zu können, muss das Paket \Package{tudscrfonts-fix} bereits \emph{vor} der 
Dokumentklasse geladen werden:
%
\begin{Code}[escapechar=§]
\RequirePackage{tudscrfonts-fix}
\documentclass{tudbook}
§\dots§
\begin{document}
§\dots§
\end{document}
