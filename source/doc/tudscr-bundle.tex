...
auf eine abermalige Erläuterung der im Paket verfügbaren Optionen und Befehle 
verzichtet. Diese werden im Folgenden lediglich noch einmal genannt, die 
dazugehörigen Erläuterungen sind in \fullref{sec:fonts} zu finden.
...
Weiterhin wird von \Package{tudscrfonts} das Paket \Package{textcase} geladen, 
welches die Befehle \Macro{MakeTextUppercase}(\Package{textcase})'none' und 
\Macro{NoCaseChange}(\Package{textcase}) zur Verfügung stellt. Der Befehl 
\Macro{ifdin} wird ebenso wie die in \autoref{sec:fonts} beschriebenen 
Textschalter und "~kommandos zur expliziten Auswahl einzelnen Schnitte der 
Hausschriften sowie die Befehle für griechische Buchstaben bereitgestellt.



\ToDo[doc]{Text anpassen bzw. überarbeiten}[v2.05]
\ChangedAt{v2.05:\Package{tudscrfonts-fix} Schriften für alternative 
Dokumentklassen im \CD der \TnUD}
...
diese Klassen das Paket \Package{tudscrfonts-fix}'full' zu verwenden.
\ChangedAt{%
...
\Macro{textdbn} und \Macro{dinbn} sowie den Befehl 
\Macro{MakeTextUppercase}(\Package{textcase})'none' zur automatisierten 
...
\ToDo[doc]{%
  Dokumentation von \Package{tudscrfonts-fix}
}[v2.05]
\ToDo[doc]{%
  Dokumentation aller möglichen Optionen und Befehle direkt beim Paket
  \Package{tudscrfonts}, wenn auch nur kurz.
}[v2.05]
\end{Bundle*}
