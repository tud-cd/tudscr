...
Dies wird hier exemplarisch mit der folgenden \colorbox{HKS44!20}{%
  Box \Macro{colorbox}[\MPValue{HKS44!20}\MPValue{Box}](\Package{xcolor})%
}
demonstriert.
\end{Example*}
%
...
\begin{Declaration}
  {\Color{HKS41}}
  (Alternativ:\Color*{cddarkblue}'none')
...
\begin{Declaration}
  {\Color{HKS36}}
  (Alternativ:\Color*{cdindigo}'none')
\begin{Declaration}
  {\Color{HKS33}}
  (Alternativ:\Color*{cdpurple}'none')
\begin{Declaration}
  {\Color{HKS57}}
  (Alternativ:\Color*{cddarkgreen}'none')
\begin{Declaration}
  {\Color{HKS65}}
  (Alternativ:\Color*{cdgreen}'none')
\printdeclarationlist
\cdcolor[4]{36}
\cdcolor[4]{33}
\cdcolor[2]{57}
...
\begin{Declaration}
  {\Color{HKS07}}
  (Alternativ:\Color*{cdorange}'none')
\printdeclarationlist
\cdcolor{07}
\end{Declaration}
\index{Layout!Farbgestaltung|?)}%
...
Der \Lettering{LaTeX}"~Kernel enthält einen Fehler, der Kapitelüberschriften im
...
\Lettering{LaTeX}"~Kernel übernommen worden. Das Paket \Package{twocolfix} behebt 
das Problem. Eine Integration dieses Bugfixes in \KOMAScript wurde bereits bei 
