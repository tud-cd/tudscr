\setchapterpreamble{%
  \begin{abstract}
    ...
    angewiesen sondern können prinzipiell mit jeder \Logo{LaTeX}"~Klasse 
    genutzt werden.%
...
\Logo{LaTeX}"~Klassen bereit, welche \emph{nicht} zum \TUDScript-Bundle 
...
welches direkt danach eine Ziffer folgt. Normalerweise setzt \Logo{LaTeX}
...
Aufgrund eines Fehlers im \Logo{LaTeXe}"~Kernel werden Kapitelüberschriften im
zweispaltigen Layout höher gesetzt, als im einspaltigen. Der 
...
\Logo{LaTeXe}"~Kernel übernommen worden. Das Paket \Package{twocolfix} behebt 
...
\Logo{LaTeX}"~Beamer"=Präsentationen im Stil des \TUDCDs mit den 
