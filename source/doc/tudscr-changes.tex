...
\begin{Declaration}
  {\Option{tudscrver=\PName{Version}}}
  (last)
  [v2.03]
...
\itemval{2.02,first}
  ...
  Der Satzspiegel im Layout des \CDs (\seeplain{\Option{cdgeometry}}) wurde in 
  ...
  Seite (\seeplain{\Option{ddcfoot}}) wird im Gegensatz zur Version~v2.02 der 
...
\itemval{2.06}
  ...
  \TUDScript nach einem \Lettering{LaTeX}-Lauf das gleiche Ausgabeergebnis 
...
Funktionalität durch den Parameter \Environment*{tudpage/pagestyle} 
ersetzt.
\end{Cessation}
\end{Cessation}
\end{Declaration*}
...
\itemval{onlydin}
  ...
  nicht \Univers sondern die \Lettering{LaTeX}"=Standardschriften respektive 
  die eines geladenen Schriftpaketes zum Einsatz.
\end{DeclareValues}
\end{Declaration}
...
den entsprechenden Einstellungen (\Option*{cdoldfont=true} oder 
\Option*{cdoldfont=din} respektive \Option*{cdoldfont=onlydin}) aktiviert 
wurde. Hierfür wird intern \Macro*'none'(\Package{textcase}){MakeTextUppercase}
...
bleiben, ist \Macro*'none'(\Package{textcase}){NoCaseChange} zu verwenden.
%
\begin{Example}
...
\Macro*'none'(\Package{textcase}){MakeTextUppercase|\MPValue{\Macro{textdbn|\MPName{Text}}}}
geschehen.
\end{Cessations}
\index{Änderungen|!)}%
