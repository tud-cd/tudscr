...
\begin{Declaration}
  {\Key{\Environment{task}}{headline=\PSet{Überschrift}}}
\begin{Declaration}
  [v2.05]
  {\Key{\Environment{task}}{style=\PSet{Stil}}}
...
(\Macro{author}()) sowie der dazugehörige Titel (\Macro{title}), 
...
(\Macro{duedate}). Am Ende wird für den Prüfungsausschussvorsitzenden 
(\Macro{chairman}) und den betreuenden Hochschullehrer (\Macro{professor}()) 
eine Unterschriftzeile gesetzt, wobei unter den Namen selbst die dazugehörigen 
...
respektive \Macro{DTMDate}(\Package{datetime2}) 
...
\begin{Declaration}
  {\Key{\Environment{evaluation}}{grade=\PSet{Note}}}
\printdeclarationlist
%
...
\begin{Declaration}
  ...
  (\Environment{tudpage}'ref')
\begin{Declaration}
  {\Key{\Environment{notice}}{headline=\PSet{Überschrift}}}
\printdeclarationlist
%
...
werden. Mit dem zweiten erfolgt~-- wie schon bei \Macro{taskform}~-- die 
Ausgabe einiger Schwerpunkte, wobei hier ebenso eine 
\Environment{itemize}(\Bundle{koma-script},\Package{enumitem})"~Umgebung direkt
nach der Überschrift (\Term{focusname}) zum Einsatz und deshalb allen 
Stichpunkten ein \Macro{item}(\Bundle{koma-script},\Package{enumitem}) 
...
Kontaktinformationen für eine oder mehrere Ansprechpartner angeben. Mehrere 
