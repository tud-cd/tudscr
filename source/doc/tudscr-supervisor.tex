...
respektive \Macro(\Package{datetime2}){DTMDate} 
...
\Environment(\Bundle{koma-script},\Package{enumitem}){itemize}"~Umgebung direkt
nach der Überschrift (\Term{focusname}) zum Einsatz und deshalb allen 
Stichpunkten ein \Macro(\Bundle{koma-script},\Package{enumitem}){item} 
...
\begin{Code}[escapechar=§]
\noticeform{%
  Kurzbeschreibung des Inhaltes der studentischen Arbeit
  §\dots§
