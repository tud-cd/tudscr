...
Parameter, welche bei der Beschreibung von \Environment'full'{tudpage} 
erläutert wurden.
...
Parameter \Environment{task/headline} kann genutzt werden, um diese 
...
\Environment{task/style=\PValue{table}} aktiviert werden. Wird hingegen 
der Parameter \Environment{task/style=\PValue{inline}} gesetzt, so 
...
\Macro'none'(\Bundle{koma-script},\Package{enumitem}){item} vorangestellt 
werden.
\end{Declaration}
%
\begin{Example}
...
Ausgabeformatierung des Datums mit \Macro'none'(\Package{isodate}){printdate} 
beziehungsweise \Macro'none'(\Package{datetime2}){DTMDate} für \Macro{duedate} 
...
Umgebung \Environment'full'{tudpage} beschrieben wurden.
...
mit dem Parameter \Environment{evaluation/headline} ersetzt werden. Am 
Ende des Gutachtens wird die mit \Environment{evaluation/grade} 
...
Textbereiches angezeigt (\seeplain{\Environment{tudpage/cdhead}}). Danach 
erfolgt die Ausgabe der Überschrift, welche für gewöhnlich dem Inhalt von 
\Term{noticename} entspricht und mit \Environment{notice/headline} 
...
Abbildung eingefügt werden (\Macro'none'(\Package{graphicx}){includegraphics}). 
...
mit dem Parameter \Environment{evaluation/grade=\PName{Note}} der 
