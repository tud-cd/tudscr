...
\begin{Declaration}{\Environment{task}[\OPList{Überschrift}]}(%
  \Environment{tudpage}'ref'%
)
\begin{Declaration}{\Key{\Environment{task}}{headline=\PSet{Überschrift}}}
\begin{Declaration}[v2.05]{\Key{\Environment{task}}{style=\PSet{Stil}}}
...
zusammensetzt. Der Parameter \Key{\Environment{task}}{headline} kann genutzt 
werden, um diese automatisch generierte Überschrift anzupassen. Als Basis 
fungiert dabei die \Environment{tudpage}"~Umgebung, weshalb sich im optionalen 
...
Parameter \Key{\Environment{task}}{style=inline} gesetzt, erfolgt die Ausgabe 
von mehreren Prüfern und Betreuern in einer Zeile. Danach erscheinen das 
Ausgabedatum (\Macro{issuedate}) sowie der verpflichtende Abgabetermin 
(\Macro{duedate}). Am Ende wird eine Unterschriftzeile für den 
Prüfungsausschussvorsitzenden (\Macro{chairman}) und den betreuenden 
Hochschullehrer (\Macro{professor}()) gesetzt, wobei unter den Namen selbst die 
dazugehörigen Bezeichner (\Term{chairmanname} und \Term{professorname}) 
...
\begin{Declaration}{\Macro{taskform}[\OList\MPName{Ziele}\MPName{Schwerpunkte}]}
\printdeclarationlist%
%
...
\Macro{item}(\Bundle{koma-script},\Package{enumitem}) vorangestellt 
werden.
\end{Declaration}
%
\begin{Example}
...
\begin{Declaration}{\Macro{chairman}[\MPName{Prüfungsausschussvorsitzender}]}
\printdeclarationlist%
%
...
\begin{Declaration}{\Macro{issuedate}[\MPName{Ausgabedatum}]}
\begin{Declaration}{\Macro{duedate}[\MPName{Abgabetermin}]}
\printdeclarationlist%
%
...
\begin{Declaration}{\Environment{evaluation}[\OPList{Überschrift}]}(%
  \Environment{tudpage}'ref'%
)
\begin{Declaration}{%
  \Key{\Environment{evaluation}}{headline=\PSet{Überschrift}}%
}
\begin{Declaration}{\Key{\Environment{evaluation}}{grade=\PSet{Note}}}
\printdeclarationlist%
%
...
Parameter \Key{\Environment{evaluation}}{headline} ersetzt werden. Am Ende des 
Gutachtens wird die mit \Key{\Environment{evaluation}}{grade} gegebene Note in 
...
\begin{Declaration}{\Macro{grade}[\MPName{Note}]}
\printdeclarationlist%
%
...
\begin{Declaration}{\Environment{notice}[\OPList{Überschrift}]}(%
  \Environment{tudpage}'ref'%
)
...
(\Key*{\Environment{tudpage}}{cdhead=date}). Danach erfolgt die Ausgabe der 
Überschrift, welche normalerweise \Term{noticename} entspricht und mit dem 
Parameter \Key{\Environment{notice}}{headline} geändert werden kann. Nach der 
...
passende Abbildung (\Macro{includegraphics}(\Package{graphicx})) eingefügt 
werden. Das zweite Argument dient~-- wie schon bei \Macro{taskform}~-- dazu, 
einige Schwerpunkte aufzuzählen. Auch hier kommt nach der gliedernden 
Überschrift (\Term{focusname}) eine 
\Environment{itemize}(\Bundle{koma-script},\Package{enumitem})"~Umgebung zum 
Einsatz, allen Schwerpunkten muss ein 
\Macro{item}(\Bundle{koma-script},\Package{enumitem}) vorangestellt werden.
\end{Declaration}
%
\begin{Example}
...
\begin{Declaration}[v2.02]{\Macro{contactperson}[\MPName{Kontaktperson(en)}]}
\begin{Declaration'}{%
  \Macro{emailaddress}[\OPName{Einstellungen}\MPName{E-Mail-Adresse}]%
}
\begin{Declaration'}{\Macro{emailaddress*}[\MPName{E-Mail-Adresse}]}
\begin{Declaration}[v2.02]{\Macro{telephone}[\MPName{Telefonnummer}]}
\begin{Declaration}[v2.05]{\Macro{telefax}[\MPName{Telefaxnummer}]}
\begin{Declaration}{\Macro{office}[\MPName{Dienstsitz}]}
...
(\Macro{emailaddress}\footnote{Erläuterungen zu \Macro*{emailaddress}()'full'}) 
angegeben werden.
\end{Declaration}
\end{Declaration}
\end{Declaration}
...
mit dem Parameter \Key{\Environment{evaluation}}{grade=\PSet{Note}} der 
Umgebung \Environment{evaluation} angegeben, so wird diese innerhalb von 
\Term{gradetext} verwendet.
\grade{\PName{Note}}
\TermTable[.8\textwidth]{gradetext}%
\end{Declaration}
