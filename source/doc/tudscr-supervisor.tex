...
zusammensetzt. Der Parameter \Environment{task/headline} kann genutzt 
...
beziehungsweise \Macro(\Package{datetime2}){DTMDate} für \Macro{duedate} und 
...
Gutachtens wird die mit \Environment{evaluation/grade} gegebene Note in 
...
\Environment{evaluation/grade} der Umgebung \Environment{evaluation} 
...
passende Abbildung (\Macro(\Package{graphicx}){includegraphics}) eingefügt 
...
\Environment(\Bundle{koma-script},\Package{enumitem}){itemize}-Umgebung zum 
Einsatz, allen Schwerpunkten muss ein 
\Macro(\Bundle{koma-script},\Package{enumitem}){item} vorangestellt werden.
\end{Declaration}
%
\begin{Example}
...
(\Macro{emailaddress}\footnote{Erläuterungen zu \Macro*'full'{emailaddress}}) 
angegeben werden.
\end{Declaration}
\end{Declaration}
\end{Declaration}
...
kann mit dem Parameter \Environment{evaluation/headline} der 
...
mit dem Parameter \Environment{evaluation/grade=\PSet{Note}} der 
