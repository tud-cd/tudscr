\chapter[%
  ...
  \entitylabel[sec:supervisor]{\Package{tudscrsupervisor}}%
}
...
restliche Dokument mit \Lettering{LaTeX} zu setzen.
...
Sie gerne das \GitHubRepo'discussions' besuchen, um mögliche Anpassungen sowie 
...
noch die Felder für den Studiengang (\Macro{course}), die Fachrichtung 
(\Macro{discipline}) sowie Matrikelnummer und Immatrikulationsjahr 
(\Macro{matriculationnumber}, \Macro{matriculationyear}) angefügt, wobei nicht 
befüllte Felder ignoriert werden. Der eigentliche Inhalt der Umgebung~-- sprich 
die Aufgabenstellung selbst~-- wird nach dem generierten Kopf ausgegeben.
...
\Environment{task/style=table} aktiviert werden. Wird hingegen der 
Parameter \Environment{task/style=inline} gesetzt, erfolgt die Ausgabe 
...
\Macro(\Bundle{koma-script},\Package{enumitem}){item} vorangestellt 
werden.
\end{Declaration}
%
\begin{Example}
...
Ausgabeformatierung des Datums mit \Macro(\Package{isodate}){printdate} 
respektive \Macro(\Package{datetime2}){DTMDate} für \Macro{duedate} 
...
passende Abbildung (\Macro'none'(\Package{graphicx}){includegraphics}) 
eingefügt werden. Mit dem zweiten erfolgt~-- wie schon bei \Macro{taskform}~-- 
die Ausgabe einiger Schwerpunkte, wobei hier ebenso eine 
\Environment'none'(\Bundle{koma-script},\Package{enumitem}){itemize}-Umgebung
direkt nach der Überschrift (\Term{focusname}) zum Einsatz kommt und deshalb 
allen Stichpunkten ein \Macro'none'(\Bundle{koma-script},\Package{enumitem}){item} 
...
\ToDo{Hinweise/Links auf Hauptklassen (in Deklaration)? (footnote raus)}[v2.07]
...
\ToDo[imp]{soll \Macro*{webpage} rein?}[v2.07]
%\begin{Declaration}
