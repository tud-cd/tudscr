...
\ChangedAt{%
  ...
  v2.04:Installationsskripte verbessert und robuster gestaltet sowie für 
        portable Distributionen \Distribution{\hologo{TeX}~Live~Portable} und
...
\begin{quoting}
\Path{%
  cd~"\$HOME/Downloads/TUD-Script_fonts_Unix"{}\,\OPValue{ENTER}
}\newline
...
\begin{quoting}[rightmargin=0pt]
  \Path{%
    cd~"\$env:USERPROFILE\bsc Downloads\bsc%
    TUD-Script_fonts_Windows"{}\,\OPValue{ENTER}%
  }\newline%
  \Path{%
    .\bsc tudscr_fonts_install.bat%
...
\begin{quoting}
  ...
  \Path{%
    bash tudscr_fonts_install.sh > fonts_install.log\,\OPValue{ENTER}%
  }%
\end{quoting}
%
...
\Path{\PName{Laufwerksbuchstabe}:\bsc} befinden.
...
\item Über das Kontextmenü die Kommandozeile ausführen und in dieser das Skript 
  ...
  \Path{%
    cd~/d~\%USERPROFILE\%\bsc Downloads\bsc%
    TUD-Script_fonts_Windows\,\OPValue{ENTER}
  }\newline%
...
\item Bei der erstmaligen Verwendung von \TUDScript werden alle weiteren 
  ...
  \Bundle*{tudscr}, \Bundle{koma-script} sowie \Package{etoolbox}, 
