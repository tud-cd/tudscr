...
verfügbar ist~-- eine durch den Anwender aktuell gehaltene \Logo{LaTeX}"=Distribution 
...
\Logo{LaTeX}"=Distribution für gewöhnlich bis zu zwei Tagen dauert.

Alle gängigen \Logo{LaTeX}"=Distributionen durchsuchen im Regelfall zuerst das lokale 
\Path{texmf}"=Nutzerverzeichnis nach Klassen sowie Paketen und erst daran 
anschließend den \Path{texmf}"~Pfad der \Logo{LaTeX}"=Distribution selbst. Dabei 
...
\Attention{%
  ...
  \TUDScript über die genutzte \Logo{LaTeX}"=Distribution \textbf{nicht} zum Tragen 
...
\Logo{LaTeX}"=Distribution erfolgen. 
...
verwendete \Logo{LaTeX}"=Distribution installiert werden kann.
...
wird. Erscheint nur der Pfad der \Logo{LaTeX}"=Distribution, ist die 
...
gefunden, so wird wahrscheinlich eine veraltete \Logo{LaTeX}"=Distribution verwendet. 
...
Sollte neben dem Pfad der \Logo{LaTeX}"=Distribution noch mindestens ein weiterer 
...
\Logo{LaTeX}"=Distribution. Hierfür ist~-- unter der Annahme, dass das automatisierte 
...
\end{quoting}
%
gefundene~-- zum Ordner der \Logo{LaTeX}"=Distribution \emph{zusätzliche}~-- 
...
\Logo{LaTeX}"=Distribution bereitgestellt wird.
...
\Attention{%
  ...
  unbedingt die \Logo{LaTeX}"=Distribution aktualisiert werden.
}
...
\Logo{LaTeX}"=Distribution bekanntes TDS"~Verzeichnis kopiert und abschließend in der 
...
\Attention{%
  ...
  lokale Nutzerversion oder über die \Logo{LaTeX}"=Distribution installiert ist.%
}
...
\ChangedAt{%
  ...
  v2.04:Installationsskripte verbessert und robuster gestaltet sowie für 
        portable Distributionen \Distribution{\Logo{TeX}~Live~Portable} und
        \Distribution{\Logo{MiKTeX}~Portable} erweitert;%
...
\Path{vptovf} benötigt, welche bei \Distribution{\Logo{TeX}~Live} oder 
\Distribution{Mac\Logo{TeX}} über \Distribution*{fontware} respektive bei
\Distribution{\Logo{MiKTeX}} über \Distribution*{miktex-texware} sowie 
\Distribution*{miktex-fonts} bereitgestellt werden und zwingend installiert 
...
von \Distribution{\Logo{MiKTeX}} durchgeführt werden. Außerdem ist es sehr 
ratsam, die Installation von \Distribution{\Logo{MiKTeX}} in der 
...
über den \Distribution{\Logo{MiKTeX}}"=Paketmanager hinzugefügt werden.
...
%
Sollte keine Vollinstallation von \Distribution{\Logo{TeX}~Live} durchgeführt 
worden sein, müssen die zuvor genannten, erforderlichen Pakete und Skripte zur 
Schriftinstallation über den \Distribution{\Logo{TeX}~Live}"=Paketmanager 
...
Aktualisierung der \Logo{LaTeX}"=Distribution~-- müssen diese über den 
manuellen Aufruf von \Path{updmap} zukünftig durch den Anwender lokal bei 
\Distribution{\Logo{TeX}~Live} respektive \Distribution{Mac\Logo{TeX}} 
registriert werden.
...
ist sowohl für \Distribution{\Logo{TeX}~Live} als auch 
\Distribution{\Logo{MiKTeX}} nutzbar und enthält~-- bis auf die jeweiligen 
...
ist sowohl für \Distribution{\Logo{TeX}~Live} als auch 
\Distribution{Mac\Logo{TeX}} nutzbar und enthält~-- bis auf die jeweiligen 
...
von \Distribution{\Logo{TeX}~Live~Portable} respektive 
\Distribution{\Logo{MiKTeX}~Portable} äquivalent zur nicht"~portablen Variante, 
...
\minisec{\Logo{TeX}~Live~Portable}
%
...
gesendet werden.
\begin{enumerate}
\item Installation von \Distribution{\Logo{TeX}~Live~Portable} in 
...
\item Das Kontextmenü von \Distribution{\Logo{TeX}~Live~Portable} mit einem 
...
\item Über das Kontextmenü die Kommandozeile ausführen und in dieser das Skript 
  ...
  den Anwender über den \emph{\Logo{TeX}~Live~Manager} installiert werden.
  \Attention{%
    Ein Ausführen ohne die über \Distribution{\Logo{TeX}~Live~Portable} 
    geöffnete Kommandozeile führt zu Fehlern.
  }%
\end{enumerate}

\minisec{\Logo{MiKTeX}~Portable}
%
\begin{enumerate}
\item Installation von \Distribution{\Logo{MiKTeX}~Portable} in 
...
\item Das Kontextmenü von \Distribution{\Logo{MiKTeX}~Portable} mit einem 
...
\item Über das Kontextmenü die Kommandozeile ausführen und in dieser das Skript 
  ...
  \Attention{%
    Ein Ausführen ohne die über \Distribution{\Logo{MiKTeX}~Portable} 
    geöffnete Kommandozeile führt zu Fehlern.
  }%
\item Bei der erstmaligen Verwendung von \TUDScript werden alle benötigten 
  Pakete von \Distribution{\Logo{MiKTeX}~Portable} installiert, falls die 
