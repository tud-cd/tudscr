...
\Attention{%
  ...
  genutzte Distribution zum Tragen kommen, falls \TUDScript als lokale 
  Nutzerversion installiert wurde.
}
...
Hierfür werden die Archive \GitHubDownload{TUD-Script_\vTUDScript_Windows.zip} 
für Windows beziehungsweise \GitHubDownload{Script_\vTUDScript_Unix.zip} für 
unixoide Betriebssysteme angeboten. Beim Ausführen des Installationsskriptes 
aus den genannten Archiven~-- für Windows \File{tudscr_\vTUDScript_install.bat} 
respektive für unixoide Betriebssysteme \File{tudscr_\vTUDScript_install.sh}~-- 
...
\Path{texmf}"=Nutzerverzeichnis beziehungsweise in ein anderes der Distribution 
bekanntes TDS"~Verzeichnis kopiert und abschließend in der Kommandozeile 
respektive im Terminal \Path{texhash} aufgerufen werden. 
...
\Attention{%
  ...
  beziehungsweise \Distribution{Mac\hologo{TeX}}|?| über \Package*{fontware}<> 
  aus \Package*{collection-fontutils}<> bereitgestellt werden und zwingend 
...
sowohl für \Distribution{\hologo{TeX}~Live}|?| als auch 
...
\item Bei der erstmaligen Verwendung von \TUDScript werden alle weiteren 
  ...
  \ToDo{koma-script und tudscr nicht als Paket, Links beachten!}[v2.06]
  \Package{tudscr}, \Package{koma-script} sowie \Package{etoolbox}, 
  \Package{xpatch} \Package{letltxmacro}, \Package{environ}, 
  \Package{trimspaces}, \Package{xcolor} und \Package{mptopdf}<pdf-mps-supp>.  
\end{enumerate}
