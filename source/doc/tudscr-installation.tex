...
\ToDo{komplett neu, Historie und Forum spielt keine Rolle (mehr)}[v2.07]
\bigskip\noindent
...
\GitHubFile*{TUD-Script_\vTUDScript{}_Windows.zip} benötigt. Für unixoide 
Betriebssysteme sind es die Dateien aus 
\GitHubFile*{TUD-Script_\vTUDScript{}_Unix.zip}.

Beim Ausführen des spezifischen Installationsskriptes~-- 
\File*{tudscr_\vTUDScript{}_install.bat} für Windows respektive 
\File*{tudscr_\vTUDScript{}_install.sh} für unixoide Betriebssysteme~-- werden 
...
\Attention{%
  In jedem Fall sollte vor der Ausführung des Installationsskriptes ein Update 
  der \Distro durchgeführt werden. Dies gilt insbesondere bei der Nutzung von 
  \MiKTeX. Andernfalls wird es unter Umständen im Installationsprozess oder bei 
  der Nutzung von \TUDScript zu Problemen kommen.
}

Alternativ zur Nutzung der bereitgestellten Installationsskripte kann auch 
einfach der Inhalt des Archivs \GitHubFile*{tudscr_\vTUDScript{}.zip} in das 
lokale \Path{texmf}"=Nutzerverzeichnis kopiert und abschließend in der 
...
\Attention{%
  ...
  und \Path{vptovf} benötigt, welche bei \TeXLive beziehungsweise \MacTeX über 
  das Paket \Bundle*{fontware} aus \Bundle*{collection-fontutils} 
...
%
\settowidth\tempdim{\File*{tudscr_fonts_install.zip}~}%
\begin{description}[labelwidth=\tempdim,labelsep=.5em]
...
dem Befehl \Path{cd} zum Ordner des fraglichen Skriptes navigiert und dieses 
mit \Path{.\bsc\PName{Skript}.bat|Tee-Object -file \PName{Skript}.log} 
...
\item Bei der erstmaligen Verwendung von \TUDScript werden alle benötigten 
  ...
  anderem betrifft dies die Pakete \Bundle*{tudscr}, \Bundle*{koma-script} 
  sowie \Package{etoolbox}, \Package{xpatch} \Package{letltxmacro}, 
