...
\Package{tudscrcomp}'full' sehen, welches eine Schnittstelle zur Nutzung alter 
...
\section{Installationshinweise für \NoCaseChange{\hologo{MiKTeX}} Portable}
\label{sec:install:portable}

\ToDo[doc]{Hinweise auf Tex Live Portable erweitern}[v2.04]
...
Verwendung von \Distribution{\hologo{MiKTeX}~Portable} sollten alle dort 
gegebenen Hinweise ebenfalls beachtet werden. Zusätzlich ist darauf zu achten, 
den Installationspfad von \Distribution{\hologo{MiKTeX}~Portable} nicht 
...
\item Installation von \Distribution{\hologo{MiKTeX}~Portable} in den Pfad
  \Path{\PName{Laufwerk}:\bsc LaTeX\bsc MiKTeXportable} o.\,ä.
\item Die Batchdatei \File{miktex-portable.cmd} im gewählten Installationspfad 	
  ausführen
\item Im Infobereich der Taskleiste mit einem Rechtsklick auf das Symbol von  
  \hologo{MiKTeX}~Portable das Kontextmenü öffnen und ein Update durchführen
\item Über das Kontextmenü die Kommandozeile starten und über diese das 
  ...
  \Attention{%
    Ein Ausführen des Skriptes ohne die über \hologo{MiKTeX}~Portable geöffnete 
    Kommandozeile führt zu Fehlern.
  }
\end{enumerate}
%
Damit sind die Schriften installiert. Bei der erstmaligen Verwendung einer 
Dokumentklasse aus dem \TUDScript-Bundle werden durch \hologo{MiKTeX} folgende 
Pakete nachinstalliert, falls diese nicht schon vorhanden sind:
%
\begin{itemize}
\item \Package{tudscr}
\item \Package{koma-script}
\item \Package{etoolbox}
\item \Package{textcase}
\item \Package{environ}
\item \Package{trimspaces}
\item \Package{xcolor}
\item \Package{mptopdf}()
\end{itemize}
