...
\Attention{%
  ...
  genutzte \Distro \textbf{nicht} zum Tragen kommen, falls 
...
Sollte neben dem Pfad der \Distro noch mindestens ein 
...
Installationsskriptes aus dem Archiv~-- \File*{tudscr_\vTUDScript{}_install.bat}
für Windows respektive \File*{tudscr_\vTUDScript{}_install.sh} für unixoide 
...
\Attention{%
  ...
  von \MiKTeX in der Basiskonfiguration~-- sollte ein 
  Update der \Distro durchgeführt werden. Andernfalls 
...
Inhalt des TDS"~Archivs \GitHubFile*{tudscr_\vTUDScript{}.zip} in das lokale 
...
externen Speichermedium \Path{\PName{Laufwerk}:\bsc} befinden.
...
\item Installation von \TeXLive+ in 
  \Path{\PName{Laufwerk}:\bsc LaTeX\bsc texlive}
...
\item Über das Kontextmenü die Kommandozeile ausführen und in dieser das Skript 
  ...
  diese mittels \Application*{\TeXLive~Manager} installiert werden.
  \Attention{%
...
\item Installation von \MiKTeX+ in 
  \Path{\PName{Laufwerk}:\bsc LaTeX\bsc MiKTeX}%
  \footnote{%
    Der Pfad darf \emph{nicht} auf der obersten Verzeichnisebene 
    \Path{\PName{Laufwerk}:\bsc} liegen.%
  }
...
\item Bei der erstmaligen Verwendung von \TUDScript werden alle benötigten 
  ...
  dies die Pakete \Bundle*{tudscr}, \Bundle{koma-script} sowie \Package{xpatch},
  \Package{etoolbox}, \Package{letltxmacro}, \Package{environ}, 
