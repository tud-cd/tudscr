...
\item \Application{\Lettering{LyX}}~-- grafisches Front"~End für \Lettering{LaTeX}
\end{itemize}
%
Für \Application{\Lettering{TeX}studio} wird im \GitHubRepo'releases' das Archiv 
\GitHubFile!'TeXstudio'(\Bundle{tudscr}){tudscr4texstudio.zip} zur Erweiterung 
...
Möchten Sie das grafische \Lettering{LaTeX}"~Frontend~\Application{\Lettering{LyX}} 
...
können. Diese sind zusammen mit einem \Application{\Lettering{LyX}}"~Dokument als 
Archiv \GitHubFile!'LyX'(\Bundle{tudscr}){tudscr4lyx.zip} im \GitHubRepo'releases'
verfügbar. Um die Layout"~Dateien zu aktivieren, müssen diese in den passenden 
Unterordner im \Application{\Lettering{LyX}}"=Installationspfad kopiert werden. 
...
\Macro(\Bundle{koma-script}){addtokomafont|\MPValue{disposition}\MPValue{%
  \Macro(\Package{setspace}){setstretch|\MPValue{1}}}}
...
(\Option{parskip=half} oder \Option{parskip=true} sondern~-- wie es 
aus typografischer Sicht zumeist sinnvoll ist~-- stattdessen Einzüge verwendet 
(\Option{parskip=false}), kann es vorkommen, dass ein ganz bestimmter 
...
\Lettering{LaTeX}"~Zähler so umdefiniert werden, dass sie bei der Änderung eines 
...
\Macro(\Package{xspace}){xspace} aus dem Paket \Package{xspace} genutzt werden. 
...
\Hint{v2.02}%
Auslassungspunkte werden für \Lettering{LaTeX} mit den Befehlen \Macro{dots} oder 
...
\Package{ellipsis} mit der Option \Option*(\Package{ellipsis}){xspace}, wodurch 
...
respektive Terminal mit \File*{inkscape.exe}. Damit dieser auch tatsächlich 
...
über die Optionen \Option*(\Package{typearea}){paper=landscape} und 
\Option*(\Package{typearea}){paper=A3} geändert werden kann.
\begin{Code}
...
\Macro{geometry/paper=\PSet{Papierformat}} das Papierformat festgelegt 
werden kann. Wird zusätzlich noch der Parameter 
\Macro{geometry/layout=\PSet{Zielformat}} angegeben, so wird damit das 
gewünschte Zielformat definiert. Dabei sollte mit 
\Macro{geometry/layoutoffset=\PLength} dieser Bereich gegebenenfalls 
etwas eingerückt werden. Die Angabe von 
\Macro{geometry/showcrop=\PBoolean} generiert außerdem noch visuelle 
Schnittmarken.
\end{Entity}
%
\begin{quoting}[rightmargin=0pt]
