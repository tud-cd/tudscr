...
\Macro{autoref}[\MPName{label}](\Package{hyperref})'none' zur Verfügung. Mit 
...
\Macro{addtokomafont}[%
  \MPValue{disposition}\MPValue{%
    \Macro{setstretch}[\MPValue{1}](\Package{setspace})'none'%
  }%
...
(\KOMAScript-Option \Option*{parskip=false}(\Bundle{koma-script})'none'), kann 
...
\Macro*{counterwithout*}[%
  \MPValue{footnote}\MPValue{chapter}%
...
\Hint{v2.02}%
Wird mit \Macro{tableofcontents}(\Bundle{koma-script})'none' das 
...
\Macro{sisetup}[\MPValue{locale = DE}](\Package{siunitx})'none' angegeben 
...
\Option{paper=\PMisc}(\Package{typearea})'none' sowohl möglich, lediglich die 
...
einfach über die Option~\Option{fontsize}(\Bundle{koma-script})'none' 
