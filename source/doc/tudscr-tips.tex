...
\Macro{autoref|\MPName{label}} zur Verfügung. Mit 
...
\Macro*{\PName{Element}autorefname} oder 
...
können die Zähler \Counter{biburlnumpenalty}, 
\Counter{biburlucpenalty} und 
\Counter{biburllcpenalty} erhöht werden. Das 
...
Ziffern~(\Counter{biburlnumpenalty}), 
Majuskeln~(\Counter{biburlucpenalty}) und 
Minuskeln~(\Counter{biburllcpenalty}) kommt. 
...
von \Macro{recalctypearea} angepasst werden. Allerdings 
...
Absatzes das Makro \Macro{noindent} aufgerufen wird. Soll 
...
\Hint{v2.02}%
Wird mit \Macro{tableofcontents} das Inhaltsverzeichnis 
...
\Macro{xspace} aus dem Paket \Package{xspace} genutzt 
...
\Macro{sisetup|\MPValue{locale = DE}} angegeben 
...
Optionen via \InlineDeclaration{%
  \Macro{usepackage|\OPName{Paketoptionen}\MPName{Paket}} 
...
vorhandenen \Environment{pspicture}"~Umgebungen 
erstellten Grafiken enthält. Mit dem Befehl \Macro{ifpdf} aus dem Paket 
...
mit der \Environment{pspicture}"~Umgebung wie gewohnt 
verwenden.
%
\begin{quoting}
\begin{Code}
...
\Option{paper=\PMisc} sowohl möglich, lediglich die 
...
über die Optionen \Option*{paper=landscape} und 
...
Mit der \TUDScript-Option \Option{bleedmargin} können
...
einspaltigem und zweispaltigen Layout (\Option{twocolumn}) 
unterschieden wird. Die nachfolgend erläuterten Makros lassen sich sehr 
einfach mit
\Macro*{renewcommand*|\MPName{Makro}\MPName{Wert}} 
ändern.
...
Für diesen Fall lässt sich das Paket \Package{scrwfile} 
...
einfach über die Option~\Option{fontsize} einstellen, 
...
\item Wenn im Dokument ohnehin das Paket \Package{hyperref} verwendet wird, 
  ...
  \Macro{hypersetup|\MPValue{pdfprintscaling=None}}
  ausreichend.
