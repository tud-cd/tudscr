...
\section{%
  \Lettering{LaTeX}-Entwicklungsumgebungen%
  \label{sec:tips:ide}%
}

\ToDo[doc]{%
  Suche nach fontenc\textbar hyphsubst (hyphsubst anders anwenden?)
}[v2.06]%
Hier werden die gängigsten Entwicklungsumgebung genannt, welche zum Erstellen 
und Bearbeiten von \Lettering{LaTeX}"~Dateien geeignet sind. Ich persönlich 
nutze seit langem \Application{\Lettering{TeX}studio}, da diese Anwendung viele 
Unterstützungs- und Assistenzfunktionen bietet. Neben diesen gibt es natürlich 
noch weitere \Lettering{LaTeX}"=Entwicklungsumgebungen. Unabhängig von der 
...
\item \Application*{LEd}~-- früher \Application*{\Lettering{LaTeX}~Editor}
\item \Application*{\Lettering{LyX}}~-- grafisches \Format*{LaTeX}"~Front"~End
\end{itemize}
%
...
Einige \Lettering{LaTeX}"~Zähler werden bei der Inkrementierung eines 
überlagerten Zählers zurückgesetzt. Diese Abhängigkeit wird durch 
\Macro{counterwithin} definiert, mit \Macro{counterwithout} kann diese gelöst 
werden. Durch \Macro{counterwithout|*\MPValue{footnote}\MPValue{chapter}} lässt 
...
\Hint{v2.02}%
Auslassungspunkte werden für \Lettering{LaTeX} mit den Befehlen \Macro{dots} 
...
mit der \PValue{T1}~Zeichensatzkodierung erforderlich, damit auch Wörter mit 
...
Trennstrich identisch. Leider wird der \Lettering{LaTeX}"=Trennungsalgorithmus 
...
\section{%
  ...
  \entitylabel[sec:tips:options]{\Macro{PassOptionsToPackage}}%
  \index{Optionen|?}%
}
...
jedes zu erstellende Verzeichnis reserviert \Lettering{LaTeX} selbst jeweils 
...
welches einige Änderungen am \Lettering{LaTeXe}"~Kernel vornimmt. Das Ziel 
...
\Lettering{LaTeX} normalerweise auf einer separaten Seite vertikal zentriert 
...
\section{%
  ...
  \entitylabel[sec:tips:svg]{\Macro{includesvg}}%
  \index{Grafiken}%
}

\Hint{v2.04}%
Dank der entsprechenden Exportfunktion ist es prinzipiell möglich, 
\Application{Inkscape}"=Grafiken in \Lettering{LaTeX}"=Dokumente einzubinden. 
...
das anschließende Einbinden von \Application{Inkscape}"=Grafiken in das 
Dokument bei der Nutzung eines \Format*{LaTeX}"~Formats möglich. Für diesen 
...
Argumente \emph{vor} den beiden obligatorischen, was bei \Lettering{LaTeX} für 
die (Re"~)Definition von Befehlen normalerweise nicht vorgesehen ist. Deshalb 
wird das Paket \Package{xparse} geladen, mit welchem dies möglich wird. 
...
\ToDo{
  ...
  widowpenalty, clubpenalty, brokenpenalty, displaywidowpenalty
}[v2.07]
