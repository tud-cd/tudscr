...
\end{Preamble}
%
Für die Erstellung einer Aufgabenstellung kann die Umgebung \Environment*{task} 
oder der Befehl \Macro*{taskform} verwendet werden. Bei beiden Varianten wird 
zu Beginn eine Tabelle mit Informationen zum Autor angegeben. Am unteren Ende 
werden der oder die Betreuer der Arbeit sowie Professor und gegebenenfalls der 
Prüfungsausschussvorsitzende ausgegeben. Dazwischen kann bei der Umgebung 
\Environment*{task} ein beliebiger Inhalt gesetzt werden. Dahingegen erzeugt 
der Befehl \Macro*{taskform} eine standardisierte Ausgabe. Dabei ist darauf zu 
achten, dass das zweite obligatorische Argument des Befehls innerhalb einer 
\Environment{itemize}"=Umgebung verwendet wird und somit \Macro{item} zu nutzen 
ist. Das Resultat des folgenden Quelltextes ist in \autoref{fig:task} zu sehen. 
%
\begin{Excerpt!}{Task}
\faculty{Juristische Fakultät}
\department{Fachrichtung Strafrecht}
...
Das Pakete \Package{glossaries} sollte immer \emph{nach} \Package{hyperref} 
...
eines \hologo{LaTeX}"=Dokumentes externe Dateien direkt beim Kompilieren 
erstellen zu können. Dies wird genutzt, um nachfolgend eine Literaturdatenbank 
\PValue{treatise-temp.bib} mit drei Einträgen manuell zu erzeugen. Zuerst 
erfolgt die Ausgabe, darunter folgt die Erläuterung:
%
\begin{Preamble}
...
\ToDo[imp]{Leerzeilen in \Environment{Preamble} verhindern}[v2.02]
