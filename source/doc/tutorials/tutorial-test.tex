...
\AfterEndEnvironment{Tutorial}{%
  ...
  \executeandcheck{\@Tutorial}%
  \executeandcheck{\input{\jobname-temp.tut}}%
  \input{\jobname-temp.tut}%
  \par\noindent%
  \setlength{\leftskip}{\@tempdimc}%
}
\newcommand\executeandcheck[1]{%
  \par
  \global\setbox1\vbox{\parindent=0pt\hbox{\strut}#1}%
  \@tempswatrue%
  \setlength{\@tempdima}{\wd1}%
  \ifdim\@tempdima=\z@\relax\@tempswafalse\fi%
  \vspace{-\baselineskip}%
  \if@tempswa%
    \hbox{\usebox1\strut}\vspace{.5\baselineskipglue}%
  \else%
    \usebox1%
  \fi%
}