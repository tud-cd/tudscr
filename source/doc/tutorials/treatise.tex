...
\Macro{setstretch}[\Parameter{Faktor}](\Package{setspace}) bereitstellt. 
...
Schluss jeder \PValue{X}"~Spalte mit \PValue{X<\PParameter{\Macro{strut}}} 
...
\ttabbox
  ...
  {\caption{§\PName{Beschriftung}§\label§\Parameter{Label}§}}
\end{table}
\begin{figure}
\ffigbox
...
\Macro{captionsetup}[\PParameter{type=figure/table}](\Package{caption}), womit
...
\Macro{captionof}[\Parameter{figure/table}\Parameter{Bezeichnung}]%
...
\Macro{dictum}[\OParameter{Autor}\Parameter{Text}](\Bundle{koma-script})
