...
Zeichenkodierung mit der Option~\Option(\Package{fontenc}){T1} aktiviert wird. 
...
Abschnitte mit
\Macro(\Bundle{koma-script}){frontmatter} (Vorspann), 
\Macro(\Bundle{koma-script}){mainmatter} (Hauptteil) und 
\Macro(\Bundle{koma-script}){backmatter} (Nachspann) gekennzeichnet werden, wobei
\Macro(\Bundle{koma-script}){frontmatter}
die Seitennummerierung auf römische Zahlen setzt und 
\Macro(\Bundle{koma-script}){mainmatter} arabische Seitenzahlen aktiviert. Mit 
\Macro(\Bundle{koma-script}){frontmatter} und 
\Macro(\Bundle{koma-script}){backmatter} 
wird weiterhin die Nummerierung der Kapitelüberschriften deaktiviert.
...
\Option(\Bundle{koma-script}){listof=toc}
...
Hilfsdateien, anhand derer die Indexe erzeugt und sortiert werden können. Für 
die Sortierung ist der Aufruf einer externen Anwendung notwendig. Die meisten 
\Distro* stellen dafür \Application{makeindex} bereit. 
Dieses Programm kann zwar Einträge alphabetisch sortieren, bietet allerdings 
weder die Nutzung von Unicode"~Symbolen noch eine Sprachunterstützung. Aufgrund 
der genannten Nachteile ist \Application{xindy} zum Sortieren der Einträge zu 
empfehlen, da dieses Programm sowohl eine Unterstützung von Unicode als auch 
die Möglichkeit, nach sprachabhängigen Regeln zu sortieren, bietet. Für die 
deutsche Sprache gibt es beispielsweise zwei Varianten der Sortierung~-- 
nach DIN oder Duden. 
...
dafür allerdings erweiterte Schreibrechte notwendig, weshalb für diese Variante 
\Format{LaTeX} mit dem Parameter \Path{-{}-shell-escape} aufgerufen werden muss. 
Für die Verwendung von \Application{xindy} ist außerdem die Angabe der Sprache 
über die Option \Option*{xindy=\MPValue{language=\dots}} zwingend notwendig. 
...
\Macro(\Package{glossaries}){newacronym|%
  \OList\MPName{Label}\MPName{Abkürzung}\MPName{Wortgruppe}}
...
und den eingeschränkten Möglichkeiten von \Lettering{LaTeX}, diese an 
...
\Attention{%
  ...
  weshalb durch das \GitHubRepo<tabu-fixed/tabu>[\Lettering{LaTeX3}-Projektteam]
...
\FinishTutorial[%
  ...
  \Attention{%
    ...
    Perl"~Skriptes \File(\Package{glossaries}){makeglossaries} unter Umständen 
