...
erweiterte Trennmuster verwendet. Für \Format{pdfLaTeX} lässt sich eine 
...
\Macro{setstretch}[\MPName{Faktor}](\Package{setspace}) bereitstellt. Wird 
...
Einen Anhang starten Sie mit \Macro{appendix}(\Bundle{koma-script}), womit 
...
](\Bundle{koma-script},\Package{caption}) genutzt werden, um diesen eine 
Bezeichnung hinzuzufügen. Mit \Macro{listoffigures}(\Bundle{koma-script}) 
beziehungsweise \Macro{listoftables}(\Bundle{koma-script}) lassen sich 
...
\Format{pdfLaTeX} diese Funktionalität nicht bereitstellt, ist hierfür der 
...
Befehl \Macro{dictum}[\OPName{Autor}\MPName{Text}](\Bundle{koma-script})
...
erfolgen, sollte der Befehl \Macro{dictum}(\Bundle{koma-script}) innerhalb von 
...
\Macro{caption}(\Bundle{koma-script},\Package{caption}) für die Beschriftung 
...
\Macro{captionof}(\Bundle{koma-script}) für die Beschriftung zur Verfügung. 
...
\PValue{\bsc\bsc}. Da jedoch einige \hologo{LaTeX}"=Pakete 
...
werden, wobei es nicht mit der klassischen \Format*{TeX}"~Engine, wohl jedoch 
mit \Format{pdfTeX} als auch \Format{LuaTeX} sowie \Format{XeTeX} funktioniert.
%
\begin{Preamble}
\usepackage[babel]{microtype}
...
zusätzliche Option \Option{-{}-shell-escape}(\Format{pdfLaTeX}) beziehungsweise 
\Option{-{}-enable-write18}(\Format{pdfLaTeX}) mit erweiterten Schreibrechten 
