...
\Attention{%
  ...
  bereitgestellt, was gegebenenfalls die zusätzliche Installation eines solchen 
  erfordert.
}

Kann das Skript \File(\Package{glossaries}){makeglossaries} nicht genutzt 
werden, kann alternativ dazu die Option \Option(\Package{glossaries}){automake}
aktiviert werden. Diese sorgt dafür, dass am Ende des \Format{pdfLaTeX}"~Laufs 
...
Schreibrechte notwendig, weshalb für diese Variante \Format{pdfLaTeX} mit dem 
Parameter \Path{-{}-shell-escape} aufgerufen werden muss. Für die Verwendung 
von \Application{xindy} ist außerdem die Angabe der Sprache über die Option 
...
\FinishTutorial[%
  ...
  \Attention{%
    Zur Nutzung des Perl"~Skriptes \File(\Package{glossaries}){makeglossaries} 
