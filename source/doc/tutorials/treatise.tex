...
\end{Preamble}
%
Das Paket \Package{scrhack}(\Bundle{koma-script}) behebt einige Probleme bei 
...
\Macro{setstretch}[\MPName{Faktor}](\Package{setspace}) bereitstellt. 
...
nötig, die sich mit der Klassenoption \Option*{BCOR=6mm}(\Package{typearea}) 
...
wird der mit \Macro{maketitle}'manual' erzeugte Titel immer im Satzspiegel des 
...
Abschnitte mit \Macro{frontmatter}(\Bundle{koma-script}) (Vorspann), 
\Macro{mainmatter}(\Bundle{koma-script}) (Hauptteil) und 
\Macro{backmatter}(\Bundle{koma-script}) (Nachspann) gekennzeichnet werden, 
wobei \Macro{frontmatter}(\Bundle{koma-script}) die Seitennummerierung auf 
römische Zahlen setzt und \Macro{mainmatter}(\Bundle{koma-script}) arabische 
Seitenzahlen aktiviert. Mit \Macro{frontmatter}(\Bundle{koma-script}) und 
\Macro{backmatter}(\Bundle{koma-script}) wird weiterhin die Nummerierung der 
Kapitelüberschriften deaktiviert.

Einen Anhang starten Sie mit \Macro{appendix}(\Bundle{koma-script}), welcher 
...
\KOMAScript-Option \Option{numbers}(\Bundle{koma-script}) kann das Verhalten 
...
\Environment{itemize}(\Bundle{koma-script},\Package{enumitem})"~Umgebung 
verwendet wird und somit \Macro{item}(\Bundle{koma-script},\Package{enumitem})
...
Das Inhaltsverzeichnis wird mit \Macro{tableofcontents}(\Bundle{koma-script}) 
erzeugt und führt die Gliederung des erstellten Dokumentes entsprechend der 
verwendeten Befehle%
\footnote{%
  \Macro{part}(\Bundle{koma-script}),
  \Macro{addpart}(\Bundle{koma-script}), 
  \Macro{chapter}(\Bundle{koma-script}), 
  \Macro{addchap}(\Bundle{koma-script}), 
  \Macro{section}(\Bundle{koma-script}),
  \Macro{addsec}(\Bundle{koma-script}) etc.%
}
...
](\Bundle{koma-script},\Package{caption})
genutzt werden, um diesen eine Bezeichnung hinzuzufügen. Die Verzeichnisse 
lassen sich mit \Macro{listoffigures}(\Bundle{koma-script}) beziehungsweise 
\Macro{listoftables}(\Bundle{koma-script}) erstellen, in denen alle 
...
\Option{listof}(\Bundle{koma-script})\PValue{=toc}
...
\Macro{caption}(\Bundle{koma-script},\Package{caption}) allerdings zusätzlich 
ergänzt mit einem zuvor aufgerufenem
\Macro{captionsetup}[\MPValue{type=figure/table}](\Package{caption}) 
...
kann der Befehl \Macro{AtEndPreamble}(\Package{etoolbox}) aus dem Paket 
...
werden, steht alternativ dazu die Option \Option{automake}(\Package{glossaries})
...
\Macro{newacronym}[\OList\MPName{Label}\MPName{Abkürzung}\MPName{Wortgruppe}](\Package{glossaries})
...
\Macro{gls}[\MPName{Label}](\Package{glossaries}) verwendet.
%
\begin{Trunk*}
...
Mit \Macro{newglossarystyle}(\Package{glossaries}) lässt sich ein eigener 
Glossarstil definieren, wobei innerhalb des neuen Stils die Umgebung 
\Environment{theglossary}(\Package{glossaries}) umdefiniert wird. Nachfolgend 
...
Abkürzung mit \Macro{gls}[\MPName{Label}](\Package{glossaries}) verwendet 
...
\Macro{newformulasymbol}[%
  \OList\MPName{Label}\MPName{Name}%
  \MPName{Symbol}\MPName{Einheit}%
]|?|%
\end{quoting}
%
...
\Macro{gls}[\MPName{Label}](\Package{glossaries}) in einer kurzen 
...
\end{Preamble*}
%
Bei der Verwendung von \Macro{gls}[\MPName{Label}](\Package{glossaries}) 
...
Umgebung \Environment{filecontents}(\Package{filecontents}) nutzen.
...
momentan \emph{leider} nur die Umgebung \Environment{longtabu}(\Package{tabu}) 
...
Dokument mit \Macro{cite}[\MPName{macro}](\Package{biblatex}) referenziert, 
...
\Macro{caption}(\Bundle{koma-script},\Package{caption}) verwendet wird, da 
...
\Macro{caption}(\Bundle{koma-script},\Package{caption}) entweder vor oder 
...
\end{Preamble}
%
Dieses stellt den Befehl \Macro{captionsetup}(\Package{caption}) bereit. Mit 
...
Befehl \Macro{caption}(\Bundle{koma-script},\Package{caption}) gegebenenfalls 
...
\Macro{captionsetup}[\MPValue{type=figure/table}](\Package{caption}), womit
...
(\Bundle{koma-script}) für die Beschriftung zur Verfügung. 
%Werden die Pakete \Package{caption} und \Package{hyperref} geladen, sollte 
%weiterhin \Macro{caption}(\Bundle{koma-script},\Package{caption}) ergänzt um 
...
die Umgebungen \Environment{quote}(\Bundle{koma-script}) und 
\Environment{quotation}(\Bundle{koma-script}) an. Allerdings verwenden beide 
fest eingestellte Absatzauszeichnungen. Sollen jedoch die mit der Option 
\Option{parskip}(\Bundle{koma-script}) vorgegebenen Einstellungen für Absätze 
...
\Macro{dictum}[\OPName{Autor}\MPName{Text}](\Bundle{koma-script})
...
erfolgen, sollte der Befehl \Macro{dictum}(\Bundle{koma-script}) innerhalb 
von \Macro{setpartpreamble}(\Bundle{koma-script}) beziehungsweise 
\Macro{setchapterpreamble}(\Bundle{koma-script}) verwendet werden. Genaueres 
...
\footnote{%
  \Macro{chapter}(\Bundle{koma-script}), 
  \Macro{section}(\Bundle{koma-script}) etc.%
}
...
\Macro{caption}(\Bundle{koma-script},\Package{caption}) respektive 
\Macro{captionof}(\Bundle{koma-script}) erstellt werden. Um nun auf diesen 
...
\end{Hint}
%
Die Option \Option{ngerman}(\Package{hyphsubst}) lädt dabei die Trennungsmuster 
...
ist stattdessen die Option \Option{german}(\Package{hyphsubst}) mit dem Wert 
...
geladen werden, wobei die Option \Option{babel}(\Package{microtype}) zur 
...
\Environment{itemize}(\Bundle{koma-script},\Package{enumitem}),
\Environment{enumerate}(\Bundle{koma-script},\Package{enumitem}) und
\Environment{description}(\Bundle{koma-script},\Package{enumitem}). Das 
häufigste Anliegen bei der Verwendung dieser Umgebungen ist das Reduzieren 
der Abstände zwischen den einzelnen, mit 
\Macro{item}(\Bundle{koma-script},\Package{enumitem}) gesetzten Punkten, 
...
\Environment{itemize}(\Bundle{koma-script},\Package{enumitem}),
\Environment{enumerate}(\Bundle{koma-script},\Package{enumitem}) und
\Environment{description}(\Bundle{koma-script},\Package{enumitem}). 
...
\Macro{lstset}[\MPValue{\PValue{escapechar=\PName{Zeichen}}}](\Package{listings})
...
ausgeführt. Außerdem ist es mit \Macro{lstnewenvironment}(\Package{listings}) 
...
den Befehl \Macro{lstinputlisting}[\OList\MPName{Dateiname}](\Package{listings})
