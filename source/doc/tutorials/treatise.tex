...
\ttabbox{%
  \caption{%
    Eine vollständig formatierte und mit dem Befehl \Macro{ttabbox} gesetzte 
    Tabelle in einer \Environment{table}"=Gleitumgebung%
...
\ToDo[doc]{ttabbox und tabellen etwas schmaler}
...
\begin{tabu} to \textwidth {@{}Z[3,l]Z[2,c]Z[3,r]Z[2,j]@{}}
\toprule
\textbf{Linksbündig} & \textbf{Zentriert} & 
\textbf{Rechtsbündig} & \textbf{Blocksatz} \tabularnewline
\midrule
...
\ToDo[doc]{tikz und auto-pst-pdf (ifdpf notwendig)}


\subsection{Die Verwendung von floatrow und pstricks in einem Dokument}
\ToDo[doc]{ttabbox und auto-pst-pdf (ifdpf notwendig)}
\begin{Hint}
\usepackage{floatrow}
\floatsetup{font=sf}
\usepackage{xparse}
