...
\Macro(\Package{setspace}){setstretch|\MPName{Faktor}} bereitstellt. Wird 
bespielsweise \OpenSans~-- die Hausschrift der \TnUD~-- verwendet, sollte der 
Faktor im Bereich \PValue{1.05}--\PValue{1.1} liegen.
...
Einen Anhang starten Sie mit \Macro(\Bundle{koma-script}){appendix}, womit 
...
\Macro(\Bundle{koma-script}){listoffigures} beziehungsweise 
\Macro(\Bundle{koma-script}){listoftables} lassen sich Verzeichnisse erstellen, 
...
Ein eigener Glossarstil kann mit \Macro(\Package{glossaries}){newglossarystyle} 
definiert werden, wobei für den neu definierten Stil die Umgebung 
\Environment(\Package{glossaries}){theglossary} umdefiniert wird. Als erstes 
kommt die Tabellenumgebung \Environment(\Package{tabularx}){tabularx} aus dem 
...
Die sehr komfortabel zu nutzende Umgebung \Environment(\Package{tabu}){tabu} 
...
\Environment(\Package{tabularx}){tabularx}-Tabellen muss generell eine feste 
...
\PValue{X<\MPValue{\Macro{strut}}} einfach der Befehl \Macro{strut} 
...
\Environment(\Package{filecontents}){filecontents} aus dem gleichnamigen Paket 
...
momentan nur die Umgebung \Environment(\Package{tabu}){longtabu} aus dem Paket 
...
\Environment(\Package{longtable}){longtable} basiert und nachfolgend verwendet 
...
die Umstellung älterer \Lettering{BibTeX}"=Datenbanken ist mit wenigen 
Handgriffen realisierbar. Für \Package{biblatex} existieren eine Menge 
unterschiedlicher, vordefinierter Zitierstile, welche sich im Vergleich zu 
\Lettering{BibTeX} auch wesentlich leichter an die individuellen Bedürfnisse 
anpassen lassen.
...
\Environment(\Package{filecontents}){filecontents}-Umgebung~-- im Gegensatz 
...
Befehl \Macro(\Bundle{koma-script}){dictum|\OPName{Autor}\MPName{Text}}
...
erfolgen, sollte der Befehl \Macro(\Bundle{koma-script}){dictum} innerhalb von 
\Macro(\Bundle{koma-script}){setpartpreamble} beziehungsweise 
...
das Paket \Package{floatrow} die Befehle \Macro(\Package{floatrow}){ttabbox} 
für Tabellen sowie \Macro(\Package{floatrow}){ffigbox} für Abbildungen bereit. 

Beide Makros erwarten im ersten obligatorischen Argument den Befehl 
\Macro(\Bundle{koma-script},\Package{caption}){caption} für die Beschriftung 
...
Verwendung von des Befehls \Macro(\Package{floatrow}){ttabbox} ist in 
...
\Lettering{LaTeX} gewöhnt und müssen respektive wollen gegebenenfalls nur noch 
bei wenigen Gleitobjekten~-- über das optionale Argument der Gleitumgebungen zur
...
\Macro(\Bundle{koma-script}){captionof} für die Beschriftung zur Verfügung. 
...
Mit dem Paket \Package{floatrow} lässt sich eine weitere Variante entwickeln, 
...
große Hilfe und stellt die Befehle \Macro(\Package{booktabs}){toprule}, 
\Macro(\Package{booktabs}){midrule} sowie \Macro(\Package{booktabs}){cmidrule} 
und \Macro(\Package{booktabs}){bottomrule} für unterschiedliche horizontale 
Linien bereit.
%
\begin{Code}{preamble}
\usepackage{booktabs}
...
\PValue{\bsc\bsc}. Da jedoch einige \Lettering{LaTeX}-Pakete 
...
ist die Verwendung von \Macro(\Package{floatrow}){ttabbox} aus dem Paket 
...
ausgeben. Hierfür werden die Befehle \Macro(\Package{varioref}){vref} sowie 
\Macro(\Package{varioref}){vpageref} zur Verfügung gestellt. Das Paket 
\Package{cleveref} vereint die Vorzüge von \Macro(\Package{hyperref}){autoref} 
in Verbindung mit \Macro(\Package{varioref}){vref} durch den Befehl 
...
Grafiken. Die zwei bekanntesten \Lettering{LaTeX}-Pakete sind 
\Package{tikz}<pgf> und \Package{pstricks}<pstricks-base>. Mit beiden wird 
...
Normalerweise verwendet \Package{tikz}<pgf> Koordinaten im Zentimeterraster. 
...
Nutzung von \Format{pdfLaTeX} etwas schwierig. Das Paket \Package{auto-pst-pdf} 
schafft hier jedoch Abhilfe. Wird das Textsatzsystem \Format{pdfLaTeX} über die 
zusätzliche Option \Option(\Format{pdfLaTeX}){-{}-shell-escape} beziehungsweise 
\Option(\Format{pdfLaTeX}){-{}-enable-write18} mit erweiterten Schreibrechten 
...
\Package{listings}-Umgebung zwischen zwei Escape-Zeichen eingefügten Angaben in 
der Form \PName{Zeichen}\PValue{\dots}\PName{Zeichen} werden als 
\Lettering{LaTeX}-Code gelesen und ausgeführt. Außerdem ist es mit dem Befehl 
...
\item \hrfn{http://www.latex-community.org/forum/}{\Lettering{LaTeX} Community}
\end{itemize} 
