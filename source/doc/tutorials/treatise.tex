...
Layout des \TUDCDs erscheint. Wie dieses Verhalten geändert werden kann, ist 
im Handbuch für \Macro{makecover}'manual' erläutert. Die resultierende Ausgabe 
des nachfolgenden Quelltextauszugs ist in \autoref{fig:title} zu sehen und 
stellt eine mögliche Ausprägung von Umschlagseite und Titel dar.
%
\begin{figure}
...
Breite der Laufweite des Fließtextes, so ist deren Gebrauch nicht unbedingt 
notwendig. In \autoref{tab:tabularbox} ist das Ergebnis der Verwendung des 
Befehls \Macro{ttabbox}(\Package{floatrow}) zu sehen.
...
\InputCode\noindent
%
Die Breite der einzelnen, vom \PValue{X}"~Spaltentyp abgeleiteten Derivaten ist 
...
\ToDo{Hinweis auf die schlechte Nutzbarkeit von \Package{tabu}}[v2.06]
...
Die zuvor mit \Package{tikz} erstellte Grafik wird nachfolgend mit dem Paket 
\Package{pstricks} erstellt. Mit diesem lassen sich PostScript-Grafiken 
erstellen. Ähnlich wie bei \Package{tikz} gibt es ein Kernpaket und zusätzliche 
