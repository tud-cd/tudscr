...
Hierfür muss das Paket \Package*{tudscrsupervisor} geladen werden.
%
\begin{Excerpt*}
\usepackage{tudscrsupervisor}
\end{Excerpt*}
...
\end{Excerpt*}
%
Um eigene Grafiken und Abbildungen mit \hologo{LaTeX} anzufertigen, kann das 
Paket \Package{tikz} verwendet werden. In \autoref{sec:figures} wird hierfür 
...
\end{Excerpt*}
%
Neben dem Paket \Package{tikz} gibt es mit \Package{pstricks} eine weitere 
Möglichkeit, Grafiken zu \enquote{programmieren}. Auch für dieses Paket ist in 
besagtem \autorefname[sec:figures] ein Beispiel zu finden.
...
\begin{figure}
\centering
\IncludeExcerpt[width=.5\textwidth]{Abstract}
...
\begin{figure}
\centering
\IncludeExcerpt[width=.5\textwidth]{Declaration}
...
Anfänger durchaus frustrierend sein. Das liegt häufig am Missverständnis von 
Gleitobjektumgebungen \Environment{table} für Tabellen und \Environment{figure} 
für Abbildungen. Diese sind in erster Linie zur Ergänzung des Fließtextes 
gedacht und sollten für das prinzipielle Verständnis des Geschrieben nicht 
notwendig sein. Das oft geforderte Verhalten, ein Gleitobjekt an einer ganz 
bestimmten und explizit festgelegten Position im Text zu setzen, ist nicht 
erforderlich, insbesondere weil dadurch der Lesefluss unnötig unterbrochen 
wird. Vielmehr ist es sinnvoll, Gleitobjekte entweder am Anfang oder Ende einer 
Seite zu platzieren, wo sie deutlich weniger stören. Allerdings sollte auf 
jedes einzelne Gleitobjekt im Fließtext über eine Referenz~-- beispielsweise 
...
In der Konsequenz ist das hier ein Plädoyer, bei der Platzierung von Tabellen 
...
zu diesem Zeitpunkt schon mit Sicherheit an das Vorgehen von \hologo{LaTeX} 
gewöhnt und müssen/wollen gegebenenfalls nur noch bei wenigen Gleitobjekten 
nachjustieren. Im \TUDScript"=Handbuch ist außerdem ein eigener Abschnitt zum 
Thema \enquote{\manualhyperref{sec:tips:floats}{Platzierung von Gleitobjekten}} 
zu finden.

\ToDo{Beispiel, placeins und FloatBarrier}
%FloatBarrier
...
%
%
%Das häufig angefragte völlige Abstellen des Gleitprozesses ist meistens keine 
%gute Lösung. Die Folge wären schlecht gefüllte Seiten und damit ein sehr 
%unruhiges Erscheinungsbild, viel Handarbeit mit hart kodierten Seitenumbrüchen 
%sowie ein Dokument, das äußerst anfällig gegen Umbruchänderungen ist. Sollte 
%es 
%dennoch in Einzelfällen erforderlich sein, dass eine Abbildung nicht gleitet, 
%dann ist die einzig logische Konsequenz, auf die Verwendung einer 
%Gleitumgebung 
%ganz zu verzichten. Dabei eignen sich Umgebungen wie {center} oder 
%{flushleft}, 
%um etwas Vor- und Nachabstand zu erzeugen und {minipage}, um Grafik und 
%Bildunterschrift zusammenzuhalten. Die Dokumentenklassen aus dem Paket 
%»KOMA-Script« stellen für diese Anwendung das Makro \captionof zur Verfügung. 
%Damit lassen sich Bild- und Tabellenunterschriften auch ohne die gleitenden 
%{figure}- oder {table}-Umgebungen erzeugen. Das Makro erwartet als erstes 
%Argument die Art der Unterschrift, die erzeugt werden soll. Ein Beispiel für 
...
\label{sec:figures}
%
\ToDo{tikz zur als ergänzender Hinweis, usepackage hierher}
%
...
Grafiken. Die beiden bekanntesten \hologo{LaTeX}-Pakete sind \Package{tikz} und 
\Package{pstricks}. Für diese wird im Folgenden eine Abbildungen exemplarisch 
erstellt, ohne dabei tiefer ins Detail zu gehen. Für beide Pakete gibt es neben 
der Dokumentation zahlreiche Beispiele und Erläuterungen im Internet.
...
\Package{tikz} Koordinaten im Zentimeter-Raster. Ich persönlich bevorzuge der 
