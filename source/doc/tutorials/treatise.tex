...
\DeclareSourcemap{
  \maps[datatype=bibtex]{
    \map[overwrite]{
      \perdatasource{\jobname-temp.bib}
      \step[fieldset=keywords, fieldvalue={,}, appendstrict]
      \step[fieldset=keywords, fieldvalue={\jobname-temp}, append]
    }
  }
}
\begin{filecontents}[nosearch,force,nowarn]{\jobname.bib}
@book{talbot2012,
  author    = {Nicola L. C. Talbot},
  title     = {\Lettering{LaTeX} for Complete Novices},
  publisher = {Dickimaw Books},
  series    = {Dickimaw \Lettering{LaTeX} Series},
  volume    = {1},
...
@book{talbot2013,
  author    = {Nicola L. C. Talbot},
  title     = {Using \Lettering{LaTeX} to Write a PhD Thesis},
  publisher = {Dickimaw Books},
  series    = {Dickimaw \Lettering{LaTeX} Series},
  volume    = {2},
...
@online{reichert2012,
  ...
  title        = {\Lettering{LaTeX}~-- Satz von Tabellen},
...
@article{neubauer1996,
  ...
  journaltitle = {Die \Lettering{TeX}nische Komödie},
  year         = {1996},
  month        = {2},
  volume       = {4},
  pages        = {23-40},
...
@online{struckmann2007,
  ...
  title   = {Einige typographische Grundregeln und
             ihre Umsetzung in \Lettering{LaTeX}},
  date    = {2007-09-03},
...
\StartTutorial[%
  ...
  \tudhyperref{sec:packages}{\TUDScript-Handbuch} zusätzlich kurz beschrieben.
...
Mit dem Paket \Package{fontenc} lässt sich die Zeichensatzkodierung für die 
Ausgabe der verwendeten Schriften im Dokument spezifizieren, wobei mit der 
Option~\Option(\Package{fontenc}){T1} die europäische Zeichensatzkodierung 
aktiviert wird. Kommt mit \Format{LuaLaTeX} oder \Format{XeLaTeX} ein 
Unicode"=Textsatzsystem zum Einsatz, sollte besser das Paket \Package{fontspec} 
geladen werden. Für die Eingabekodierung wird standardmäßig UTF"~8 verwendet.%
\footnote{%
  Beim Einsatz von \Format{pdfLaTeX} ist eine abweichende Eingabekodierung der 
  Quelldatei möglich. Diese lässt sich entweder automatisch mit dem Paket 
  \Package{selinput} ermitteln oder manuell mit dem Paket \Package{inputenc} 
  festgelegen. In beiden Fällen muss das Paket \Package{fontenc} \emph{vorher} 
  geladen worden sein.
} 
...
Das Paket \Package(\Bundle{koma-script}){scrhack} behebt bei der Verwendung 
einer \KOMAScript-Klasse einige Kompatibilitätsprobleme mit Drittpaketen.
Bei Bedarf sollte dieses frühzeitig in der Präambel geladen werden. 
%
\begin{Code}{preamble}
\usepackage{scrhack}
\end{Code}
...
\Option(\Package{typearea}){twoside} als Klassenoption angegeben werden. Im 
...
\KOMAScript-Option \Option(\Package{typearea}){DIV} verantwortlich, mit der 
\KOMAScript-Option \Option(\Package{typearea}){BCOR} wird die Bindekorrektur 
...
\Macro(\Package{setspace}){setstretch|\MPName{Faktor}} bereitstellt. 
...
sollte der Faktor im Bereich \PValue{1.05}\nobreakdashes--\PValue{1.1} liegen.
...
nötig, die sich mit der Klassenoption \Option*(\Package{typearea}){BCOR=6mm} 
...
Abschnitte mit \Macro(\Bundle{koma-script}){frontmatter} (Vorspann), 
\Macro(\Bundle{koma-script}){mainmatter} (Hauptteil) und 
\Macro(\Bundle{koma-script}){backmatter} (Nachspann) gekennzeichnet werden, 
wobei \Macro(\Bundle{koma-script}){frontmatter} die Seitennummerierung auf 
römische Zahlen setzt und \Macro(\Bundle{koma-script}){mainmatter} arabische 
Seitenzahlen aktiviert. Mit \Macro(\Bundle{koma-script}){frontmatter} und 
\Macro(\Bundle{koma-script}){backmatter} wird weiterhin die Nummerierung der 
Kapitelüberschriften deaktiviert.

Einen Anhang starten Sie mit \Macro(\Bundle{koma-script}){appendix}, welcher 
...
\KOMAScript-Option \Option(\Bundle{koma-script}){numbers} kann das Verhalten 
...
\Environment(\Bundle{koma-script},\Package{enumitem}){itemize}-Umgebung 
verwendet wird und somit \Macro(\Bundle{koma-script},\Package{enumitem}){item}
...
\begin{figure}
\centering
\IncludeStandalone[.5\textwidth]{abstract}
\caption{%
...
\begin{figure}
\centering
\IncludeStandalone[.5\textwidth]{declaration}
\caption{%
...
Das Inhaltsverzeichnis wird mit \Macro(\Bundle{koma-script}){tableofcontents} 
erzeugt und führt die Gliederung des erstellten Dokumentes entsprechend der 
verwendeten Befehle%
\footnote{%
  \Macro(\Bundle{koma-script}){part},
  \Macro(\Bundle{koma-script}){addpart}, 
  \Macro(\Bundle{koma-script}){chapter}, 
  \Macro(\Bundle{koma-script}){addchap}, 
  \Macro(\Bundle{koma-script}){section},
  \Macro(\Bundle{koma-script}){addsec} etc.%
}
...
\Macro(\Bundle{koma-script},\Package{caption}){caption|%
  \OPName{Verzeichniseintrag}\MPName{Bezeichnung}}
genutzt werden, um diesen eine Bezeichnung hinzuzufügen. Die Verzeichnisse 
lassen sich mit \Macro(\Bundle{koma-script}){listoffigures} beziehungsweise 
\Macro(\Bundle{koma-script}){listoftables} erstellen, in denen alle 
...
\Option(\Bundle{koma-script}){listof=toc} lassen sich die Verzeichnisse der 
...
\Macro(\Bundle{koma-script}){captionof|%
  \MPName{Typ}\OPName{Verzeichniseintrag}\MPName{Bezeichnung}}
oder~-- falls die beiden Pakete \Package{caption} und \Package{hyperref} 
genutzt werden~-- auch wie gewohnt mit 
\Macro(\Bundle{koma-script},\Package{caption}){caption} allerdings zusätzlich 
ergänzt mit einem zuvor aufgerufenem
\Macro(\Package{caption}){captionsetup|\MPValue{type=figure/table}} 
...
manuell erfolgen muss, weshalb ich von diesem eher abrate.
...
kann der Befehl \Macro(\Package{etoolbox}){AtEndPreamble} aus dem Paket 
...
Perl"~Skript \Application(\Package{glossaries}){makeglossaries} verwendet 
werden, welches alle nötigen Optionen an die jeweilige Anwendung weiterleitet.

\Attention{%
  Für den Aufruf von \Application{xindy} selbst als auch für die Ausführung des 
  Skriptes \Application(\Package{glossaries}){makeglossaries} ist ein 
  Perl"=Interpreter notwendig. Dieser wird lediglich von \TeXLive nicht jedoch 
  von \MiKTeX direkt bereitgestellt, was gegebenenfalls die zusätzliche 
  Installation eines solchen erfordert.
}

Kann das Perl"~Skript \Application(\Package{glossaries}){makeglossaries} nicht 
genutzt werden, steht die Option \Option(\Package{glossaries}){automake} 
alternativ dazu bereit. Mit dieser erfolgt der Aufruf von \Application{xindy} 
respektive \Application{makeindex} direkt. Soll \Application{xindy} verwendet 
werden, sind dafür allerdings erweiterte Schreibrechte notwendig, weshalb 
\Format{LaTeX} hierfür mit dem Parameter \Path{-{}-shell-escape} aufgerufen 
werden muss. Für \Application{xindy} ist außerdem die Angabe der Sprache über 
die Option \Option*{xindy=\MPValue{language=\dots}} zwingend notwendig. 

Ist weder die Nutzung von \Application(\Package{glossaries}){makeglossaries} 
noch der~-- im Zweifel sicherheitskritische~-- Aufruf von \Format{LaTeX} mit 
...
Neben \Macro(\Package{glossaries}){newglossaryentry} für allgemeine 
Glossareinträge stellt das Paket \Package{glossaries} den Befehl 
\Macro(\Package{glossaries}){newacronym|%
  \OList\MPName{Label}\MPName{Abkürzung}\MPName{Bezeichnung}} 
zur spezifischen Definition von Abkürzungen bereit, welche anschließend mit 
\Macro(\Package{glossaries}){gls}\MPName{Label} und artverwandten Befehlen 
genutzt werden können. Detaillierte Hinweise hierzu sind der Dokumentation von 
\Package{glossaries} nachzuschlagen. Für ein kleines Beispiel werden drei 
...
% TODO geht das eleganter mit CodeHook (glsdisablehyper + Setzen des Stils)?
...
kommt die Tabellenumgebung \Environment(\Package{tabularx}){tabularx} 
exemplarisch zum Einsatz, welche in \autoref{sec:tabularx} vorgestellt wird.
...
Die Umgebung \Environment(\Package{tabu}){tabu} wird durch das Paket 
...
Für die bereits genutzten \Environment(\Package{tabularx}){tabularx}"~Tabellen 
...
Wie \Environment(\Package{tabularx}){tabularx} bietet auch die Umgebung 
\Environment(\Package{tabu}){tabu} einen \PValue{X}"~Spaltentyp. Für diese kann 
...
Umgebung \Environment(\Package{filecontents}){filecontents} nutzen.
...
\Environment(\Package{tabu}){longtabu}, welche wiederum auf der Umgebung 
\Environment(\Package{longtable}){longtable} basiert.
\Attention{%
...
definiert werden.
%
\CodeHook{\let\providecaptionname\defcaptionname}
\begin{Code}[input]{hint}
\providecaptionname{%
...
\begin{Code}{hint}
\newformulasymbol[parent=§\dots§letters]{§\dots§}{§\dots§}{§\dots§}{§\dots§}
\end{Code}
...
die Umstellung älterer Datenquellen im klassischen Format von \Lettering{BibTeX}
...
UTF"~8"=Eingabekodierung nutzen, wenn \Application{biber} zur Sortierung der 
...
Die \Environment(\Package{filecontents}){filecontents}-Umgebung kann verwendet 
...
\Environment(\Package{filecontents}){filecontents}~-- im Gegensatz zur 
...
%
\begin{Code}[input]{preamble}
\begin{filecontents}[nosearch,force,nowarn]{\jobname-temp.bib}
@book{goossens94,
...
@manual{hanisch19,
  author    = {Hanisch, Falk},
  title     = {Ein \LaTeX-Bundle für Dokumente im
               Corporate Design der Technischen Universität Dresden},
  date      = {2019},
...
\end{Code}
% TODO geht das eleganter mit CodeHook (Kolumnentitel und heading)?
\begin{quoting}[rightmargin=0pt]
\makeatletter
...
Die Positionierung von Abbildungen mit \Lettering{LaTeX} kann zu Beginn für 
viele Anfänger durchaus frustrierend sein. Das liegt häufig am Missverständnis 
der beiden Standard"=Gleitobjektumgebungen für Tabellen (\Environment{table}) 
und Abbildungen (\Environment{figure}). Diese sind in erster Linie zur 
...
Ein weiterer Grund, \Lettering{LaTeX} die Platzierung von Tabellen und 
...
geachtet werden, dass der Befehl \Macro{label} immer erst \emph{nach} 
\Macro(\Bundle{koma-script},\Package{caption}){caption} verwendet wird, da 
...
und Abbildungen vollständig auf \Lettering{LaTeX} zu vertrauen. Anfangs kann 
...
\Macro(\Bundle{koma-script},\Package{caption}){caption} entweder vor oder 
...
\Environment{table}-Umgebung in der selben Schrift wie der Fließtext 
...
Dieses stellt den Befehl \Macro(\Package{caption}){captionsetup} bereit. Mit 
...
Zumeist soll die Beschriftung für Gleitobjekte nicht zentriert sondern 
linksbündig gesetzt werden. Dies lässt sich ebenfalls mit den Mitteln des 
Paketes \Package{caption} und der passenden Paketoption 
\Option(\Package{caption}){justification} erreichen. Damit dies auch für 
einzeilige Beschriftungen angewendet wird, muss zusätzlich die Einstellung 
\Option(\Package{caption}){singlelinecheck} deaktiviert werden. Mit der 
Paketoption \Option(\Package{caption}){format} kann außerdem beeinflusst 
...
\Package{floatrow} die Befehle \Macro(\Package{floatrow}){ttabbox} für Tabellen 
sowie \Macro(\Package{floatrow}){ffigbox} für Abbildungen bereit. 
...
Befehl \Macro(\Bundle{koma-script},\Package{caption}){caption} gegebenenfalls 
...
\ttabbox
  ...
  {\caption{§\PName{Beschriftung}§\label{§\PName{Tabellenlabel}§}}}
\end{table}
\begin{figure}
\ffigbox
  §\MPName{Abbildungsdefinition}§
  {\caption{§\PName{Beschriftung}§\label{§\PName{Abbildungslabel}§}}}
\end{figure}
\end{Code}
...
\Macro(\Package{floatrow}){ffigbox} oder \Macro(\Package{floatrow}){ttabbox}
...
wahrscheinlich schon an das standardmäßige Vorgehen von \Lettering{LaTeX} 
gewöhnt und wollen gegebenenfalls lediglich bei wenigen Gleitobjekten~-- über 
das optionale Argument der Gleitumgebungen zur individuellen Empfehlung für die 
Platzierung~-- mit folgenden Werten nachjustieren:
%
\begin{description}[labelindent=\parindent,leftmargin=*,style=nextline,noitemsep]
\item[\OPValue{h} (here)]
...
\includegraphics[scale=0.75]{TUD-black}
\caption{%
...
\Macro(\Package{caption}){captionsetup|\MPValue{type=figure/table}}, womit
...
lassen. Wird auf das Paket \Package{caption} verzichtet, stellt \KOMAScript 
alternativ den Befehl 
\Macro(\Bundle{koma-script}){captionof|\MPName{figure/table}\MPName{Bezeichnung}}
für die Beschriftung zur Verfügung. 

\ToDo{\Environment{table-} und \Environment{figure-}}
...
entsprechenden Argumenten definiert werden sollte. Zum Verständnis des 
Quelltextauszuges sei auf die Dokumentation des Paketes verwiesen.
%
\begin{Code}[input]{hint}
\floatbox[\captionsetup{type=figure}]{figure}[\FBwidth][%
  \dimexpr\FBheight+\abovedisplayskip\relax%
][b]{\includegraphics[scale=0.75]{TUD-black}}{%
  \caption{%
...
Hilfe und stellt die Befehle \Macro(\Package{booktabs}){toprule}, 
\Macro(\Package{booktabs}){midrule}, \Macro(\Package{booktabs}){cmidrule} 
sowie \Macro(\Package{booktabs}){bottomrule} für diverse horizontale Linien 
bereit.
%
\begin{Code}{preamble}
\usepackage{booktabs}
...
\PValue{\bsc\bsc}. Da einige \Lettering{LaTeX}"~Pakete diesen Befehl jedoch 
...
werden die Spaltentypen \PValue{D} und \PValue{F} für den links- und 
...
\Attention{%
  Durch Änderungen am \Lettering{LaTeX}"~Kernel wurde das Paket leider 
...
\Macro(\Package{floatrow}){ttabbox} aus dem Paket \Package{floatrow}, um die 
...
\begin{table}
\ttabbox{%
  \begin{tabu} to .85\textwidth {@{}Z[8,l]Z[7,c]Z[8,r]Z[9,j]@{}}
    \toprule
    \textbf{Linksbündig} & \textbf{Zentriert} & 
    \textbf{Rechtsbündig} & \textbf{Blocksatz} \tabularnewline\midrule
    Ein linksbündiger Blindtext 
      zur Darstellung einer \texttt{Z[l]}"~Spalte &
    Ein zentrierter Blindtext 
      zur Darstellung einer \texttt{Z[c]}"~Spalte &
    Ein rechtsbündiger Blindtext 
      zur Darstellung einer \texttt{Z[r]}"~Spalte &
    Ein längerer Blindtext im Blocksatz 
      zur besseren Darstellung einer \texttt{Z}"~Spalte
    \tabularnewline\bottomrule
  \end{tabu}%
}{%
...
die Umgebungen \Environment(\Bundle{koma-script}){quote} und 
\Environment(\Bundle{koma-script}){quotation} an. Allerdings verwenden beide 
fest eingestellte Absatzauszeichnungen. Sollen jedoch die mit der Option 
\Option(\Bundle{koma-script}){parskip} vorgegebenen Einstellungen für Absätze 
...
\end{Code}
% TODO ist das notwendig? wenn ja, geht das eleganter mit CodeHook
\begin{quoting}[rightmargin=0pt]
\makeatletter
\let\markboth\@gobbletwo
\let\markright\@gobble
...
\Macro(\Bundle{koma-script}){dictum|\OPName{Autor}\MPName{Text}}
...
erfolgen, sollte der Befehl \Macro(\Bundle{koma-script}){dictum} innerhalb 
von \Macro(\Bundle{koma-script}){setpartpreamble} beziehungsweise 
\Macro(\Bundle{koma-script}){setchapterpreamble} verwendet werden. Genaueres 
...
\footnote{%
  \Macro(\Bundle{koma-script}){chapter}, 
  \Macro(\Bundle{koma-script}){section} etc.%
}
...
\Macro(\Bundle{koma-script},\Package{caption}){caption} respektive 
\Macro(\Bundle{koma-script}){captionof} erstellt werden. Um nun auf diesen 
Anker zu referenzieren, sollte nicht \Macro{ref} sondern 
\Macro(\Package{hyperref}){autoref} im Fließtext genutzt werden. Letztgenannter 
Befehl erzeugt nicht nur einen Link mit der dazugehörigen Nummerierung sondern 
stellt dieser die Bezeichnung des referenzierten 
Objektes\footnote{\enquote{Abbildung}, \enquote{Tabelle}, \dots} voran. 
...
ausgeben. Hierfür lassen sich die Befehle \Macro(\Package{varioref}){vref} 
sowie \Macro(\Package{varioref}){vpageref} nutzen. Das Paket \Package{cleveref} 
kombiniert die Vorzüge von \Macro(\Package{hyperref}){autoref} und 
\Macro(\Package{varioref}){vref} durch den Befehl 
\Macro(\Package{cleveref}){cref}.
...
\PValue{T1}"~Zeichensatzkodierung respektive \Package{fontspec} notwendig. Der 
...
\end{itemize}
%
Die letzten beiden Punkte werden sehr gut und automatisch durch das 
...
\Environment(\Bundle{koma-script},\Package{enumitem}){itemize},
\Environment(\Bundle{koma-script},\Package{enumitem}){enumerate} und
\Environment(\Bundle{koma-script},\Package{enumitem}){description}. Das 
häufigste Anliegen bei der Verwendung dieser Umgebungen ist das Reduzieren 
der Abstände zwischen den einzelnen, mit 
\Macro(\Bundle{koma-script},\Package{enumitem}){item} gesetzten Punkten, 
...
\Environment(\Bundle{koma-script},\Package{enumitem}){itemize},
\Environment(\Bundle{koma-script},\Package{enumitem}){enumerate} und
\Environment(\Bundle{koma-script},\Package{enumitem}){description}. 
...
stehen mit \TUDScriptTutorial{mathtype} und \TUDScriptTutorial{mathswap} zwei 
weitere Tutorials zur Verfügung, die sich explizit mit der Thematik eines guten 
mathematischen Satz beschäftigen.

Das Paket \Package{amsmath} stellt für \Lettering{LaTeX} den 
...
Für das Setzen neuer Dokumente sollte deshalb das Paket \Package{mathtools} 
verwendet werden, das auf \Package{amsmath} basiert und dieses um zusätzliche 
...
Die meisten \Lettering{LaTeX}"=Schriftfamilien enthalten Ligaturen, die für 
...
\begin{Code}{preamble}
\usepackage{ellipsis}
\let\ellipsispunctuation\relax % keine Sonderbehandlung von Interpunktionen
...
Wird das Paket mit einer der \TUDScript-Klassen geladen, wird der genannte 
Befehl automatisch für alle bereitgestellten Datumsfelder verwendet. Alternativ 
dazu kann das Paket \Package{datetime2} genutzt werden, dessen Befehl 
\Macro(\Package{datetime2}){DTMDate} wird ebenfalls unterstützt.
%
\begin{Code}{preamble}
\usepackage{isodate}
...
Abbildungen mit dem Befehl \Macro(\Package{graphicx}){includegraphics} aus dem 
Paket \Package{graphicx}. 
...
sind noch zwei weitere \Lettering{TikZ}"=Programmbibliotheken notwendig.
%
\begin{Code}{hint}
\usepackage{tikz}
\usetikzlibrary{chains}
...
Normalerweise verwendet \Package{tikz} Koordinaten im Zentimeterraster. Ich 
...
Ähnlich wie bei \Lettering{TikZ} gibt es für PSTricks ein Kernpaket und 
zusätzliche Ergänzungen. Diese werden jedoch nicht als Programmbibliotheken 
sondern ganz normal als \Lettering{LaTeX}-Pakete eingebunden.
%
\begin{Code}{hint}
\usepackage{pstricks,pst-node}
\end{Code}
...
Paket \Package{auto-pst-pdf} schafft hier Abhilfe. Wird \Format{pdfLaTeX} 
...
Ähnlich wie zuvor bei der \Lettering{TikZ}-Grafik wird auch die nachfolgende 
PSTricks-Grafik auf die Textbreite des Dokumentes skaliert, wobei eine Einheit 
genau \SI{1}{\percent} dieser entspricht. Außerdem werden noch ein paar weitere 
Einstellungen gemacht. Dies geschieht mit:
%
\begin{Code}[input]{hint}
\psset{%
  unit=.01\textwidth,%
...
Die innerhalb einer Gleitumgebung erzeugte Grafik ist in \autoref{fig:pstricks} 
dargestellt.
%
\begin{Code}[swap]{hint}
\begin{figure}
...
\PName{Zeichen}\PValue{\dots}\PName{Zeichen} innerhalb einer 
\Package{listings}-Umgebung als \Lettering{LaTeX}-Code gelesen und 
ausgeführt werden. Außerdem ist es mit dem Befehl 
\Macro(\Package{listings}){lstnewenvironment} möglich, eine individuelle 
...
%
\begin{itemize}
\item \hrfn{https://www.mrunix.de/forums/forumdisplay.php?38}{mrunix}
\item \hrfn{https://golatex.de}{golatex}
\item \hrfn{https://tex.stackexchange.com}{\Lettering{LaTeX} Stack Exchange}
\item \hrfn{https://latex.org/forum}{\Lettering{LaTeX} Community}
\end{itemize} 
...
\FinishTutorial[%
  ...
  frühestens nach dem ersten Durchlauf von \Format{LaTeX}. Dies erfolgt mit 
  ...
  \Path{makeglossaries~\PName{Dateiname}} ausreichend. Danach ist ein weiterer
  Lauf von \Format{LaTeX} notwendig, um die Verzeichnisse in das Dokument 
  einzubinden. Gegebenenfalls ist für korrekte Seitenangaben in Abkürzungs- 
  und Symbolverzeichnis ein abermaliges Ausführen des Skriptes notwendig.
  \Attention{%
    Zur Nutzung von \Application(\Package{glossaries}){makeglossaries} 
    mit \MiKTeX muss gegebenenfalls ein separater Perl"=Interpreter installiert 
    werden. In \autoref{sec:glossaries} ist dazu mehr zu finden.%
  }
]
\ListOfToDo
\end{document}
