...
\begin{filecontents}[nosearch,force,silent]{\jobname.bib}
@book{talbot2012,
...
Das Paket \Package(\Bundle{koma-script}){scrhack} behebt einige Probleme bei 
...
\Option(\Bundle{koma-script}){listof}\PValue{=toc}
...
Befehl \Macro(\Package{scrbase}){AfterPackage*} aus dem \KOMAScript-Bundle. Mit 
...
werden, welches alle notwendigen Optionen an die jeweilige Anwendung 
weiterleitet.
...
kommt die Tabellenumgebung \Environment(\Package{tabularx}){tabularx}, welche 
...
\end{filecontents}
\end{Code}
% TODO: was soll das? lieber [nosearch,force,silent]
% \begingroup%
% \makeatletter
...
geachtet werden, dass \Macro{label} immer erst \emph{nach} 
...
%weiterhin \Macro(\Bundle{koma-script},\Package{caption}){caption} ergänzt um 
...
\Macro(\Package{hyperref}){autoref} im Fließtext genutzt werden. 
...
\subsection{Das Paket \Package'none'{pstricks}}
...
\Package{listings}-Umgebung werden als \Lettering{LaTeX}-Code gelesen und 
...
den Befehl
\Macro(\Package{listings}){lstinputlisting|\OList\MPName{Dateiname}} 
