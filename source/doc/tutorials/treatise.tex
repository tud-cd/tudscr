...
Perl"~Skript \Application(\Package{glossaries}){makeglossaries.pl} verwendet 
...
\Attention{%
  ...
  Skriptes \Application(\Package{glossaries}){makeglossaries.pl} ist ein 
...
Kann das Skript \Application(\Package{glossaries}){makeglossaries.pl} nicht 
...
\Format{pdfLaTeX} hierfür mit \Path{-{}-shell-escape} aufgerufen werden muss. 
Für \Application{xindy} ist außerdem die Angabe der Sprache über die Option 
\Option*{xindy=\MPValue{language=\dots}} zwingend notwendig. 

Ist weder die Nutzung von \Application(\Package{glossaries}){makeglossaries.pl} 
...
\makeatother
\expanded{\noexpand\printbibliography[keyword=\jobname-temp]}
\end{quoting}
...
Paket \Package{auto-pst-pdf} schafft hier Abhilfe. Wird \Format{pdfLaTeX} via 
\Path{-{}-shell-escape} mit erweiterten Schreibrechten aufgerufen, so werden 
die Grafiken automatisch im PDF"~Format erzeugt und eingebunden. Ohne 
erweiterte Schreibrechte lässt sich der Prozess mit dem Paket \Package{pst-pdf} 
manuell ausführen.
%
\begin{Code}{hint}
...
\FinishTutorial[%
  ...
  \Attention{%
    Zur Nutzung von \Application(\Package{glossaries}){makeglossaries.pl} 
