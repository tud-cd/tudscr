...
höherwertiges Papier mit einer Dichte von \SI{100}{g/m^2} verwendet, so 
entsprechen 100~Blatt in etwa einer Höhe von \SI{12}{mm}. Dementsprechend 
wäre bei eine Bindekorrektur \SI{6}{mm} diesem Beispiel notwendig. Diese 
...
Sicht im Normalfall verzichtet werden. Die Gründe hierfür werden im Handbuch zu 
\TUDScript bei der Vorstellung des Paketes \Package*{setspace} erläutert, 
welches im Zweifelsfall für dieses Anliegen verwendet werden sollte. Ebenfalls 
wird im Handbuch beschrieben, wie das Paket zu verwenden ist.
...
Für ein kleines Beispiel werden fünf Formelzeichen definiert. Für eine gute 
Darstellung der mathematischen Brüche wird dabei der Befehl \Macro{sfrac} aus 
dem Paket \Package{xfrac} genutzt.
%
\begin{Trunk*}
...
\end{Trunk*}
%
Die so definierten Symbole werden nun mit \Macro{gls}\Parameter{Label} in einer 
kurzen Textpassage verwendet.
%
\begin{Hint*}
...
\Macro{ffigbox} für Abbildungen bereit. Beide Makros erwarten im ersten 
obligatorischen Argument den Befehl \Macro{caption} für die Beschriftung 
...
wenn das zu setzende Objekt schmaler als \Length{textwidth} ist. Das Ergebnis 
der Verwendung von \Macro{ttabbox} ist in \autoref{tab:tabularbox} zu sehen. 
...
\ttabbox{%
  \caption{%
    Eine vollständig formatierte und mit \Macro{ttabbox} gesetzte Tabelle 
    in einer \Environment{table}"=Gleitumgebung%
...
definiert werden. Dies geschieht mit dem Befehl \Macro{DeclareCaptionSubType} 
aus dem Paket \Package{subcaption}.
%
\begin{Preamble}
\DeclareCaptionSubType[alph]{figure}
\DeclareCaptionSubType[alph]{table}
...
Hierfür stehen die Pakete \Package{tabularx}, \Package{tabulary} oder auch 
\Package{tabu} zur Verfügung, welche nun kurz vorgestellt werden sollen. Das 
Paket \Package{tabu} ist relativ neu und versucht, viele Funktionalitäten ganz 
unterschiedlicher Pakete~-- beispielsweise von \Package{tabularx}~-- für den 
...
bietet sich das Paket \Package{quoting} an.
%
\begin{Preamble}
\usepackage{quoting}
\end{Preamble}
...
Paket \Package{hyphsubst} hingewiesen, welches dieses Manko~-- aufgrund der 
...
\end{Hint}
%
Die Option \Option{ngerman}[ngerman-x-latest] führt dabei zur Verwendung der 
Trennmuster für die neue deutsche Rechtschreibung (\PValue{ngerman}), wobei 
\PValue{ngerman-x-latest} die neuesten aller verfügbaren Trennmuster lädt. Für 
die alte Orthographie ist stattdessen \Option{german}[german-x-lastest] zu 
verwenden.
...
Abkürzung \enquote{\gls{zB}} mit der nachfolgenden Definition:
%
\begin{Hint}
...
Dokumente sollte deswegen das Paket \Package{mathtools} verwendet werden, das 
zwar auf \Package{amsmath} basiert, dieses Paket allerdings um zusätzliche
Funktionalitäten ergänzt und Bugfixes bereithält.

Neben diesem Tutorial stehen mit \Tutorial{mathtype} und \Tutorial{mathswap} 
...
\end{Hint*}
%
Die \Package{pstricks}-Grafik wird innerhalb einer Gleitumgebung erzeugt, die 
Ausgabe ist in \autoref{fig:pstricks} dargestellt.
%
\begin{Hint}
\begin{figure}
...
\section{Einbinden von Quelltexten}
\ToDo[doc]{listings und fancyvrb}

%\usepackage{listings} % [numbered] als Option gibt es für listings nicht
%
%% Einstellungen, damit listings auch mit utf8 problemlos funktioniert
%\lstset{%
%  inputencoding=utf8,extendedchars=true,
%  literate=%
%    {ä}{{\"a}}1 {ö}{{\"o}}1 {ü}{{\"u}}1
...
\section{Vermischtes}
Hier taucht alles auf, was sich nicht in einen der vorherigen Abschnitte 
eingliedern ließ.
...
\subsection{Fußnoten}
Oftmals wird gewünscht, das Standardverhalten des Rücksetzens des Zählers für 
Fußnoten beim Beginn eines neuen Kapitels zu deaktivieren. Dies kann mit dem 
Paket \Package{chngcntr} realisiert werden. Dazu muss in der Präambel lediglich 
...
Das Paket \Package{textcomp} stellt mehrere Befehle für die einfache Nutzung 
von Sonderzeichen in der im Fließtext zur Verfügung.


\subsection{Erweiterte Befehlsdeklaration}
\ToDo{Kurze Beschreibung}
\begin{Hint}
\usepackage{xparse}
...
\ListOfToDo
\ToDo[doc]{Einarbeiten von \File{paragraph.tex} in \File{treatise.tex}}[v2.02]
\ToDo{Abstände nach Kapitelüberschriften von Verzeichnissen passen nicht zum 
normalen Layout}
\ToDo[doc]{%
  Ergebnis Copy \& Paste Beispiel prüfen, verbessern und ggf. erweitern%
}[v2.02]
