...
\begin{document}
\subject{Mathematiksatz mit \Lettering{LaTeX}}
...
\StartTutorial[%
  ...
  \Lettering{LaTeX}-Dokument. Sowohl bei der Einbindung von externen Daten als 
  ...
  \Lettering{LaTeX} selbst zu bewerkstelligen.
  
  Es wird beschrieben, wie das \TUDScript-Paket \Package'manual'{mathswap} für 
  ...
  angepasst. Alternativ dazu kann auch das Paket \Package'manual'(){ionumbers} 
  verwendet werden.
]
Bevor das eigentliche Tutorial beginnt, werden sowohl eine Dokumentklasse als 
auch die für jedes \Lettering{LaTeX}-Dokument meiner Meinung nach sinnvollen 
Pakete geladen.
%
\begin{Preamble}
...
\end{Preamble}
%
Zusätzlich wird das Paket \Package'manual'{mathswap} aus dem \TUDScript-Bundle 
...
\Lettering{LaTeX} im mathematischen Modus bei der Angabe im deutschen 
Zahlenformat~-- Punkt~(\PValue{.}) als Gruppierungs- und Komma~(\PValue{,}) als 
Dezimaltrennzeichen~-- folgendermaßen ausgegeben:%
\footnote{%
  Da in der Präambel dieses Dokumentes das Paket \Package'manual'{mathswap} 
  bereits geladen wurde, wird mit \Macro'manual'{mathswapoff} auf das 
  Standardverhalten von \Lettering{LaTeX} geschaltet. Die Funktionalität wird 
  mit \Macro'manual'{mathswapon} wieder aktiviert.%
}
%
\CodePreamble{%
  Ohne \Package'manual'{mathswap} keine typografische Zifferngruppierung%
}
\begin{Trunk*}
\mathswapoff
\(4.523,58\)
...
\end{Trunk*}
%
Mit dem Paket \Package'manual'{mathswap} wird das Ziel verfolgt, unabhängig vom 
...
respektive einer \Lettering{LaTeX}"=Standardklasse werden diese im 
Mathematikmodus immer nach dem englischen Zahlenformat~-- Komma~(\PValue{,}) 
als Gruppierungs- und Punkt~(\PValue{,}) als Dezimaltrennzeichen~-- betrachtet.
...
verwendete Zahl bei aktiver Funktionalität von \Package'manual'{mathswap} im 
...
die beiden Befehle \Macro'manual'{commaswap|\MPValue{\dots}} sowie 
\Macro'manual'{dotswap|\MPValue{\dots}} genutzt werden. Mit diesen lässt 
...
werden. Dies lässt sich mit \Macro{commaswap|\MPValue{\Macro*{,}}} 
bewerkstelligen. Der Punkt ist demzufolge mit einem Komma durch die Angabe von
\Macro{dotswap|\MPValue{,}} zu ersetzen:%
\footnote{%
...
Wird das Paket \Package'manual'{mathswap} zusammen mit einer \TUDScript-Klasse 
...
folglich das Komma durch einen Punkt \Macro{commaswap|\MPValue{.}} und der 
Punkt mit \Macro{dotswap|\MPValue{\Macro*{,}}} durch ein halbes Leerzeichen 
ersetzt werden:%
%
\CodePreamble{%
