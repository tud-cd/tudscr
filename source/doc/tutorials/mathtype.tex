...
\title{Ein Beitrag zum mathematischen Satz mit \Lettering{LaTeX}}
...
Ein guter Mathematiksatz ist in \Lettering{LaTeX} durchaus Sisyphusarbeit. Wenn 
...
das induzierte Moment einer Verbrennungskraftmaschine~\M{VM_{ind}} angeben
...
in \Lettering{LaTeX}~-- \textbf{nach} dem obligatorischen noch ein optionales 
Argument für einen zusätzlichen Subindex angegeben werden, um damit der 
natürlichen Schreibweise zu entsprechen. Es wird der Befehl \Macro{M} so 
definiert, das dieser entweder mit \Macro{M|\MPName{Index}} oder in der 
Variante \Macro{M|\MPName{Index}\OPName{Indexindex}} mit nachgelagertem 
...
soll zeigen, wie dies prinzipiell mit \Lettering{LaTeX}-Mitteln funktioniert. 
