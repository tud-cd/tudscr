...
des Titels und der Teileseiten in \Color*{HKS41} und die Schrift auf selbigen 
in \Color*{HKS41}\PValue{!30} gehalten Der Hintergrund der Kapitelseiten 
erscheint in \Color*{HKS41}\PValue{!10} und die Schrift in \Color*{HKS41}. Wird 
die Variante mit weniger Farbeinsatz gewählt, bleibt der Hintergrund der 
Gliederungsseiten weiß, die darauf eingesetzte Schrift ist bei Elementen 
\Color*{HKS41}.
...
werden.
\Attention{%
  In diesem Fall \emph{muss} dieses Paket zusammen mit den alten Klassen 
  verwendet werden, wobei sich das Ausgabeergebnis im Vergleich zu den alten 
  Schriften ändern kann. Alternativ können diese auch parallel zu den neuen 
  Schriftfamilien installiert werden. Hierfür werden die Skripte
  \hrfn{https://github.com/tud-cd/tudscrold/releases/download/v1.0/tudfonts_install.bat}{\File{tudfonts\_install.bat}}
  beziehungsweise
  \hrfn{https://github.com/tud-cd/tudscrold/releases/download/v1.0/tudfonts_install.sh}{\File{tudfonts\_install.sh}}
  bereitgestellt.
}
...
Paketes mit den Mitteln von \TUDScript umsetzen lassen.
