...
\footnote{%
  Titelseite (\Macro{maketitle}), Umschlagseite (\Macro{makecover}),
...
separaten Layout erscheint. Für die Umschlagseite werden fast alle Felder und 
Schriftelemente unterstützt, welche auch für den Titel im \CD zur Verfügung 
stehen.
...
durchzuführen. Hierfür ist es sinnvoll, mit \Option{cdcover}[true] die farbige 
Ausprägung der Umschlagseite zu deaktivieren, falls diese für das restliche 
Dokument aktiv ist (\Option{cd}[color]).
\begin{Declaration}[%
...
dieser (\Macro*{partpagestyle}) auf \textbf{plain.tudheadings} gesetzt.
...
geändert werden. \ChangedAt{v2.02} Der verwendete Seitenstil lässt sich mit dem 
Parameter \Key*{\Environment{declarations}}{pagestyle}[\PName{Seitenstil}] 
angeben. Dieser unterstützt auch die \PageStyle{tudheadings}"=Seitenstile. Mit 
dem Parameter \Key*{\Environment{declarations}}{columns}[\PName{Anzahl}] kann 
die Erklärung mehrspaltig gesetzt werden, wenn das Paket \Package{multicol} 
geladen wurde.
...
werden diese auch mit Erst- und Zweitgutachter betitelt.

Für den betreuenden Hochschullehrer (\Macro{professor}) gibt es logischerweise 
keine Möglichkeit
