...
dem Eintrag für das Inhaltsverzeichnis sowie der Ausprägung der automatischen 
...
können, wurde zuvor \vpageref{sec:greek} beschrieben.
%
\begin{values}
\itemfalse
...
\subsection{Unterstützung von OpenType-Schriften}
\label{sec:fonts:fontspec}
\index{OpenType-Schriften}
%
Wird als Dokumentprozessor nicht \hologo{pdfLaTeX} sondern \hologo{XeLaTeX} 
oder \hologo{LuaLaTeX} verwendet, können mit dem Paket \Package{fontspec} auch 
Schriften im OpenType-Format eingebunden werden, womit sich die Auswahl der 
verwendbaren Schriften in einem \hologo{LaTeX}"=Dokument stark vergrößert. 
Prinzipiell ist somit das Einbinden aller OpenType"=Systemschriften möglich.
...
anstelle der PostScript"=Schriften verwendet. Dazu müssen diese über das 
Betriebssystem installiert sein. Eine Installation der PostScript"=Schriften 
mit Skript wäre damit eigentlich obsolet. Allerdings sind für die Kompilierung 
...
ist~-- auch weiterhin die Schriften im PostScript"=Format nötig. Außerdem 
liefern die Schriftfamilien des \CDs keinerlei mathematische Glyphen, sodass 
diese bei der Installation der PostScript"=Schriften via Skript aus den 
Schriftpaketen \Package{cmbright} und \Package{iwona} entnommen werden müssen.

Sollten die PostScript"=Schriften installiert sein, gibt es auch beim Einsatz 
von \hologo{LuaLaTeX} oder \hologo{XeLaTeX} keinen triftigen Grund, diese 
Option zu verwenden. Lediglich wenn eine Installation der PostScript"=Schriften 
nicht möglich ist, soll die Option \Option*{fontspec} die Nutzung der Schriften 
des \CDs ermöglichen. Dabei sei angemerkt, dass es bei aktivierter Option 
sowohl zu Problemen beim Kerning als auch beim mathematischen Satz kommen kann.
...
\itemtrue*
   ...
   oder \hologo{XeLaTeX} jedoch nicht mit \hologo{pdfLaTeX} funktioniert. 
   Sowohl beim mathematischen Satz als auch beim Kerning der Schriften kann es 
   zu Problemen kommen. Die Verwendung dieser Einstellung sollte nur erfolgen, 
   wenn eine Installation der PostScript"=Schriften absolut nicht möglich ist.
\end{values}
\end{Declaration}
...
\hyperref[sec:title]{Titel- und Umschlagseite in \autoref{sec:title}}, die 
...
werden, verschoben werden. Demgegenüber erlaubt es \Length*{headingsvskip}, die 
Überschrift sowohl eines Titelkopfes (\Option{titlepage}[false]) als auch eines 
Kapitels bei deaktivierter Kapitelseite (\Option{chapterpage}[false]) in ihrer 
vertikalen Position anzupassen. Normalerweise werden alle der zuvor genannten 
Überschriften im Layout relativ tief im Textbereich gesetzt. Mit negativen 
Werten wird diese nach oben verschoben, positive Werte setzen diese 
dementsprechend tiefer. Beim Verschieben nach oben, sollte darauf geachtet 
werden, dass diese sich danach noch innerhalb des Satzspiegels befinden.
\end{Declaration}
\end{Declaration}

\subsubsection{Einstellungen für Umschlagseite, Titel, Teile und Kapitel}
Das Verhalten aller Elemente%
\footnote{%
  Umschlagseite (\Macro{makecover}), Titelseite (\Macro{maketitle}),
...
mit Hilfe der folgenden Optionen überschrieben werden. Die gültigen 
...
Unabhängig von der gewählten Variante der Titelseite wird diese immer mit 
\Macro{maketitle} erzeugt. 
...
\Option*{cdcover} kann~-- unabhängig von der Einstellung \Option*{cd}~-- das 
Erscheinungsbild der Umschlagseite geändert werden. 

Die Verwendung der beiden Befehle \Macro{maketitle} und \Macro{makecover} wird 
...
\footnote{%
  ...
  Rückseite) der entsprechenden Elemente des Layouts.
}
...
\PageStyle*{empty.tudheadings} nun zwischen diesen Stilen umgeschaltet wird. 
%
Um auf das normale Verhalten zurückzuschalten, muss lediglich einer der beiden 
Seitenstile \PageStyle*{scrheadings} oder \PageStyle*{plain.scrheadings} 
aktiviert werden.
\end{Declaration}
\end{Declaration}
\end{Declaration}
...
\Package{babel} entweder als Paketoption oder besser noch als Klassenoption 
...
auch direkt und ohne den Parameter \Key*{\Environment{tudpage}}{language} als 
optionales Argument übergeben werden. Des Weiteren wird das Paket 
\Package{multicol} unterstützt. Wird dieses geladen, kann mit dem Parameter 
...
\index{Titel|!(}
%
Für den Titel und die Umschlagseite werden alle Felder unterstützt, die bereits 
durch \KOMAScript{} bereitgestellt werden. Darüber hinaus werden für die 
\TUDScript-Klassen weitere Felder definiert, die Auswirkungen auf die Gestalt 
des Titels und der Umschlagseite haben.
\ChangedAt{v2.02}
Für alle Felder der Umschlag- und Titelseite lässt sich die verwendete Schrift 
anpassen. Dabei werden sowohl die bereits durch \KOMAScript{} bereitgestellten 
Schriftelemente \Font{titlehead}, \Font{subject}, \Font{title}, 
\Font{subtitle}, \Font{author}, \Font{date}, \Font{publishers} und 
\Font{dedication} als auch die neuen \Font{titlepage} und \Font{thesis} 
unterstützt.
%
\begin{Example}
...
Im zweispaltigen Satz (\Option{twocolumn}) wird die Titelseite selbst immer
einspaltig gesetzt. Wird statt dieser jedoch ein Titelkopf verwendet 
(\Option{titlepage}[false]), so folgt normalerweise direkt nach dem Titel der 
zweispaltige Fließtext. Die \TUDScript-Klassen ermöglichen es mit dem Befehl 
\Macro*{maketitleonecolumn}, nach dem Titelkopf zusätzlich auch noch eine 
weitere Textpassage~-- beispielsweise eine Zusammenfassung~-- einspaltig zu 
setzen. Wird der Befehl bei aktivierter Titelseite (\Option{titlepage}[true])
...
\ChangedAt{v2.02}
Wird im zweispaltigen Satz (\Option{twocolumn}) ein Titelkopf verwendet 
(\Option{titlepage}[false]) und auf dem Titel soll das \DDC-Logo im Fuß gesetzt 
werden, so wird der verfügbare Platz für die Spalten falsch berechnet. Abhilfe 
schafft hier das Paket \Package{afterpage}, welches einfach in der Präambel 
geladen werden muss.
\end{Declaration}
\end{Declaration}
...
passieren, dass der Kopf des Haupttitels selbst in die Kopfzeile ragt. Dies 
wird durch die \TUDScript-Klassen nicht geprüft und muss gegebenenfalls vom 
...
in Majuskeln und \DIN gesetzt werden.
...
einer Dissertation angegeben. Die Angabe mehrerer Person erfolgt wie beim 
Befehl \Macro{author} durch die Trennung mittels \Macro{and}.
...
\begin{Declaration}[%
  ...
  v2.02!Unterstützung der Schriftelemente \Font*{titlehead}{,} 
    \Font*{subject}{,} \Font*{title}{,} \Font*{subtitle}{,} \Font*{author}{,} 
    \Font*{publishers} und \Font*{thesis}%
]{\Macro{makecover}\OLParameter{cdlayout}}
\begin{Declaration}{\Key{\Macro{makecover}}{cdlayout}[\PBoolean]}
\begin{Declaration}[v2.02]{\Key{\Macro{makecover}}{pagenumber}[\PBoolean]}
\begin{Declaration}{\Key{\Macro{makecover}}{cdcover}[\PSet]}
...
die Autoren und gegebenenfalls der Verlag ausgegeben. Für die Einstellung 
\Option*{cdcover}[false] wird lediglich der normale \KOMAScript"=Titel{} als 
separate Umschlagseite ausgegeben. 

Der Satzspiegel der Umschlagseite kann mit \Key*{\Macro{makecover}}{cdlayout} 
im optionalen Argument geändert werden. Wird dieser aktiviert, so erscheint das 
Cover~-- unabhängig von der Option \Option{geometry}~-- im asymmetrischen 
...
Befehlen \Macro{coverpagetopmargin}, \Macro{coverpageleftmargin}, 
\Macro{coverpagerightmargin} sowie \Macro{coverpagebottommargin} eingestellt 
...
\item[balanced/twoside]
  ...
  \Attention{%
    Der erzeugte Satzspiegel ist nicht sehr vorteilhaft. Insbesondere auf 
    rechten Seiten rückt das Logo der \TnUD dabei sehr weit an den Blattrand.
  }
\end{values}
%
...
Binderandkorrektur von \unit[5]{mm} mit der Klassenoption \Option{BCOR}[5mm] 
gesetzt werden.
...
eine Binderandkorrektur von \Option{BCOR}[5mm] bzw. \Option{BCOR}[6mm] zu 
wählen.
\end{Example}
...
Mit dem Parameter \Key*{\Environment{declarations}}{language} kann die Sprache 
für den ausgegebenen Text der Erklärung geändert werden.
\ChangedAt{v2.02}
...
\Attention{%
  ...
  mit der Option \Option{declaration}[simple/multiple/fill/nofill] innerhalb 
  der Umgebung \Environment*{declarations} nur wirksam sind, wenn eine 
  Titelseite (\Option{titlepage}[true]) und \emph{keine} Überschriften in Form 
  von Kapiteln (\Option{declaration}[chapter]) verwendet werden.%
}
\end{Declaration}
\end{Declaration}
\end{Declaration}
...
\Option{declaration}[simple/multiple/fill/nofill] erfolgen, beachtet.

Er kann sowohl innerhalb der \Environment{declarations}"=Umgebung als auch 
außerhalb direkt im Dokument verwendet werden und akzeptiert im optionalen 
Argument dabei alle für die genannte Umgebung beschriebenen Parameter.
\end{Declaration}
\end{Declaration}
\end{Declaration}
\end{Declaration}
...
\ChangedAt{v2.02}
Bei der Bezeichnung des Gutachters wird erkannt, ob einer oder mehrere 
angegeben wurden und der entsprechende Eintrag gewählt.
