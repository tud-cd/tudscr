...
\title{%
  Ein \Lettering{LaTeX}-Bundle für Dokumente \mbox{im \CD der} \mbox{\TnUD}%
}
...
Das \TUDScript-Bundle setzt das \TUDCD für \Lettering{LaTeX} um. Die 
enthaltenen Klassen und Pakete basieren auf dem \KOMAScript-Bundle und sind 
sehr eng mit diesen verwoben. Momentan ergänzen sie das Vorlagenbundle von 
Klaus~Bergmann, das auf den \Lettering{LaTeX}"=Standardklassen basiert und als 
veraltet betrachtet werden kann. Die dazugehörigen Klassen sollen mittel- bis 
...
\tableofcontents
\ToDo{\cs{TeX}, \cs{LaTeX}, \cs{app} etc. mit \cs{Lettering} ersetzen}[v2.06]
\ToDo{Index für \cs{cs}, \cs{env} etc. und überprüfen, ob vorhanden}[v2.06]
\ToDo{Zeilenumbüche der Dokumentation in source und doc überarbeiten}[v2.06]
\ToDo{Whitespace am Zeileende via git filter entfernen}[v2.06]
...
\ToDo{%
  dtx-guards überarbeiten: \val{doc}>\val{sourcedoc}, \val{manual}>\val{guide};
  \pkg{tudscrmanual}>\pkg{tudscrtutorial}, \cls{tudscrdoc}>\cls{tudscrsource}?
}[v2.06]
\ToDo{
  Revision aller Hooks: Verwendung von (Before/After[AtEndOf])(Class/Package)
  sowie \cs{AtEndPreamble}, \cs{AfterEndPreamble}, \cs{AtBeginDocument} und 
  \cs{AfterPreamble} prüfen; insbesondere deren Kombination stellt die
  Ausführungsreihenfolge keineswegs sicher; ggf. \cs{DeclareHookRule} nötig
}[v2.07]
...
\ToDo{
  \cs{kernel@ifnextchar}, \cs{kernel@ifstar} etc. prüfen und bestenfalls mit 
  \cs{NewDocumentCommand} ersetzen
}[v2.07]
