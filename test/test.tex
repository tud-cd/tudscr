...
\renewcommand*\TUD@parameter@set[2]{%
  \@expandtwoargs\in@{=}{#2}%
  \ifin@%
    \@expandtwoargs\kvsetkeys{#1}{#2}%
  \else%
    \@expandtwoargs\kvsetkeys{#1}{\TUD@parameter@nokey=#2}%
  \fi%
}
\renewcommand*\TUD@parameter@defaulthandler[3]{%
  \ifstr{#2}{\TUD@parameter@nokey}{%
    \def\@tempa##1{%
      ...
      \ifxnumber{\@tempb}{%
        ...
        \iflanguageloaded{\@tempb}{%
          ...
          \ClassError{\TUDClassName}{%
            Unknown default value `\@tempb' for #1%
          }{%
            If value `\@tempb' is a language, you haven't loaded it.%
          }%
        }%
      }%
    }%
...
\renewcommand*\TUD@parameter@handler[1][]{%
% TODO: @nokey@ hier zusätzlich oder ausschließlich testen?
%       (nicht in defaulthandler)
  \ifxblank{#1}{%
    \TUD@parameter@checkfamily{\TUD@parameter@handler}%
...
\TUD@parameter{tudpage}{%
  \TUD@parameter@define{key}{\def\testkey{#1}}%
%  \TUD@parameter@handler{\TUD@parameter@defaulthandler{tudpage}{#1}{#2}}%
  \TUD@parameter@handler{\def\testkey{\kv@value}}%
}

\TUD@parameter@set{tudpage}{key=blaa}%

\testkey

\TUD@parameter@set{tudpage}{blaß}%
%
\testkey

\TUD@parameter@set{tudpage}{2}%

%\testkey

%\def\testvalue{nothing}
%\define@key{test}{key}{\def\testvalue{#1}}
%\kv@set@family@handler{test}{%
%  \typeout{++++++ *****}
%  \ifx\kv@value\relax%
%    \kv@handled@true%
%    \def\kv@key{key}%
%    \def\kv@value{key}%
%%    aaa
%%    \expandafter\kvsetkeys\expandafter{test}\expandafter{key=\kv@key}%
%  \else%
%    \kv@handled@false%
%  \fi%
%}
%
%\kvsetkeys{test}{key=blaß}
%
%\testvalue
%
%\kvsetkeys{test}{blasser}
%
%\testvalue
%
%%\kvsetkeys{test}{aaa=blaer}
%
%\testvalue

\makeatother

%\noticeform[{ößch}]{text}{\item aaa}
%\taskform[ösch]{text}{\item aaa}
%\evaluationform[ösch]{text}{aaa}{aaa}{aaa}
\end{document}
