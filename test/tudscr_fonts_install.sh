...
proof_userinput()
{
  ...
  echo   $texmfpath
  echo
  echo   Soll der angegebene Pfad genutzt werden?
  if [ ! -d $texmfpath ]; then
    echo "Der gewuenschte Ordner existiert nicht, wird jedoch erstellt."
  fi
...
mkvaldir()
{
  mkdir -p $1
  if [ $? -ne 0 ] ; then
    echo Keine Schreibberechtigung für den Pfad
    echo $1
    echo "Versuchen Sie das Ausführen mit 'sudo bash <Skriptname>'"
    abort
  fi
}
#
...
if [ -z "$texpath" ]; then
  ...
  echo Möglicherweise hilft der Aufruf des Skriptes mit:
