...
\sflogo{LaTeX}"=Standardschriften beziehungsweise ein anderes Schriftpaket zu 
verwenden. Die Erläuterungen dazu sind in \autoref{sec:text} zu finden. Zur 
Verwendung der Schriften mit \sflogo{LaTeX}"~Klassen, welche nicht zum 
...
\begin{DeclareValues}
\itemval=false=
  Es werden die \sflogo{LaTeX}"=Standardschriften und nicht die Hausschrift 
  des \CDs verwendet. Der Anwender kann beliebige Schriftpakete nutzen.%
  \footnote{%
    Für die Verwendung der klassischen \sflogo{LaTeX}"~Schriften, ist das Paket 
...
\itemval=nomath,nocdmath=<\Option{cdmath=false}>
  ...
  werden die \sflogo{LaTeX}"=Standardschriften verwendet und es lassen sich 
...
\itemval=texgreek,standardgreek=<\Option{slantedgreek=standard}>[v2.06]
  Es kommt der \sflogo{LaTeX}"~Standard zum Einsatz, bei dem griechische 
...
\begin{DeclareValues}
\itemval=false=
  Es werden die normalen \sflogo{LaTeX}"=Serifenschriften beziehungsweise die 
...
\itemval=standard,latex=[v2.06]
  ...
  griechischen Lettern dem \sflogo{LaTeX}"~Standard beim mathematischen Satz.
\end{DeclareValues}
\end{Declaration}
...
zahlreiche zusätzliche für die \sflogo{LaTeX}"=Standardschriften zur Verfügung. 
...
Dieser Befehl wird sowohl bei den \sflogo{LaTeX}"=Standardklassen als auch bei 
