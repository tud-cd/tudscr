...
\printdeclarationlist(%
  ...
  mit \Macro*{MakeTextUppercase|\MPValue{\Macro*{textdbn|\MPName{Text}}}} ein 
...
aktiviert wurde. Hierfür wird das Paket \Package{textcase} zusammen mit den 
alten Schriftfamilien geladen und intern \Macro{MakeTextUppercase} verwendet. 
...
\Attention{%
  ...
  \InlineDeclaration{\Class{tudposter}} respektive eine der \TUDScript-Klassen 
  in der \emph{Version~v1.0} genutzt haben, können Sie dieses \autorefname ohne 
  Weiteres überspringen. Sämtliche der hier vorgestellten Optionen und Befehle 
...
beispielsweise das Paket \Package{glossaries} oder~-- mit Abstrichen~-- das 
nicht ganz so umfangreiche Paket \Package{nomencl} verwiesen.
\end{Declaration}
\end{Declaration}
\index{Kompatibilität!\Class{tudbook}|)}%
...
festgelegten Satzspiegel mittels des Befehls \Macro{geometry} aus dem Paket 
\Package{geometry} über die drei Werte \PValue{paper}, \PValue{layout} sowie 
\PValue{layoutoffset} anzupassen und das erzeugte Papierformat um den gegebenen 
...
Weitere Informationen hierzu sind in \fullref{sec:tips:crop} sowie im 
