...
Ähnliche Funktionalitäten werden bereits durch die Pakete \Package{ziffer} und 
\Package*{icomma} bereitgestellt. Bei letzterem muss jedoch beim Verfassen des 
Dokumentes durch den Autor beachtet werden, ob das verwendete Komma einem 
Dezimaltrennzeichen entspricht ($t=1,\!2$) oder einem normalen Komma im 
Mathematiksatz ($z=f(x,y)$), wo ein gewisser Abstand nach dem Komma 
durchaus gewünscht ist und entsprechend ein Leerzeichen danach verwendet werden 
oder nicht. Das Paket \Package{ziffer} liefert die gewünschte Funktionalität,%
\footnote{kein Leerraum nach Komma, wenn direkt danach eine Ziffer folgt}
ist allerdings etwas unflexibel, was den Umgang mit Trennzeichen anbelangt. Als 
Alternative zu diesem Paket kann außerdem \Package{ionumbers} verwendet werden.
