...
\itempkg{graphicx}
  ...
  wird \Macro{includegraphics} genutzt.
\itempkg{xcolor}
  \index{Farben}%
...
\itempkg{etoolbox,xpatch,letltxmacro}
  ...
  \ToDo{kvsetkeys, environ, trimspaces nicht mehr verwenden, Doku raus!}[v2.07]
...
\itempkg{csquotes}
  ...
  \ToDo{noindentafter raus? Update auf hooks? geht das mit xparse?}[v2.07]
...
\itempkg{ziffer}
  Das Komma wird als Dezimaltrennzeichen zwischen zwei Ziffern definiert, was 
  für nicht-englischsprachige Dokumente sinnvoll sein kann. Folgt dem Komma 
  keine Ziffer, wird wie normalerweise der obligatorische Freiraum gesetzt.
\index{Zifferngruppierung|)}%
\end{DeclarePackages}
%
...
\itempkg{tabularborder}
  ...
  \Package{tabu} werden nicht unterstützt. Wie sich dieser Abstand für die zu 
  den Paketen korrespondierenden Tabellen"=Umgebungen manuell entfernt lässt, 
  ist unter \autoref{sec:tips:table} zu finden.
\itempkg{tabu}[%
...
%
\begin{DeclarePackages}
\itempkg{standalone}<\Bundle||{standalone},\Class{standalone}>
...
\itempkg{xkeyval,pgfkeys,expkv,l3keys}[v2.02]
  \index{Befehlsdeklaration}%
  ...
  können. Diese kann auch für eigens definierte Makros genutzt werden, um 
  mehrere Parameter innerhalb eines optionalen Argumentes nutzen zu können.
  
  Soll~-- beispielsweise zur Entwicklung eigener Pakete~-- gänzlich auf die 
  Funktionalitäten von \KOMAScript verzichtet und dennoch eine Verarbeitung
  von Optionen im Schlüssel"=Wert"=Format gewünscht sein, kann das Paket 
  \Package{xkeyval} zum Einsatz kommen, welches insbesondere Möglichkeiten zur 
  erweiterten Deklaration unterschiedlicher Typen von Schlüsseln bereitstellt. 
  Alternativ dazu lässt sich auch die \Package{expkv}"~Familie beziehungsweise 
  \Package{pgfkeys} oder für \hologo{LaTeX3} auch \Package{l3keys} nutzen.
