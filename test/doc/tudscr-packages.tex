...
%\itempkg{mweights}
%  \index{Schriftstärke}%
%  In \sflogo{LaTeX} existieren die Schriftfamilien für Serifenschriften 
...
sich für mich persönlich bei der Arbeit mit \sflogo{LaTeX} bewährt haben. 
...
\itempkg{hyphsubst,dehyph-exptl}
  \index{Worttrennung|!}%
  Die möglichen Trennstellen von Wörtern wird von \sflogo{LaTeX} mithilfe 
...
\itempkg{setspace}
  ...
  von \sflogo{LaTeX} ist an und für sich nicht notwendig. Sinnvoll ist dies 
...
\itempkg{quoting}
  \index{Zitate}%
  \sflogo{LaTeX} bietet von Haus aus \emph{zwei} verschiedene Umgebungen~-- 
...
\itempkg{selnolig}(\Application'CTAN:delig'{DeLig|})
  ...
  \footnote{%
    ...
    \enquote{ffl} bei den meisten \sflogo{LaTeX}"~Schriften.%
  }
...
\itempkg{ellipsis}
  In \sflogo{LaTeX} folgen den Befehlen für Auslassungspunkte (\Macro{dots} und 
...
%
Die meisten \sflogo{LaTeX}"=Entwicklungsumgebungen enthalten eine integrierte 
...
\itempkg{spelling}
  ...
  der reine Textanteil aus dem \sflogo{LaTeX}"=Dokument extrahiert~-- wobei 
...
\itempkg{lmodern,cfr-lm}
  \index{Schriftart}%
  Soll mit den klassischen \sflogo{LaTeX}"=Standardschriften gearbeitet werden, 
...
\itempkg{sansmathfonts,sansmath}
  Sollten die normalen \sflogo{LaTeX}"~Schriften Computer~Modern verwendet 
...
\itempkg{ionumbers}
  ...
  Verwendung von \sflogo{LaTeX}. Insbesondere, wenn in einem deutschsprachigen 
...
\itempkg{enumitem}
  ...
  Funktionalitäten der \sflogo{LaTeX}"=Umgebungen für Aufzählungen 
...
\itempkg{biblatex}
  ...
  Das Paket kann als legitimer Nachfolger zu \sflogo{BibTeX} gesehen werden. 
  ...
  besser umsetzbar als mit \sflogo{BibTeX}. Wird \Application{biber} für die 
...
\itempkg{booktabs}
  Für einen guten Tabellensatz mit \sflogo{LaTeX} gibt es bereits zahlreiche 
...
\itempkg{tabu}[%
  ...
  \Attention{%
    Durch Änderungen am \sflogo{LaTeXe}"~Kernel wurde das Paket unbrauchbar, 
    weshalb durch das \GitHubRepo[\sflogo{LaTeX3}-Projektteam](tabu-fixed/tabu) 
...
Für das Erstellen eigener Vektorgrafiken, welche die \sflogo{LaTeX}"~Schriften 
...
Text an \sflogo{LaTeX} unterstützen, werden genutzt. Für das Programmieren von 
...
Weiterverarbeitung durch \sflogo{LaTeX} erlaubt, möchte ich auf die freien 
...
\itempkg{svg}
  ...
  SVG"~Grafik in ein \sflogo{LaTeX}"=Dokument durchgeführt werden. Weitere 
...
\itempkg{marginnote}
  ...
  in \sflogo{LaTeX} spezielle Gleitobjekte, wodurch selbige nicht immer direkt 
...
\itempkg{xparse}
  ...
  \index{Umgebungen}%
  \index{Befehlsdeklaration}%
  Dieses mächtige Paket entstammt dem \sflogo{LaTeX3}"~Projekt und bietet für 
  die Erstellung eigener Befehle und Umgebungen einen alternativen Ansatz zu 
  den bekannten \sflogo{LaTeX}"=Deklarationsbefehlen \Macro*{newcommand} und 
...
\itempkg{keyval,xkeyval,pgfkeys,expkv,l3keys}[v2.02]
  ...
  für \sflogo{LaTeX3} auch \Package{l3keys} genutzt werden.
...
\itempkg{calc}
  ...
  Low"~Level"~\sflogo{TeX}"=Primitiven durchführen. Dieses Paket stellt eine 
...
\itempkg{listings}[v2.02]
  ...
  \sflogo{LaTeX}. Es bietet die Möglichkeit, externe Quelldateien einzulesen 
...
\itempkg{selinput,inputenc}
  ...
  aktuellen \sflogo{LaTeX}"=Entwicklungsumgebung unterstützt wird. Dies sollte 
  ...
  Paket \Package{selinput} (automatisch) für \sflogo{LaTeX} festgelegt werden:
\begin{quoting}[rightmargin=0pt]
\begin{Code}
\usepackage{selinput}
