...
\else
\faculty{Fakult\"at Mathematik und Naturwissenschaften}
\department{Fachrichtung Mathematik}
\institute[Institut für Algebra]{}
\chair[Professur für \TeX nische Algebra]{}
\fi
...
%\ifoldclass\else
%\geometry{showcrop,paper=b0paper,layout=a0paper,layoutoffset=2cm}
%\fi
%\def\schnittrand{1em}
...
\newcommand*\getscale[2]{%
  \setlength\@tempdima{\p@*\ratio{#1}{#2}}%
  \edef\myscale{\strip@pt\@tempdima}%
}
\makeatother
\newcommand\aufbau{%
\getscale{\linewidth}{8.7cm}
% \begin{figurehere}
\begin{tikzpicture}[ultra thick,scale={\myscale},anchor=center,text 
width=]\scriptsize
...
\pagebreak
\telefon{}%
\homepage{http://www.math.tu-dresden.de/~schlemme/tudmathposter/}%
\email{Tobias.Schlemmer@mailbox.tu-dresden.de}%
\title{2. Plakat}%
